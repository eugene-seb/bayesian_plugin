...
/**
 * Fallback page of /mod/bayesian/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $bayesian, $pagevars) =
        question_edit_setup('editq', '/mod/bayesian/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/bayesian/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new bayesian_add_random_form(new moodle_url('/mod/bayesian/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    bayesian_add_random_questions($bayesian, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids);
...
// Setup $PAGE.
$streditingbayesian = get_string('editinga', 'moodle', get_string('modulename', 'bayesian'));
$PAGE->navbar->add($streditingbayesian);
$PAGE->set_title($streditingbayesian);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$bayesianname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            throw new \moodle_exception('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontobayesian', 'bayesian', $bayesianname), 2);
$mform->display();
echo $OUTPUT->footer();
