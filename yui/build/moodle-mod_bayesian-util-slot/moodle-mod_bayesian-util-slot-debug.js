YUI.add('moodle-mod_bayesian-util-slot', function (Y, NAME) {

/**
 * A collection of utility classes for use with slots.
 *
 * @module moodle-mod_bayesian-util
 * @submodule moodle-mod_bayesian-util-slot
 */

Y.namespace('Moodle.mod_bayesian.util.slot');

/**
 * A collection of utility classes for use with slots.
 *
 * @class Moodle.mod_bayesian.util.slot
 * @static
 */
Y.Moodle.mod_bayesian.util.slot = {
    CSS: {
        SLOT: 'slot',
    ...
    reorderSlots: function() {
        ...
        slots.each(function(slot) {
            ...
            if (!Y.Moodle.mod_bayesian.util.page.getPageFromSlot(slot)) {
                // Move the next page to the front.
                var nextpage = slot.next(Y.Moodle.mod_bayesian.util.page.SELECTORS.PAGE);
                slot.swap(nextpage);
            }
    ...
    updateOneSlotSections: function() {
        Y.all('.mod-bayesian-edit-content ul.slots li.section').each(function(section) {
    ...
    remove: function(slot) {
        var page = Y.Moodle.mod_bayesian.util.page.getPageFromSlot(slot);
        slot.remove();
        // Is the page empty.
        if (!Y.Moodle.mod_bayesian.util.page.isEmpty(page)) {
            return;
        }
        // If so remove it. Including add menu and page break.
        Y.Moodle.mod_bayesian.util.page.remove(page);
    },
    ...
    addPageBreak: function(slot) {
        var nodetext = M.mod_bayesian.resource_toolbox.get('config').addpageiconhtml;
    ...
    reorderPageBreaks: function() {
        ...
        slots.each(function(slot, key) {
            ...
            var action = '';
            var iconname = '';
            if (Y.Moodle.mod_bayesian.util.page.isPage(nextitem)) {
            ...
            pagebreaklink.set('title', M.util.get_string(action, 'bayesian'));
            pagebreaklink.setData('action', action);
            // Update the image title.
            var icon = pagebreaklink.one(this.SELECTORS.ICON);
            icon.set('title', M.util.get_string(action, 'bayesian'));
            icon.set('alt', M.util.get_string(action, 'bayesian'));
    ...
    updateDependencyIcon: function(slot, requiresprevious) {
        ...
        if (requiresprevious) {
            link.set('title', M.util.get_string('questiondependencyremove', 'bayesian', a));
            link.setData('action', 'removedependency');
            window.require(['core/templates'], function(Templates) {
                Templates.renderPix('t/locked', 'core', M.util.get_string('questiondependsonprevious', 'bayesian')).then(
                    function(html) {
                        icon.replace(html);
                    }
                );
            ...
            link.set('title', M.util.get_string('questiondependencyadd', 'bayesian', a));
            link.setData('action', 'adddependency');
            window.require(['core/templates'], function(Templates) {
                Templates.renderPix('t/unlocked', 'core', M.util.get_string('questiondependencyfree', 'bayesian')).then(
                    function(html) {
                        icon.replace(html);
                    }
                );
...
}, '@VERSION@', {"requires": ["node", "moodle-mod_bayesian-util-base"]});
