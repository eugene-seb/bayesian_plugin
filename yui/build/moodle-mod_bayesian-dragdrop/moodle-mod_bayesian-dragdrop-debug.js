YUI.add('moodle-mod_bayesian-dragdrop', function (Y, NAME) {

/* eslint-disable no-unused-vars */
/**
 * Drag and Drop for bayesian sections and slots.
 *
 * @module moodle-mod-bayesian-dragdrop
 */
...
var CSS = {
    ...
    CONTENT: 'content',
    COURSECONTENT: 'mod-bayesian-edit-content',
 ...
 *
 * @class M.mod_bayesian.dragdrop.section
 * @constructor
...
Y.extend(DRAGSECTION, M.core.dragdrop, {
    ...
    initializer: function() {
        ...
        if (this.sectionlistselector) {
            ...
            del.dd.plug(Y.Plugin.DDConstrained, {
                // Keep it inside the .mod-bayesian-edit-content
    ...
    drop_hit: function(e) {
        ...
        if (dragnodeid === dropnodeindex) {
            Y.log("Skipping move - same location moving " + dragnodeid + " to " + dropnodeindex,
                  'debug', 'moodle-mod_bayesian-dragdrop');
            return;
        }

        Y.log("Moving from position " + dragnodeid + " to position " + dropnodeindex, 'debug', 'moodle-mod_bayesian-dragdrop');
        ...
        params.bayesianid = this.get('bayesianid');
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    try {
                        ...
                        M.mod_bayesian.edit.process_sections(Y, sectionlist, responsetext, loopstart, loopend);
                    } catch (e) {
                        // Ignore.
                    }
                    ...
                    do {
                        ...
                        for (index = loopstart; index <= loopend; index++) {
                            if (Y.Moodle.core_course.util.section.getId(sectionlist.item(index - 1)) >
                                ...
                                M.mod_bayesian.edit.swap_sections(Y, index - 1, index);
    ...
    NAME: 'mod_bayesian-dragdrop-section',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_bayesian.init_section_dragdrop = function(params) {
...
Y.extend(DRAGRESOURCE, M.core.dragdrop, {
    ...
    setup_for_section: function() {
        Y.Node.all('.mod-bayesian-edit-content ul.slots ul.section').each(function(resources) {
    ...
    drop_hit: function(e) {
        ...
        params.field = 'move';
        params.id = Number(Y.Moodle.mod_bayesian.util.slot.getId(dragnode));
        ...
        if (previousslot) {
            params.previousid = Number(Y.Moodle.mod_bayesian.util.slot.getId(previousslot));
        }

        var previouspage = dragnode.previous(SELECTOR.PAGE);
        if (previouspage) {
            params.page = Number(Y.Moodle.mod_bayesian.util.page.getId(previouspage));
        }
        ...
        Y.io(uri, {
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    M.mod_bayesian.bayesianbase.invoke_function('set_visibility_resource_ui', params);
                    this.unlock_drag_handle(drag, CSS.EDITINGMOVE);
                    window.setTimeout(function() {
                        spinner.hide();
                    }, 250);
                    M.mod_bayesian.resource_toolbox.reorganise_edit_page();
                },
                failure: function(tid, response) {
                    this.ajax_failure(response);
    ...
    NAME: 'mod_bayesian-dragdrop-resource',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_bayesian = M.mod_bayesian || {};
M.mod_bayesian.init_resource_dragdrop = function(params) {
    ...
    "requires": [
        ...
        "moodle-mod_bayesian-util-base",
        "moodle-mod_bayesian-util-page",
