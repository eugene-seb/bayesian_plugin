YUI.add('moodle-mod_bayesian-toolboxes', function (Y, NAME) {
...
var CSS = {
    ...
    SELECTOR = {
        ...
        SHOW: 'a.' + CSS.SHOW,
        SLOTLI: 'li.slot',
        SUMMARKS: '.mod_bayesian_summarks'
    },
...
Y.extend(TOOLBOX, Y.Base, {
    ...
    send_request: function(data, statusspinner, success_callback, optionalconfig) {
        ...
        data.bayesianid = this.get('bayesianid');
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'bayesian', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a bayesian in editing mode.
 *
 * @module mod_bayesian-resource-toolbox
 * @namespace M.mod_bayesian.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    initializer: function() {
        M.mod_bayesian.bayesianbase.register_module(this);
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'bayesian', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_bayesian.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_bayesian.util.slot.remove(element);
    ...
    delete_multiple_action: function(ev) {
        ...
        if (typeof problemsection !== 'undefined') {
            var alert = new M.core.alert({
                title: M.util.get_string('cannotremoveslots', 'bayesian'),
                message: M.util.get_string('cannotremoveallsectionslots', 'bayesian', problemsection)
            });
    ...
    delete_multiple_with_confirmation: function(ev) {
        ...
        Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
            var slot = Y.Moodle.mod_bayesian.util.slot.getSlotFromComponent(node);
            ids += ids === '' ? '' : ',';
            ids += Y.Moodle.mod_bayesian.util.slot.getId(slot);
            slots.push(slot);
        });
        var element = Y.one('div.mod-bayesian-edit-content');
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('areyousureremoveselected', 'bayesian'),
            modal: true
        });
        ...
        confirm.on('complete-yes', function() {
            ...
            this.send_request(data, spinner, function(response) {
                ...
                if (response.deleted) {
                    // Actually remove the element.
                    Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
                        Y.Moodle.mod_bayesian.util.slot.remove(node.ancestor('li.activity'));
                    });
    ...
    edit_maxmark: function(ev, button, activity) {
        ...
        var instancemaxmark = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class': 'resource',
                'field': 'getmaxmark',
                'id': Y.Moodle.mod_bayesian.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_bayesian.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_bayesian.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_bayesian.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_bayesian.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_bayesian.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_bayesian.util.slot.reorderSlots();
        Y.Moodle.mod_bayesian.util.slot.reorderPageBreaks();
        Y.Moodle.mod_bayesian.util.page.reorderPages();
        Y.Moodle.mod_bayesian.util.slot.updateOneSlotSections();
        Y.Moodle.mod_bayesian.util.slot.updateAllDependencyIcons();
    },

    NAME: 'mod_bayesian-resource-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_bayesian.resource_toolbox = null;
M.mod_bayesian.init_resource_toolbox = function(config) {
    M.mod_bayesian.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_bayesian.resource_toolbox;
};
/**
 ...
 *
 * @module moodle-mod_bayesian-toolboxes
 * @namespace M.mod_bayesian.toolboxes
 */
 ...
 *
 * This class is responsible for managing AJAX interactions with sections
 * when adding, editing, removing section headings when editing a bayesian.
 *
 * @class section
 * @constructor
 * @extends M.mod_bayesian.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'bayesian', activity.getData('sectionname')),
            modal: true
        });
    ...
    edit_section_title_submit: function(ev, activity, oldtext) {
        ...
        if (newtext !== null && newtext !== oldtext) {
            ...
            if (newtext.trim() === '') {
                ...
                instancesectiontext = M.util.get_string('sectionnoname', 'bayesian');
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'bayesian', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'bayesian', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'bayesian', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'bayesian', response.instancesection));
                    }
                }
            });
        }
    ...
    NAME: 'mod_bayesian-section-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_bayesian.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_bayesian-bayesianbase",
        "moodle-mod_bayesian-util-slot",
