...
class qbank_helper {
    ...
    /**
     * Get the information about which questions should be used to create a bayesian attempt.
     *
     ...
     *
     * @param int $bayesianid the id of the bayesian to load the data for.
     * @param \context_module $bayesiancontext the context of this bayesian.
     * @param int|null $slotid optional, if passed only load the data for this one slot (if it is in this bayesian).
     * @return array indexed by slot, with information about the content of each slot.
     */
    public static function get_question_structure(int $bayesianid, \context_module $bayesiancontext,
            int $slotid = null): array {
        global $DB;

        $params = [
            'draft' => question_version_status::QUESTION_STATUS_DRAFT,
            'bayesiancontextid' => $bayesiancontext->id,
            'bayesiancontextid2' => $bayesiancontext->id,
            'bayesiancontextid3' => $bayesiancontext->id,
            'bayesianid' => $bayesianid,
            'bayesianid2' => $bayesianid,
        ];
        $slotidtest = '';
        $slotidtest2 = '';
        if ($slotid !== null) {
        ...
        $slotdata = $DB->get_records_sql("
             ...
             -- case where a particular question has been added to the bayesian.
             LEFT JOIN {question_references} qr ON qr.usingcontextid = :bayesiancontextid AND qr.component = 'mod_bayesian'
             ...
             LEFT JOIN (
                   SELECT lv.questionbankentryid, MAX(lv.version) AS version
                     FROM {bayesian_slots} lslot
                     JOIN {question_references} lqr ON lqr.usingcontextid = :bayesiancontextid2 AND lqr.component = 'mod_bayesian'
                    ...
                    WHERE lslot.bayesianid = :bayesianid2
                          $slotidtest2
             ...
             LEFT JOIN {question_set_references} qsr ON qsr.usingcontextid = :bayesiancontextid3 AND qsr.component = 'mod_bayesian'
                                        AND qsr.questionarea = 'slot' AND qsr.itemid = slot.id

                 WHERE slot.bayesianid = :bayesianid
                       $slotidtest
        ...
        foreach ($slotdata as $slot) {
            ...
            if ($slot->filtercondition) {
                ...
                $slot->qtype = 'random';
                $slot->name = get_string('random', 'bayesian');
                $slot->length = 1;
                ...
                $slot->name = get_string('missingquestion', 'bayesian');
                $slot->maxmark = 0;
                $slot->questiontext = ' ';
     ...
     * @param int $bayesianid the id of the bayesian to load the data for.
     * @param \context_module $bayesiancontext the context of this bayesian.
     * @param int $slotid optional, if passed only load the data for this one slot (if it is in this bayesian).
     ...
     */
    public static function choose_question_for_redo(int $bayesianid, \context_module $bayesiancontext,
            int $slotid, qubaid_condition $qubaids): int {
        $slotdata = self::get_question_structure($bayesianid, $bayesiancontext, $slotid);
