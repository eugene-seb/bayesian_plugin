...
/**
 * Renderer outputting the bayesian editing UI.
 *
...
namespace mod_bayesian\output;
defined('MOODLE_INTERNAL') || die();

use mod_bayesian\question\bank\qbank_helper;
use \mod_bayesian\structure;
use \html_writer;
use renderable;
...
class edit_renderer extends \plugin_renderer_base {
    ...
    protected $togglegroup = 'bayesian-questions';

    /**
     * Render the edit page
     *
     * @param \bayesian $bayesianobj object containing all the bayesian settings information.
     * @param structure $structure object containing the structure of the bayesian.
     ...
     */
    public function edit_page(\bayesian $bayesianobj, structure $structure,
        \core_question\local\bank\question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading(get_string('questions', 'bayesian'));

        // Information at the top.
        $output .= $this->bayesian_state_warnings($structure);

        $output .= html_writer::start_div('mod_bayesian-edit-top-controls');
        ...
        $output .= $this->bayesian_information($structure);
        $output .= html_writer::end_tag('div');
        $output .= $this->maximum_grade_input($structure, $pageurl);
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_div('d-flex justify-content-between flex-wrap mb-1');
        $output .= html_writer::start_div('mod_bayesian-edit-action-buttons btn-group edit-toolbar', ['role' => 'group']);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->selectmultiple_button($structure);
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_div('d-flex flex-column justify-content-around');
        $output .= $this->total_marks($bayesianobj->get_bayesian());
        $output .= html_writer::end_tag('div');
        $output .= html_writer::end_tag('div');
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->js_call_amd('mod_bayesian/bayesianquestionbank', 'init', [
                $thiscontext->id
            ]);

            $this->page->requires->js_call_amd('mod_bayesian/add_random_question', 'init', [
                $thiscontext->id,
                $pagevars['cat'],
    ...
    /**
     * Render any warnings that might be required about the state of the bayesian,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the bayesian structure.
     * @return string HTML to output.
     */
    public function bayesian_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function bayesian_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'bayesian', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'bayesianopeningstatus',
    ...
    /**
     * Render the form for setting a bayesian' overall grade
     *
     * @param structure $structure the bayesian structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
    ...
    public function maximum_grade_input($structure, \moodle_url $pageurl) {
        ...
        $output .= html_writer::start_tag('form', array('method' => 'post', 'action' => 'edit.php',
                'class' => 'bayesiansavegradesform form-inline'));
        ...
        $output .= html_writer::empty_tag('input', array('type' => 'text', 'id' => 'inputmaxgrade',
                ...
                'value' => $structure->formatted_bayesian_grade(),
                'class' => 'form-control'));
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'class' => 'btn btn-secondary ml-1',
                'name' => 'savechanges', 'value' => get_string('save', 'bayesian')));
     ...
     * @param structure $structure the structure of the bayesian being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {
        $header = html_writer::tag('span', get_string('repaginatecommand', 'bayesian'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'bayesian'),
        ...
        if (!$structure->can_be_repaginated()) {
            ...
            $this->page->requires->js_call_amd('mod_bayesian/repaginate', 'init');
        }
    ...
    protected function selectmultiple_button(structure $structure) {
        $buttonoptions = array(
            ...
            'value' => get_string('selectmultipleitems', 'bayesian'),
        ...
        return html_writer::tag('button', get_string('selectmultipleitems', 'bayesian'), $buttonoptions);
    }
    ...
    protected function selectmultiple_controls(structure $structure) {
        ...
        $buttondeleteoptions = array(
            ...
            'value' => get_string('deleteselected', 'mod_bayesian'),
            'class' => 'btn btn-secondary',
            'data-action' => 'toggle',
        ...
        $output .= html_writer::tag('div',
                        html_writer::tag('button', get_string('deleteselected', 'mod_bayesian'), $buttondeleteoptions) .
                        " " .
        ...
        $toolbaroptions = array(
            ...
            'role' => 'toolbar',
            'aria-label' => get_string('selectmultipletoolbar', 'bayesian'),
        );
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'bayesian');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'bayesian', $select) .
    ...
    /**
     * Render the total marks available for the bayesian.
     *
     * @param \stdClass $bayesian the bayesian settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($bayesian) {
        $totalmark = html_writer::span(bayesian_format_grade($bayesian, $bayesian->sumgrades), 'mod_bayesian_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'bayesian', $totalmark),
     ...
     *
     * @param structure $structure the structure of the bayesian being edited.
     * @param \stdClass $section The bayesian_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if ($section->heading) {
            ...
            $sectionheadingtext = get_string('sectionnoname', 'bayesian');
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'bayesian', $sectionheadingtext),
     ...
     *
     * @param structure $structure object containing the structure of the bayesian.
     * @param \stdClass $section data from the bayesian_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'bayesian');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'bayesian'),
    ...
    /**
     * Render an icon to remove a section from the bayesian.
     *
     * @param object $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'bayesian', format_string($section->heading));
        $url = new \moodle_url('/mod/bayesian/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the bayesian.
     *
     * This function calls {@link core_course_renderer::bayesian_section_question()}
     *
     * @param structure $structure object containing the structure of the bayesian.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_bayesian($slot) &&
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-bayesian-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'bayesian'), array('class' => 'add-menu'));
        ...
        // Disable the link if bayesian has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'bayesian');
        }

        // Get section, page, slotnumber and maxmark.
        $actions = array();

        // Add a new question to the bayesian.
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'bayesian', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'bayesian');
        }
        ...
        if ($structure->can_add_random_questions()) {
            $returnurl = new \moodle_url('/mod/bayesian/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
            $params = ['returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion'];
            $url = new \moodle_url('/mod/bayesian/addrandom.php', $params);
            ...
            if ($page) {
                $title = get_string('addrandomquestiontopage', 'bayesian', $page);
            } else {
                $title = get_string('addrandomquestionatend', 'bayesian');
            }
    ...
    /**
     * Render the form that contains the data for adding a new question to the bayesian.
     *
     * @param structure $structure object containing the structure of the bayesian.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, int $slot, \moodle_url $pageurl) {
        ...
        if ($structure->get_slot_by_number($slot)->qtype !== 'random') {
            ...
            $this->page->requires->js_call_amd('mod_bayesian/question_slot', 'init', [$slotid]);
        }

        // Render the question slot template.
        $output .= $this->render_from_template('mod_bayesian/question_slot', $data);
    ...
    public function get_checkbox_render(structure $structure, int $slot) : string {
        ...
        $checkbox = new \core\output\checkbox_toggleall($this->togglegroup, false,
            [
                ...
                'label' => get_string('selectquestionslot', 'bayesian', $questionslot),
    ...
    public function get_action_icon(structure $structure, int $slot, \moodle_url $pageurl) : string {
        ...
        $questionicons = '';
        if ($qtype !== 'random') {
            $questionicons .= $this->question_preview_icon($structure->get_bayesian(),
     ...
     * @return string HTML to output.
     */
    public function question_preview_icon($bayesian, $questiondata, $label = null, $variant = null) {
        ...
        $url = bayesian_question_preview_url($bayesian, $question, $variant);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'bayesian');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'bayesian');
    ...
    /**
     * Render an icon to remove a question from the bayesian.
     *
     * @param structure $structure object containing the structure of the bayesian.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the bayesian.
     *
     * @param structure $structure object containing the structure of the bayesian.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('bayesianid' => $structure->get_bayesianid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'bayesian');
            $image = $this->image_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'bayesian');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'bayesian', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'bayesian'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'bayesian', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'bayesian'),
                    'moodle', array('title' => ''));
    ...
    /**
     * Renders html to display a name with the link to the question on a bayesian edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = bayesian_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'bayesian').' '.$title));
    ...
    public function random_question(structure $structure, $slotnumber, $pageurl) {
        ...
        $editurl = new \moodle_url('/mod/bayesian/editrandom.php',
                array('returnurl' => $pageurl->out_as_local_url(), 'slotid' => $slot->id));

        $temp = clone($question);
        $temp->questiontext = '';
        $temp->name = qbank_helper::describe_random_question($slot);
        $instancename = bayesian_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'bayesian');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                        get_string('seequestions', 'bayesian'), array('class' => 'mod_bayesian_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'bayesian')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'bayesian'),
                )
            )
        );
    ...
    public function render_question_chooser(renderable $chooser) {
        return $this->render_from_template('mod_bayesian/question_chooser', $chooser->export_for_template($this));
    }
    ...
    public function question_chooser() {
        $chooser = \mod_bayesian\output\question_chooser::get($this->page->course, [], null);
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/bayesian/edit_rest.php';
        $config->sectionurl = '/mod/bayesian/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_bayesian-toolboxes',
                'M.mod_bayesian.init_resource_toolbox',
                array(array(
                        'courseid' => $structure->get_courseid(),
                        'bayesianid' => $structure->get_bayesianid(),
        ...
        $this->page->requires->strings_for_js(array('areyousureremoveselected'), 'bayesian');
        $this->page->requires->yui_module('moodle-mod_bayesian-toolboxes',
                'M.mod_bayesian.init_section_toolbox',
                array(array(
                        'courseid' => $structure,
                        'bayesianid' => $structure->get_bayesianid(),
        ...
        $this->page->requires->yui_module('moodle-mod_bayesian-dragdrop', 'M.mod_bayesian.init_section_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'bayesianid' => $structure->get_bayesianid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_bayesian-dragdrop', 'M.mod_bayesian.init_resource_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'bayesianid' => $structure->get_bayesianid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'bayesian', '');
     ...
     *
     * @param \mod_bayesian\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_bayesian\question\bank\custom_view $questionbank, array $pagevars) {
