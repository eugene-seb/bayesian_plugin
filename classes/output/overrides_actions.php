...
/**
 * Render overrides action in the bayesian secondary navigation
 *
...
class overrides_actions implements renderable, templatable {
     ...
     *
     * @param \renderer_base $output an instance of the bayesian renderer.
     * @return \single_button the button, ready to reander.
     */
    public function create_add_button(\renderer_base $output): \single_button {
        $addoverrideurl = new moodle_url('/mod/bayesian/overrideedit.php',
                ['cmid' => $this->cmid, 'action' => 'add' . $this->mode]);

        if ($this->mode === 'group') {
            $label = get_string('addnewgroupoverride', 'bayesian');
        } else {
            $label = get_string('addnewuseroverride', 'bayesian');
        }
    ...
    public function export_for_template(renderer_base $output): array {
        ...
        $useroverridesurl = new moodle_url('/mod/bayesian/overrides.php', ['cmid' => $this->cmid, 'mode' => 'user']);
        $groupoverridesurl = new moodle_url('/mod/bayesian/overrides.php', ['cmid' => $this->cmid, 'mode' => 'group']);

        $menu = [
            $useroverridesurl->out(false) => get_string('useroverrides', 'bayesian'),
            $groupoverridesurl->out(false) => get_string('groupoverrides', 'bayesian')
        ];

        $overridesnav = new url_select($menu, $PAGE->url->out(false), null, 'bayesianoverrides');
