...
namespace mod_bayesian\completion;
...
use grade_item;
use bayesian;
use bayesian_access_manager;

/**
 * Activity custom completion subclass for the bayesian activity.
 *
 * Class for defining mod_bayesian's custom completion rules and fetching the completion statuses
 * of the custom completion rules for a given bayesian instance and a user.
 *
...
class custom_completion extends activity_custom_completion {
    ...
    protected function check_passing_grade_or_all_attempts(): bool {
        ...
        $attempts = bayesian_get_user_attempts($this->cm->instance, $this->userid, 'finished', true);
        if (!$attempts) {
            return false;
        }
        $lastfinishedattempt = end($attempts);
        $context = context_module::instance($this->cm->id);
        $bayesianobj = bayesian::create($this->cm->instance, $this->userid);
        $accessmanager = new bayesian_access_manager(
            $bayesianobj,
            time(),
            has_capability('mod/bayesian:ignoretimelimits', $context, $this->userid, false)
        );
    ...
    public function get_custom_rule_descriptions(): array {
        ...
        $description['completionminattempts'] = get_string('completiondetail:minattempts', 'mod_bayesian', $minattempts);

        // Completion pass grade is now part of core. Only show the following if it's combined with min attempts.
        $completionpassorattempts = $this->cm->customdata['customcompletionrules']['completionpassorattemptsexhausted'] ?? [];
        if (!empty($completionpassorattempts['completionattemptsexhausted'])) {
            $description['completionpassorattemptsexhausted'] = get_string('completiondetail:passorexhaust', 'mod_bayesian');
        }
