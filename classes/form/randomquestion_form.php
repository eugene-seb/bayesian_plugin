...
class randomquestion_form extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('advcheckbox', 'includesubcategories', get_string('recurse', 'bayesian'), null, null, array(0, 1));
        ...
        if ($defaultvalues->fromtags) {
            ...
            foreach ($defaultvalues->fromtags as $fromtag) {
                if (!$fromtagselement->optionExists($fromtag)) {
                    $optionname = get_string('randomfromunavailabletag', 'mod_bayesian', explode(',', $fromtag)[1]);
