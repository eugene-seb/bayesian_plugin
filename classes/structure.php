...
/**
 * Defines the \mod_bayesian\structure class.
 *
...
/**
 * bayesian structure class.
 *
 * The structure of the bayesian. That is, which questions it is built up
 * from. This is used on the Edit bayesian page (edit.php) and also when
 * starting an attempt at the bayesian (startattempt.php). Once an attempt
 ...
 */
class structure {
    /** @var \bayesian the bayesian this is the structure of. */
    protected $bayesianobj = null;

    /**
     * @var \stdClass[] the questions in this bayesian. Contains the row from the questions
     * table, with the data from the bayesian_slots table added, and also question_categories.contextid.
     */
    protected $questions = array();

    /** @var \stdClass[] bayesian_slots.slot => the bayesian_slots rows for this bayesian, agumented by sectionid. */
    protected $slotsinorder = array();

    /**
     * @var \stdClass[] currently a dummy. Holds data that will match the
     * bayesian_sections, once it exists.
     */
    ...
    /**
     * Create an instance of this class representing an empty bayesian.
     * @return structure
     */
    ...
    /**
     * Create an instance of this class representing the structure of a given bayesian.
     * @param \bayesian $bayesianobj the bayesian.
     * @return structure
     */
    public static function create_for_bayesian($bayesianobj) {
        $structure = self::create();
        $structure->bayesianobj = $bayesianobj;
        $structure->populate_structure();
        return $structure;
    }

    /**
     * Whether there are any questions in the bayesian.
     * @return bool true if there is at least one question in the bayesian.
     */
    ...
    /**
     * Get the number of questions in the bayesian.
     * @return int the number of questions in the bayesian.
     */
     ...
     * question_category.contextid, and the bayesian_slots data for the question in this bayesian.
     */
     ...
     * questions cannot be depended upon because of bayesian options.
     * @param int $slotnumber the index of the slot in question.
    ...
    public function can_finish_during_the_attempt($slotnumber) {
        if ($this->bayesianobj->get_navigation_method() == bayesian_NAVMETHOD_SEQ) {
            return false;
        }
        ...
        if (in_array($this->get_question_type_for_slot($slotnumber), array('random', 'missingtype'))) {
            return \question_engine::can_questions_finish_during_the_attempt(
                    $this->bayesianobj->get_bayesian()->preferredbehaviour);
        }
        ...
        try {
            $quba = \question_engine::make_questions_usage_by_activity('mod_bayesian', $this->bayesianobj->get_context());
            $tempslot = $quba->add_question(\question_bank::load_question(
                    $this->slotsinorder[$slotnumber]->questionid));
            $quba->set_preferred_behaviour($this->bayesianobj->get_bayesian()->preferredbehaviour);
    ...
    /**
     * Get the course id that the bayesian belongs to.
     * @return int the course.id for the bayesian.
     */
    ...
    /**
     * Get the course module id of the bayesian.
     * @return int the course_modules.id for the bayesian.
     */
    ...
    /**
     * Get id of the bayesian.
     * @return int the bayesian.id for the bayesian.
     */
     ...
     * @return \stdClass the bayesian settings row from the database.
     */
    ...
    /**
     * bayesianzes can only be repaginated if they have not been attempted, the
     * questions are not shuffled, and there are two or more questions.
     * @return bool whether this bayesian can be repaginated.
     */
    ...
    /**
     * bayesianzes can only be edited if they have not been attempted.
     * @return bool whether the bayesian can be edited.
     */
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            $this->canbeedited = !bayesian_has_attempts($this->bayesianobj->get_bayesianid());
        }
        return $this->canbeedited;
    }

    /**
     * This bayesian can only be edited if they have not been attempted.
     * Throw an exception if this is not the case.
     */
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            $reportlink = bayesian_attempt_summary_link_to_reports($this->get_bayesian(),
                    $this->bayesianobj->get_cm(), $this->bayesianobj->get_context());
            throw new \moodle_exception('cannoteditafterattempts', 'bayesian',
                    new \moodle_url('/mod/bayesian/edit.php', array('cmid' => $this->get_cmid())), $reportlink);
        }
    }

    /**
     * How many questions are allowed per page in the bayesian.
     * This setting controls how frequently extra page-breaks should be inserted
     * automatically when questions are added to the bayesian.
     * @return int the number of questions that should be on each page of the
     * bayesian by default.
     */
    public function get_questions_per_page() {
        return $this->bayesianobj->get_bayesian()->questionsperpage;
    }

    /**
     * Get bayesian slots.
     * @return \stdClass[] the slots in this bayesian.
     */
    ...
    /**
     * Is this slot the last one in the bayesian?
     * @param int $slotnumber the index of the slot in question.
     * @return bool whether this slot the last one in the bayesian.
     */
    public function is_last_slot_in_bayesian($slotnumber) {
        end($this->slotsinorder);
        return $slotnumber == key($this->slotsinorder);
    }

    /**
     * Is this the first section in the bayesian?
     * @param \stdClass $section the bayesian_sections row.
     * @return bool whether this is first section in the bayesian.
     */
    ...
    /**
     * Is this the last section in the bayesian?
     * @param \stdClass $section the bayesian_sections row.
     * @return bool whether this is first section in the bayesian.
     */
    ...
    /**
     * Get the final slot in the bayesian.
     * @return \stdClass the bayesian_slots for for the final slot in the bayesian.
     */
     ...
     * @return \stdClass the requested bayesian_slots row.
    ...
    /**
     * Get all the slots in a section of the bayesian.
    ...
    /**
     * Get all the sections of the bayesian.
     * @return \stdClass[] the sections in this bayesian.
     */
    ...
    /**
     * Get the overall bayesian grade formatted for display.
     * @return string the maximum grade for this bayesian.
     */
    public function formatted_bayesian_grade() {
        return bayesian_format_grade($this->get_bayesian(), $this->get_bayesian()->grade);
    }
    ...
    public function formatted_question_grade($slotnumber) {
        return bayesian_format_question_grade($this->get_bayesian(), $this->slotsinorder[$slotnumber]->maxmark);
    }

    /**
     * Get the number of decimal places for displyaing overall bayesian grades or marks.
     * @return int the number of decimal places.
     */
    public function get_decimal_places_for_grades() {
        return $this->get_bayesian()->decimalpoints;
    }
    ...
    public function get_decimal_places_for_question_marks() {
        return bayesian_get_grade_format($this->get_bayesian());
    }
    ...
    public function get_edit_page_warnings() {
        ...
        if (bayesian_has_attempts($this->bayesianobj->get_bayesianid())) {
            $reviewlink = bayesian_attempt_summary_link_to_reports($this->bayesianobj->get_bayesian(),
                    $this->bayesianobj->get_cm(), $this->bayesianobj->get_context());
            $warnings[] = get_string('cannoteditafterattempts', 'bayesian', $reviewlink);
        }
    ...
    /**
     * Get the date information about the current state of the bayesian.
    ...
    public function get_dates_summary() {
        $timenow = time();
        $bayesian = $this->bayesianobj->get_bayesian();

        // Exact open and close dates for the tool-tip.
        $dates = array();
        if ($bayesian->timeopen > 0) {
            if ($timenow > $bayesian->timeopen) {
                $dates[] = get_string('bayesianopenedon', 'bayesian', userdate($bayesian->timeopen));
            } else {
                $dates[] = get_string('bayesianwillopen', 'bayesian', userdate($bayesian->timeopen));
            }
        }
        if ($bayesian->timeclose > 0) {
            if ($timenow > $bayesian->timeclose) {
                $dates[] = get_string('bayesianclosed', 'bayesian', userdate($bayesian->timeclose));
            } else {
                $dates[] = get_string('bayesiancloseson', 'bayesian', userdate($bayesian->timeclose));
            }
        }
        if (empty($dates)) {
            $dates[] = get_string('alwaysavailable', 'bayesian');
        }
        $explanation = implode(', ', $dates);

        // Brief summary on the page.
        if ($timenow < $bayesian->timeopen) {
            $currentstatus = get_string('bayesianisclosedwillopen', 'bayesian',
                    userdate($bayesian->timeopen, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($bayesian->timeclose && $timenow <= $bayesian->timeclose) {
            $currentstatus = get_string('bayesianisopenwillclose', 'bayesian',
                    userdate($bayesian->timeclose, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($bayesian->timeclose && $timenow > $bayesian->timeclose) {
            $currentstatus = get_string('bayesianisclosed', 'bayesian');
        } else {
            $currentstatus = get_string('bayesianisopen', 'bayesian');
        }
    ...
    /**
     * Set up this class with the structure for a given bayesian.
     */
    protected function populate_structure() {
        global $DB;

        $slots = qbank_helper::get_question_structure($this->bayesianobj->get_bayesianid(), $this->bayesianobj->get_context());
        ...
        foreach ($slots as $slotdata) {
            ...
            $slot = clone($slotdata);
            $slot->bayesianid = $this->bayesianobj->get_bayesianid();
            $this->slotsinorder[$slot->slot] = $slot;
        }

        // Get bayesian sections in ascending order of the firstslot.
        $this->sections = $DB->get_records('bayesian_sections', ['bayesianid' => $this->bayesianobj->get_bayesianid()], 'firstslot');
    ...
    protected function populate_question_numbers() {
        ...
        foreach ($this->slotsinorder as $slot) {
            if ($this->questions[$slot->questionid]->length == 0) {
                $slot->displayednumber = get_string('infoshort', 'bayesian');
            } else {
    ...
    public function get_version_choices_for_slot(int $slotnumber): array {
        ...
        foreach ($versions as $version) {
            ...
            if ($version->version === $latestversion->version) {
                $version->versionvalue = get_string('questionversionlatest', 'bayesian', $version->version);
            } else {
                $version->versionvalue = get_string('questionversion', 'bayesian', $version->version);
            }
        ...
        $alwaysuselatest->versionvalue = get_string('alwayslatest', 'bayesian');
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if ($followingslotnumber == $movingslotnumber && !$this->is_last_slot_in_bayesian($followingslotnumber)) {
        ...
        } else if (!$this->is_last_slot_in_bayesian($moveafterslotnumber) &&
        ...
        if ($moveafterslotnumber > $movingslotnumber) {
            ...
            if ($this->is_last_slot_in_bayesian($moveafterslotnumber) ||
        ...
        if ($slotreorder) {
            update_field_with_unique_index('bayesian_slots', 'slot', $slotreorder,
                    array('bayesianid' => $this->get_bayesianid()));
        }

        // Page has changed. Record it.
        if ($movingslot->page != $page) {
            $DB->set_field('bayesian_slots', 'page', $page,
                    array('id' => $movingslot->id));
        }

        // Update section fist slots.
        bayesian_update_section_firstslots($this->get_bayesianid(), $headingmovedirection,
        ...
        $emptypages = $DB->get_fieldset_sql("
                SELECT DISTINCT page - 1
                 ...
                 WHERE bayesianid = ?
                   AND page > 1
                   AND NOT EXISTS (SELECT 1 FROM {bayesian_slots} WHERE bayesianid = ? AND page = slot.page - 1)
              ORDER BY page - 1 DESC
                ", array($this->get_bayesianid(), $this->get_bayesianid()));

        foreach ($emptypages as $emptypage) {
            $DB->execute("
                    UPDATE {bayesian_slots}
                       SET page = page - 1
                     WHERE bayesianid = ?
                       AND page > ?
                    ", array($this->get_bayesianid(), $emptypage));
        }

        $trans->allow_commit();

        // Log slot moved event.
        $event = \mod_bayesian\event\slot_moved::create([
            'context' => $this->bayesianobj->get_context(),
            'objectid' => $idmove,
            'other' => [
                'bayesianid' => $this->bayesianobj->get_bayesianid(),
    ...
    /**
     * Refresh page numbering of bayesian slots.
    ...
    public function refresh_page_numbers($slots = array()) {
        ...
        if (!count($slots)) {
            $slots = $DB->get_records('bayesian_slots', array('bayesianid' => $this->get_bayesianid()), 'slot, page');
        }
    ...
    /**
     * Refresh page numbering of bayesian slots and save to the database.
     * @param \stdClass $bayesian the bayesian object.
     * @return \stdClass[] array of slot objects.
     */
    public function refresh_page_numbers_and_update_db() {
        ...
        foreach ($slots as $slot) {
            $DB->set_field('bayesian_slots', 'page', $slot->page,
                    array('id' => $slot->id));
        }
    ...
    public function remove_slot($slotnumber) {
        ...
        $slot = $DB->get_record('bayesian_slots', array('bayesianid' => $this->get_bayesianid(), 'slot' => $slotnumber));
        if (!$slot) {
            return;
        }
        $maxslot = $DB->get_field_sql('SELECT MAX(slot) FROM {bayesian_slots} WHERE bayesianid = ?', array($this->get_bayesianid()));

        $trans = $DB->start_delegated_transaction();
        // Delete the reference if its a question.
        $questionreference = $DB->get_record('question_references',
                ['component' => 'mod_bayesian', 'questionarea' => 'slot', 'itemid' => $slot->id]);
        ...
        if ($questionsetreference) {
            $DB->delete_records('question_set_references',
                ['id' => $questionsetreference->id, 'component' => 'mod_bayesian', 'questionarea' => 'slot']);
        }
        $DB->delete_records('bayesian_slots', array('id' => $slot->id));
        for ($i = $slot->slot + 1; $i <= $maxslot; $i++) {
            $DB->set_field('bayesian_slots', 'slot', $i - 1,
                    array('bayesianid' => $this->get_bayesianid(), 'slot' => $i));
            $this->slotsinorder[$i]->slot = $i - 1;
            $this->slotsinorder[$i - 1] = $this->slotsinorder[$i];
            unset($this->slotsinorder[$i]);
        }

        bayesian_update_section_firstslots($this->get_bayesianid(), -1, $slotnumber);
        ...
        $event = \mod_bayesian\event\slot_deleted::create([
            'context' => $this->bayesianobj->get_context(),
            'objectid' => $slot->id,
            'other' => [
     ...
     *
     * Saves changes to the question grades in the bayesian_slots table and any
     * corresponding question_attempts.
     * It does not update 'sumgrades' in the bayesian table.
     *
     * @param \stdClass $slot row from the bayesian_slots table.
    ...
    public function update_slot_maxmark($slot, $maxmark) {
        ...
        $DB->update_record('bayesian_slots', $slot);
        \question_engine::set_max_mark_in_attempts(new \qubaids_for_bayesian($slot->bayesianid),
                $slot->slot, $maxmark);
        $trans->allow_commit();

        // Log slot mark updated event.
        // We use $num + 0 as a trick to remove the useless 0 digits from decimals.
        $event = \mod_bayesian\event\slot_mark_updated::create([
            'context' => $this->bayesianobj->get_context(),
            'objectid' => $slot->id,
            'other' => [
                'bayesianid' => $this->get_bayesianid(),
    ...
    public function update_question_dependency($slotid, $requireprevious) {
        global $DB;
        $DB->set_field('bayesian_slots', 'requireprevious', $requireprevious, array('id' => $slotid));

        // Log slot require previous event.
        $event = \mod_bayesian\event\slot_requireprevious_updated::create([
            'context' => $this->bayesianobj->get_context(),
            'objectid' => $slotid,
            'other' => [
                'bayesianid' => $this->get_bayesianid(),
     ...
     *
     * Saves changes to the slot page relationship in the bayesian_slots table and reorders the paging
    ...
    public function update_page_break($slotid, $type) {
        ...
        $bayesianslots = $DB->get_records('bayesian_slots', array('bayesianid' => $this->get_bayesianid()), 'slot');
        $repaginate = new \mod_bayesian\repaginate($this->get_bayesianid(), $bayesianslots);
        $repaginate->repaginate_slots($bayesianslots[$slotid]->slot, $type);
        $slots = $this->refresh_page_numbers_and_update_db();

        if ($type == repaginate::LINK) {
            // Log page break created event.
            $event = \mod_bayesian\event\page_break_deleted::create([
                'context' => $this->bayesianobj->get_context(),
                'objectid' => $slotid,
                'other' => [
                    'bayesianid' => $this->get_bayesianid(),
                    'slotnumber' => $bayesianslots[$slotid]->slot
                ]
            ]);
            $event->trigger();
        } else {
            // Log page deleted created event.
            $event = \mod_bayesian\event\page_break_created::create([
                'context' => $this->bayesianobj->get_context(),
                'objectid' => $slotid,
                'other' => [
                    'bayesianid' => $this->get_bayesianid(),
    ...
    public function add_section_heading($pagenumber, $heading = null) {
        ...
        if ($heading !== null) {
            ...
            $section->heading = get_string('newsectionheading', 'bayesian');
        }
        $section->bayesianid = $this->get_bayesianid();
        $slotsonpage = $DB->get_records('bayesian_slots', array('bayesianid' => $this->get_bayesianid(), 'page' => $pagenumber), 'slot DESC');
        $firstslot = end($slotsonpage);
        $section->firstslot = $firstslot->slot;
        $section->shufflequestions = 0;
        $sectionid = $DB->insert_record('bayesian_sections', $section);

        // Log section break created event.
        $event = \mod_bayesian\event\section_break_created::create([
            'context' => $this->bayesianobj->get_context(),
            'objectid' => $sectionid,
            'other' => [
                'bayesianid' => $this->get_bayesianid(),
    ...
    public function set_section_heading($id, $newheading) {
        global $DB;
        $section = $DB->get_record('bayesian_sections', array('id' => $id), '*', MUST_EXIST);
        $section->heading = $newheading;
        $DB->update_record('bayesian_sections', $section);

        // Log section title updated event.
        $firstslot = $DB->get_record('bayesian_slots', array('bayesianid' => $this->get_bayesianid(), 'slot' => $section->firstslot));
        $event = \mod_bayesian\event\section_title_updated::create([
            'context' => $this->bayesianobj->get_context(),
            'objectid' => $id,
            'other' => [
                'bayesianid' => $this->get_bayesianid(),
    ...
    public function set_section_shuffle($id, $shuffle) {
        ...
        $firstslot = $DB->get_record('bayesian_slots', array('bayesianid' => $this->get_bayesianid(), 'slot' => $section->firstslot));
        $event = \mod_bayesian\event\section_shuffle_updated::create([
            'context' => $this->bayesianobj->get_context(),
            'objectid' => $id,
            'other' => [
                'bayesianid' => $this->get_bayesianid(),
    ...
    public function remove_section_heading($sectionid) {
        global $DB;
        $section = $DB->get_record('bayesian_sections', array('id' => $sectionid), '*', MUST_EXIST);
        if ($section->firstslot == 1) {
            throw new \coding_exception('Cannot remove the first section in a bayesian.');
        }
        $DB->delete_records('bayesian_sections', array('id' => $sectionid));

        // Log page deleted created event.
        $firstslot = $DB->get_record('bayesian_slots', array('bayesianid' => $this->get_bayesianid(), 'slot' => $section->firstslot));
        $event = \mod_bayesian\event\section_break_deleted::create([
            'context' => $this->bayesianobj->get_context(),
            'objectid' => $sectionid,
            'other' => [
                'bayesianid' => $this->get_bayesianid(),
    ...
    /**
     * Whether the current user can add random questions to the bayesian or not.
    ...
    public function can_add_random_questions() {
        if ($this->canaddrandom === null) {
            $bayesiancontext = $this->bayesianobj->get_context();
            $relatedcontexts = new \core_question\local\bank\question_edit_contexts($bayesiancontext);
