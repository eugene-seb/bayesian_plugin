 ...
 * @since      Moodle 3.1
 */
class mod_bayesian_external extends external_api {

    /**
     * Describes the parameters for get_bayesianzes_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_bayesianzes_by_courses_parameters() {
    ...
    /**
     * Returns a list of bayesianzes in a provided list of courses,
     * if no list is provided all bayesianzes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of bayesianzes details
     * @since Moodle 3.1
     */
    public static function get_bayesianzes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedbayesianzes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_bayesianzes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the bayesianzes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $bayesianzes = get_all_instances_in_courses("bayesian", $courses);
            foreach ($bayesianzes as $bayesian) {
                ...
                $bayesian = bayesian_update_effective_access($bayesian, $USER->id);

                // Entry to return.
                $bayesiandetails = helper_for_get_mods_by_courses::standard_coursemodule_element_values(
                        $bayesian, 'mod_bayesian', 'mod/bayesian:view', 'mod/bayesian:view');

                if (has_capability('mod/bayesian:view', $context)) {
                    $bayesiandetails['introfiles'] = external_util::get_area_files($context->id, 'mod_bayesian', 'intro', false, false);
                    ...
                    $hasfeedback = bayesian_has_feedback($bayesian);
                    $bayesiandetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;

                    $timenow = time();
                    $bayesianobj = bayesian::create($bayesian->id, $USER->id);
                    $accessmanager = new bayesian_access_manager($bayesianobj, $timenow, has_capability('mod/bayesian:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the bayesian.
                    if (!$accessmanager->prevent_access()) {
                        $bayesiandetails['hasquestions'] = (int) $bayesianobj->has_questions();
                        $bayesiandetails['autosaveperiod'] = get_config('bayesian', 'autosaveperiod');
                        ...
                        // Any course module fields that previously existed in bayesian.
                        $bayesiandetails['completionpass'] = $bayesianobj->get_cm()->completionpassgrade;
                    }
                    ...
                    foreach ($viewablefields as $field) {
                        $bayesiandetails[$field] = $bayesian->{$field};
                    }
                }
                $returnedbayesianzes[] = $bayesiandetails;
            }
        }
        $result = array();
        $result['bayesianzes'] = $returnedbayesianzes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_bayesianzes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_bayesianzes_by_courses_returns() {
        return new external_single_structure(
            array(
                'bayesianzes' => new external_multiple_structure(
                    new external_single_structure(array_merge(
                        helper_for_get_mods_by_courses::standard_coursemodule_elements_returns(true),
                        [
                            'timeopen' => new external_value(PARAM_INT, 'The time when this bayesian opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this bayesian closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for bayesian attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a bayesian attempt.', VALUE_OPTIONAL),
                            ...
                            'attemptonlast' => new external_value(PARAM_INT, 'Whether subsequent attempts start from the answer
                                                                    ...
                                                                    VALUE_OPTIONAL),
                            'grademethod' => new external_value(PARAM_INT, 'One of the values bayesian_GRADEHIGHEST, bayesian_GRADEAVERAGE,
                                                                    bayesian_ATTEMPTFIRST or bayesian_ATTEMPTLAST.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their bayesian
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_bayesian_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their bayesian
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their bayesian attempts
                            ...
                            'reviewspecificfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            bayesian attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewgeneralfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            bayesian attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewrightanswer' => new external_value(PARAM_INT, 'Whether users are allowed to review their bayesian
                                                                        attempts at various times. A bit field, like
                                                                        reviewattempt.', VALUE_OPTIONAL),
                            'reviewoverallfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their bayesian
                                                                            attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'questionsperpage' => new external_value(PARAM_INT, 'How often to insert a page break when editing
                                                                        the bayesian, or when shuffling the question order.',
                                                                        VALUE_OPTIONAL),
                            'navmethod' => new external_value(PARAM_ALPHA, 'Any constraints on how the user is allowed to navigate
                                                                around the bayesian. Currently recognised values are
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the bayesian overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the bayesian was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a bayesian attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this bayesian can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark bayesian complete when the student has
                            ...
                            'allowofflineattempts' => new external_value(PARAM_INT, 'Whether to allow the bayesian to be attempted
                                                                            offline in the mobile app', VALUE_OPTIONAL),
                            'autosaveperiod' => new external_value(PARAM_INT, 'Auto-save delay', VALUE_OPTIONAL),
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the bayesian has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the bayesian has questions', VALUE_OPTIONAL),
                        ]
                    ))
                ),
    ...
    /**
     * Utility function for validating a bayesian.
     *
     * @param int $bayesianid bayesian instance id
     * @return array array containing the bayesian, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_bayesian($bayesianid) {
        global $DB;

        // Request and permission validation.
        $bayesian = $DB->get_record('bayesian', array('id' => $bayesianid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($bayesian, 'bayesian');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return array($bayesian, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_bayesian.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_bayesian_parameters() {
        return new external_function_parameters (
            array(
                'bayesianid' => new external_value(PARAM_INT, 'bayesian instance id'),
            )
        );
    }
     ...
     */
    public static function view_bayesian($bayesianid) {
        global $DB;

        $params = self::validate_parameters(self::view_bayesian_parameters(), array('bayesianid' => $bayesianid));
        $warnings = array();

        list($bayesian, $course, $cm, $context) = self::validate_bayesian($params['bayesianid']);
    ...
    /**
     * Describes the view_bayesian return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_bayesian_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                ...
                'status' => new external_value(PARAM_ALPHA, 'bayesian status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given bayesian and user.
     *
     * @param int $bayesianid bayesian instance id
     * @param int $userid user id
     * @param string $status bayesian status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($bayesianid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;

        $warnings = array();

        $params = array(
            'bayesianid' => $bayesianid,
            'userid' => $userid,
            'status' => $status,
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/bayesian:viewreports', $context);
        }

        // Update bayesian with override information.
        $bayesian = bayesian_update_effective_access($bayesian, $params['userid']);
        $attempts = bayesian_get_user_attempts($bayesian->id, $user->id, $params['status'], $params['includepreviews']);
        $attemptresponse = [];
        foreach ($attempts as $attempt) {
            $reviewoptions = bayesian_get_review_options($bayesian, $attempt, $context);
            if (!has_capability('mod/bayesian:viewreports', $context) &&
                    ($reviewoptions->marks < question_display_options::MARK_AND_MAX || $attempt->state != bayesian_attempt::FINISHED)) {
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'bayesian' => new external_value(PARAM_INT, 'Foreign key reference to the bayesian that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this bayesian.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this bayesian
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time bayesian cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a bayesian.
     *
     * @param int $bayesianid bayesian instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($bayesianid, $userid = 0) {
        global $DB, $USER, $CFG;
        ...
        // This code was mostly copied from mod/bayesian/view.php. We need to make the web service logic consistent.
        // Get this user's attempts.
        $attempts = bayesian_get_user_attempts($bayesian->id, $user->id, 'all');
        $canviewgrade = false;
        if ($attempts) {
            if ($USER->id != $user->id) {
                // No need to check the permission here. We did it at by require_capability('mod/bayesian:viewreports', $context).
                $canviewgrade = true;
            } else {
                // Work out which columns we need, taking account what data is available in each attempt.
                [$notused, $alloptions] = bayesian_get_combined_reviewoptions($bayesian, $attempts);
                $canviewgrade = $alloptions->marks >= question_display_options::MARK_AND_MAX;
            }
        }

        $grade = $canviewgrade ? bayesian_get_best_grade($bayesian, $user->id) : null;
        ...
        $gradinginfo = grade_get_grades($course->id, 'mod', 'bayesian', $bayesian->id, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given bayesian.'),
                'grade' => new external_value(PARAM_FLOAT, 'The grade (only if the user has a grade).', VALUE_OPTIONAL),
                'gradetopass' => new external_value(PARAM_FLOAT, 'The grade to pass the bayesian (only if set).', VALUE_OPTIONAL),
                'warnings' => new external_warnings(),
            )
        );
    }
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($bayesianid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = bayesian_get_user_attempts($bayesian->id, $user->id, 'all', true);
    ...
    /**
     * Starts a new attempt at a bayesian.
     *
     * @param int $bayesianid bayesian instance id
     ...
     */
    public static function start_attempt($bayesianid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        // Check questions.
        if (!$bayesianobj->has_questions()) {
            throw new moodle_bayesian_exception($bayesianobj, 'noquestionsfound');
        }

        // Create an object to manage all the other (non-roles) access rules.
        $timenow = time();
        $accessmanager = $bayesianobj->get_access_manager($timenow);

        // Validate permissions for creating a new attempt and start a new preview attempt if required.
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            bayesian_validate_new_attempt($bayesianobj, $accessmanager, $forcenew, -1, false);

        // Check access.
        if (!$bayesianobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_bayesian_exception($bayesianobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == bayesian_attempt::OVERDUE) {
                    throw new moodle_bayesian_exception($bayesianobj, 'stateoverdue');
                } else {
                    throw new moodle_bayesian_exception($bayesianobj, 'attemptstillinprogress');
                }
            }
            $offlineattempt = WS_SERVER ? true : false;
            $attempt = bayesian_prepare_and_start_new_attempt($bayesianobj, $attemptnumber, $lastattempt, $offlineattempt);
        }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = bayesian_attempt::create($params['attemptid']);
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_bayesian_exception($attemptobj->get_bayesianobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_bayesian_exception($attemptobj->get_bayesianobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == bayesian_attempt::OVERDUE) {
            throw new moodle_bayesian_exception($attemptobj->get_bayesianobj(), 'stateoverdue');
        }

        // User submitted data (like the bayesian password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_bayesian_exception($attemptobj->get_bayesianobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_bayesian_exception($attemptobj->get_bayesianobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_bayesian_exception($attemptobj->get_bayesianobj(), 'Out of sequence access');
            }
     ...
     *
     * @return external_single_structure the question data. Some fields may not be returned depending on the bayesian display settings.
     * @since  Moodle 3.1
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the bayesian this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the bayesian', VALUE_OPTIONAL),
                'state' => new external_value(PARAM_ALPHA, 'the state where the question is in.
                    It will not be returned if the user cannot see it due to the bayesian display correctness settings.',
                    VALUE_OPTIONAL),
            ...
            'The question data. Some fields may not be returned depending on the bayesian display settings.'
        );
    }
     ...
     *
     * @param  bayesian_attempt  $attemptobj  the bayesian attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(bayesian_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_bayesianobj()->get_context()->id;
    ...
    /**
     * Returns information for the given attempt page for a bayesian attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_bayesian_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/bayesian:preview capability,
    ...
    /**
     * Returns a summary of a bayesian attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the bayesian. This function is intended for the bayesian auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a bayesian and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    public static function process_attempt($attemptid, $data, $finishattempt = false, $timeup = false, $preflightdata = array()) {
        ...
        $attemptobj = bayesian_attempt::create($params['attemptid']);
        $failifoverdue = !($attemptobj->get_bayesianobj()->get_bayesian()->overduehandling == 'graceperiod');
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_bayesian_exception($attemptobj->get_bayesianobj(), 'attemptclosed');
            } else if (!$displayoptions->attempt) {
                throw new moodle_bayesian_exception($attemptobj->get_bayesianobj(), 'noreview', null, '',
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_bayesianobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = bayesian_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_bayesian(), false);
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the bayesian (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_bayesian_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'bayesianid' => new external_value(PARAM_INT, 'bayesian instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given bayesian.
     *
     * @param int $bayesianid bayesian instance id
     ...
     * @throws moodle_exception
     */
    public static function get_bayesian_feedback_for_grade($bayesianid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_bayesian_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = bayesian_feedback_record_for_grade($params['grade'], $bayesian);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id,
                                                        'mod_bayesian', 'feedback', $feedback->id);
            $result['feedbacktext'] = $text;
            $result['feedbacktextformat'] = $format;
            $feedbackinlinefiles = external_util::get_area_files($context->id, 'mod_bayesian', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_bayesian_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_bayesian_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_bayesian_access_information.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_bayesian_access_information_parameters() {
        return new external_function_parameters (
            array(
                'bayesianid' => new external_value(PARAM_INT, 'bayesian instance id')
            )
    ...
    /**
     * Return access information for a given bayesian.
     *
     * @param int $bayesianid bayesian instance id
     ...
     * @throws  moodle_bayesian_exception
     */
    public static function get_bayesian_access_information($bayesianid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_bayesian_access_information_parameters(), $params);

        list($bayesian, $course, $cm, $context) = self::validate_bayesian($params['bayesianid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/bayesian:attempt', $context);;
        $result['canmanage'] = has_capability('mod/bayesian:manage', $context);;
        $result['canpreview'] = has_capability('mod/bayesian:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/bayesian:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/bayesian:viewreports', $context);;

        // Access manager now.
        $bayesianobj = bayesian::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/bayesian:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new bayesian_access_manager($bayesianobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_bayesian_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_bayesian_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the bayesian or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the bayesian settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the bayesian or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the bayesian reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a bayesian.
     *
     * @param int $bayesianid bayesian instance id
     ...
     * @throws  moodle_bayesian_exception
     */
    public static function get_attempt_access_information($bayesianid, $attemptid = 0) {
        global $DB, $USER;
        ...
        if ($unfinishedattempt = bayesian_get_user_attempt_unfinished($bayesian->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != bayesian_attempt::IN_PROGRESS and $unfinishedattempt->state != bayesian_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_bayesian_required_qtypes.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_bayesian_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'bayesianid' => new external_value(PARAM_INT, 'bayesian instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given bayesian.
     ...
     * @throws  moodle_bayesian_exception
     */
    public static function get_bayesian_required_qtypes($bayesianid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_bayesian_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $bayesianobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_bayesian_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_bayesian_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the bayesian'),
                'warnings' => new external_warnings(),
            )
        );
    }
