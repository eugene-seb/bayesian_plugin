...
$PAGE->set_url('/mod/bayesian/comment.php', array('attempt' => $attemptid, 'slot' => $slot));
...
if (!$attemptobj->is_finished()) {
    throw new \moodle_exception('attemptclosed', 'bayesian');
}

// Check login and permissions.
require_login($attemptobj->get_course(), false, $attemptobj->get_cm());
$attemptobj->require_capability('mod/bayesian:grade');

// Print the page header.
$PAGE->set_pagelayout('popup');
$PAGE->set_title(get_string('manualgradequestion', 'bayesian', array(
        'question' => format_string($attemptobj->get_question_name($slot)),
        'bayesian' => format_string($attemptobj->get_bayesian_name()), 'user' => fullname($student))));
...
// bayesian name.
$summarydata['bayesianname'] = array(
    'title'   => get_string('modulename', 'bayesian'),
    'content' => format_string($attemptobj->get_bayesian_name()),
);

// Question name.
$summarydata['questionname'] = array(
    'title'   => get_string('question', 'bayesian'),
...
<?php
echo '</form>';
$PAGE->requires->js_init_call('M.mod_bayesian.init_comment_popup', null, false, bayesian_get_js_module());
