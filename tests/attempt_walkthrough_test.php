 ...
 * @covers \bayesian_attempt
 */
class attempt_walkthrough_test extends \advanced_testcase {

    /**
     * Create a bayesian with questions and walk through a bayesian attempt.
     */
    public function test_bayesian_attempt_walkthrough() {
        global $SITE;
        ...
        $bayesian = $bayesiangenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 3));
        ...
        bayesian_add_bayesian_question($matchq->id, $bayesian);
        ...
        $timenow = time();
        $attempt = bayesian_create_attempt($bayesianobj, 1, false, $timenow, false, $user1->id);
        ...
        // Check bayesian grades.
        $grades = bayesian_get_user_grades($bayesian, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'bayesian', $bayesian->id, $user1->id);
    ...
    /**
     * Create a bayesian containing one question and a close time.
     *
     * The question is the standard shortanswer test question.
     * The bayesian is set to close 1 hour from now.
     * The bayesian is set to use a grade period of 1 hour once time expires.
     *
     * @return \stdClass the bayesian that was created.
     */
    protected function create_bayesian_with_one_question(): \stdClass {
        global $SITE;
        $this->resetAfterTest();
        ...
        // Add them to the bayesian.
        bayesian_add_bayesian_question($saq->id, $bayesian, 0, 1);
        bayesian_update_sumgrades($bayesian);

        return $bayesian;
    }

    public function test_bayesian_attempt_walkthrough_submit_time_recorded_correctly_when_overdue() {

        $bayesian = $this->create_bayesian_with_one_question();
        ...
        // Start the attempt.
        $attempt = bayesian_prepare_and_start_new_attempt($bayesianobj, 1, null);

        // Process some responses from the student.
        $attemptobj = bayesian_attempt::create($attempt->id);
        $this->assertEquals(1, $attemptobj->get_number_of_unanswered_questions());
        $attemptobj->process_submitted_actions($bayesian->timeclose - 30 * MINSECS, false, [1 => ['answer' => 'frog']]);

        // Attempt goes overdue (e.g. if cron ran).
        $attemptobj = bayesian_attempt::create($attempt->id);
        $attemptobj->process_going_overdue($bayesian->timeclose + 2 * get_config('bayesian', 'graceperiodmin'), false);
        ...
        $attemptobj = bayesian_attempt::create($attempt->id);
        $attemptobj->process_attempt($bayesian->timeclose + 30 * MINSECS, true, false, 1);

        // Verify the attempt state.
        $attemptobj = bayesian_attempt::create($attempt->id);
        $this->assertEquals(1, $attemptobj->get_attempt_number());
        $this->assertEquals(true, $attemptobj->is_finished());
        $this->assertEquals($bayesian->timeclose + 30 * MINSECS, $attemptobj->get_submitted_date());
        $this->assertEquals($user->id, $attemptobj->get_userid());
        $this->assertTrue($attemptobj->has_response_to_at_least_one_graded_question());
        $this->assertEquals(0, $attemptobj->get_number_of_unanswered_questions());
    }

    public function test_bayesian_attempt_walkthrough_close_time_extended_at_last_minute() {
        global $DB;

        $bayesian = $this->create_bayesian_with_one_question();
        $originaltimeclose = $bayesian->timeclose;
        ...
        // Teacher edits the bayesian to extend the time-limit by one minute.
        $DB->set_field('bayesian', 'timeclose', $originaltimeclose + MINSECS, ['id' => $bayesian->id]);
        \course_modinfo::clear_instance_cache($bayesian->course);

        // Timer expires in the student browser and thinks it is time to submit the bayesian.
        ...
        $this->assertEquals(bayesian_attempt::IN_PROGRESS, $attemptobj->get_state());
        $this->assertEquals(0, $attemptobj->get_submitted_date());
        $this->assertEquals($user->id, $attemptobj->get_userid());
    }

    /**
     * Create a bayesian with a random as well as other questions and walk through bayesian attempts.
     */
    public function test_bayesian_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $bayesian = $bayesiangenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the bayesian.
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            bayesian_start_new_attempt($bayesianobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $bayesianwithvariants = null;

    /**
     * Create a bayesian with a single question with variants and walk through bayesian attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_bayesian_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->bayesianwithvariants === null) {
            // Make a bayesian.
            $bayesiangenerator = $this->getDataGenerator()->get_plugin_generator('mod_bayesian');

            $this->bayesianwithvariants = $bayesiangenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            bayesian_add_bayesian_question($calc->id, $this->bayesianwithvariants, 0);
        }


        // Make a new user to do the bayesian.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $bayesianobj = bayesian::create($this->bayesianwithvariants->id, $user1->id);
        ...
        // Select variant.
        bayesian_start_new_attempt($bayesianobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check bayesian grades.
        $grades = bayesian_get_user_grades($this->bayesianwithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'bayesian', $this->bayesianwithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
