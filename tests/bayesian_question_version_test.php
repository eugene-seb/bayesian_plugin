...
/**
 * Question versions test for bayesian.
 *
 ...
 */
class bayesian_question_version_test extends \advanced_testcase {
    ...
    /**
     * Test the bayesian question data for changed version in the slots.
     *
     * @covers ::get_version_options
     */
    public function test_bayesian_questions_for_changed_versions() {
        $this->resetAfterTest();
        $bayesian = $this->create_test_bayesian($this->course);
        // Test for questions from a different context.
        $context = \context_module::instance(get_coursemodule_from_instance("bayesian", $bayesian->id, $this->course->id)->id);
     ...
     * @covers ::get_version_options
     */
    public function test_bayesian_question_attempts_with_changed_version() {
        $this->resetAfterTest();
        $bayesian = $this->create_test_bayesian($this->course);
        // Test for questions from a different context.
        $context = \context_module::instance(get_coursemodule_from_instance("bayesian", $bayesian->id, $this->course->id)->id);
        ...
        $questiongenerator->update_question($numq, null, ['name' => 'This is the third version']);
        bayesian_add_bayesian_question($numq->id, $bayesian);
        list($bayesianobj, $quba, $attemptobj) = $this->attempt_bayesian($bayesian, $this->student);
        ...
        submit_question_version::execute($slot->id, (int)$selectversions[1]->version);
        list($bayesianobj, $quba, $attemptobj) = $this->attempt_bayesian($bayesian, $this->student, 2);
        ...
        list($bayesianobj, $quba, $attemptobj) = $this->attempt_bayesian($bayesian, $this->student, 3);
        ...
        list($bayesianobj, $quba, $attemptobj) = $this->attempt_bayesian($bayesian, $this->student, 4);
