...
/**
 * Contains the class containing unit tests for the bayesian notify attempt manual grading completed cron task.
 *
...
use context_course;
use context_module;
use mod_bayesian\task\bayesian_notify_attempt_manual_grading_completed;
use question_engine;
use bayesian;
...
/**
 * Class containing unit tests for the bayesian notify attempt manual grading completed cron task.
 *
 * @package mod_bayesian
 * @copyright 2021 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_notify_attempt_manual_grading_completed_test extends advanced_testcase {
     ...
     * Standard test setup.
     *
     * Create a course with a bayesian and a student and a(n editing) teacher.
     * the bayesian has a truefalse question and an essay question.
     *
     * Also create some bits of a bayesian attempt to be used later.
     */
    public function setUp(): void {
        ...
        $this->bayesian = $this->getDataGenerator()->create_module('bayesian', ['course' => $this->course->id]);
        ...
        assign_capability('mod/bayesian:emailnotifyattemptgraded', CAP_ALLOW, $studentrole->id, $coursecontext, true);
        ...
        $this->bayesian = $bayesiangenerator->create_instance(['course' => $this->course->id, 'questionsperpage' => 0,
        ...
        bayesian_add_bayesian_question($truefalse->id, $this->bayesian);
        bayesian_add_bayesian_question($essay->id, $this->bayesian);

        $this->bayesianobj = bayesian::create($this->bayesian->id);
        $this->quba = question_engine::make_questions_usage_by_activity('mod_bayesian', $this->bayesianobj->get_context());
        $this->quba->set_preferred_behaviour($this->bayesianobj->get_bayesian()->preferredbehaviour);
    }
    ...
    public function test_get_list_of_attempts_within_conditions() {
        ...
        $attempt1 = bayesian_create_attempt($this->bayesianobj, 1, null, $timenow - HOURSECS, false, $this->student->id);
        bayesian_start_new_attempt($this->bayesianobj, $this->quba, $attempt1, 1, $timenow - HOURSECS);
        bayesian_attempt_save_started($this->bayesianobj, $this->quba, $attempt1);

        // Process some responses from the student (30 mins ago) and submit (20 mins ago).
        $attemptobj1 = bayesian_attempt::create($attempt1->id);
        ...
        bayesian_save_best_grade($attemptobj1->get_bayesian(), $this->student->id);

        // Not quite time to send yet.
        $task = new bayesian_notify_attempt_manual_grading_completed();
    ...
    public function test_get_list_of_attempts_without_manual_graded() {
        ...
        $attempt2 = bayesian_create_attempt($this->bayesianobj, 2, null, $timenow - HOURSECS, false, $this->student->id);
        bayesian_start_new_attempt($this->bayesianobj, $this->quba, $attempt2, 2, $timenow - HOURSECS);
        bayesian_attempt_save_started($this->bayesianobj, $this->quba, $attempt2);

        // Process some responses from the student (30 mins ago) and submit (now).
        $attemptobj2 = bayesian_attempt::create($attempt2->id);
    ...
    public function test_notify_manual_grading_completed_task_without_capability() {
        ...
        $timenow = time();
        $attempt = bayesian_create_attempt($this->bayesianobj, 3, null, $timenow, false, $this->teacher->id);
        bayesian_start_new_attempt($this->bayesianobj, $this->quba, $attempt, 3, $timenow - HOURSECS);
        bayesian_attempt_save_started($this->bayesianobj, $this->quba, $attempt);
        ...
        $DB->update_record('bayesian_attempts', $update);
        bayesian_save_best_grade($attemptobj->get_bayesian(), $this->student->id);

        // Run the bayesian notify attempt manual graded task.
        ob_start();
        $task = new bayesian_notify_attempt_manual_grading_completed();
        $task->set_time_for_testing($timenow + 5 * HOURSECS + 1);
        $task->execute();
    ...
    public function test_notify_manual_grading_completed_task_with_capability() {
        ...
        $timenow = time();
        $attempt = bayesian_create_attempt($this->bayesianobj, 4, null, $timenow, false, $this->student->id);
        bayesian_start_new_attempt($this->bayesianobj, $this->quba, $attempt, 4, $timenow - HOURSECS);
        bayesian_attempt_save_started($this->bayesianobj, $this->quba, $attempt);
