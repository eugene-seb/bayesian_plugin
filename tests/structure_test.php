...
class structure_test extends \advanced_testcase {
    ...
    /**
     * Create a course with an empty bayesian.
     * @return array with three elements bayesian, cm and course.
     */
    protected function prepare_bayesian_data() {
        ...
        return array($bayesian, $cm, $course);
    }

    /**
     * Creat a test bayesian.
     *
     ...
     * @return bayesian the created bayesian.
     */
    protected function create_test_bayesian($layout) {
        list($bayesian, $cm, $course) = $this->prepare_bayesian_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                bayesian_add_bayesian_question($q->id, $bayesian, $page);
                $lastpage = $page;
            }
        }
     ...
     * @param array $expectedlayout as for $layout in {@link create_test_bayesian()}.
     * @param structure $structure the structure to test.
     */
    protected function assert_bayesian_layout($expectedlayout, structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first bayesian section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The bayesian contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the bayesian.');
            }
        }
    }
     ...
     * used by create_test_bayesian as assert_bayesian_layout.
    ...
    public function test_get_bayesian_slots() {
        $bayesianobj = $this->create_test_bayesian(array(
                array('TF1', 1, 'truefalse'),
    ...
    public function test_bayesian_has_one_section_by_default() {
        $bayesianobj = $this->create_test_bayesian(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = structure::create_for_bayesian($bayesianobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = structure::create_for_bayesian($bayesianobj);
        $this->assert_bayesian_layout(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
                array('TF2', 2, 'truefalse'),
            ), $structure);
    ...
    public function test_bayesian_remove_slot() {
        $bayesianobj = $this->create_test_bayesian(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_bayesian_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        bayesian_add_random_questions($bayesianobj->get_bayesian(), 1, $cat->id, 1, false);
        $structure = structure::create_for_bayesian($bayesianobj);
        $sql = 'SELECT qsr.*
                 FROM {question_set_references} qsr
                 JOIN {bayesian_slots} qs ON qs.id = qsr.itemid
                 WHERE qs.bayesianid = ?
                   AND qsr.component = ?
                   AND qsr.questionarea = ?';
        $randomq = $DB->get_record_sql($sql, [$bayesianobj->get_bayesianid(), 'mod_bayesian', 'slot']);
        ...
        $this->assertFalse($DB->record_exists('question_set_references',
            array('id' => $randomq->id, 'component' => 'mod_bayesian', 'questionarea' => 'slot')));
    }
    ...
    public function test_can_remove_last_question_in_a_bayesian() {
        $bayesianobj = $this->create_test_bayesian(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
            ));
        ...
        bayesian_add_bayesian_question($q->id, $bayesianobj->get_bayesian(), 0);
        $structure = structure::create_for_bayesian($bayesianobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        bayesian_add_bayesian_question($q->id, $bayesianobj->get_bayesian(), 1);
    ...
    public function test_can_add_random_questions() {
        ...
        $bayesian = $this->create_test_bayesian([]);
        $course = $bayesian->get_course();
        ...
        $structure = structure::create_for_bayesian($bayesian);
    ...
    public function test_get_version_choices_for_slot() {
        ...
        $bayesianobj = $this->create_test_bayesian([]);

        // Create a question with two versions.
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category(['contextid' => $bayesianobj->get_context()->id]);
        ...
        bayesian_add_bayesian_question($q->id, $bayesianobj->get_bayesian());

        // Create the bayesian object.
        $structure = structure::create_for_bayesian($bayesianobj);
    ...
    public function test_has_use_capability() {
        ...
        // Create a bayesian with question.
        $bayesianobj = $this->create_test_bayesian([]);
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category(['contextid' => $bayesianobj->get_context()->id]);
        $q = $questiongenerator->create_question('essay', null,
