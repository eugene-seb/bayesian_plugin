...
/**
 * Unit tests restoring bayesian attempts
 *
...
class restore_attempt_test extends \advanced_testcase {
     ...
     * Test restore dates.
     *
     * @covers \restore_bayesian_activity_structure_step
     */
    public function test_restore_question_attempts_missing_users(): void {
        ...
        // Contains a bayesian with four attempts from different users. The users are as follows (user ID -> user):
        ...
        $restoredbayesian = $DB->get_record('bayesian', []);
        ...
        $this->assertContains("Mapped user ID not found for user 4, bayesian {$restoredbayesian->id}, attempt 1. Skipping bayesian attempt",
            $loginfomessages);
        $this->assertContains("Mapped user ID not found for user 5, bayesian {$restoredbayesian->id}, attempt 1. Skipping bayesian attempt",
            $loginfomessages);

        // User 01 has supplied the wrong answer, assert dates match the backup file too.
        $user01attempt = $DB->get_record('bayesian_attempts', [
            'bayesian' => $restoredbayesian->id,
        ...
        $user04attempt = $DB->get_record('bayesian_attempts', [
            'bayesian' => $restoredbayesian->id,
