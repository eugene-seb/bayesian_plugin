...
/**
 * bayesian module test data generator class
 *
 * @package mod_bayesian
 * @copyright 2012 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_bayesian_generator extends testing_module_generator {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        $defaultbayesiansettings = array(
            ...
            'showuserpicture'        => 0,
            'showblocks'             => 0,
            'navmethod'              => bayesian_NAVMETHOD_FREE,
        );

        foreach ($defaultbayesiansettings as $name => $value) {
    ...
    /**
     * Create a bayesian attempt for a particular user at a particular course.
     *
     * @param int $bayesianid the bayesian id (from the mdl_quit table, not cmid).
     ...
     */
    public function create_attempt($bayesianid, $userid, array $forcedrandomquestions = [],
            array $forcedvariants = []) {
        // Build bayesian object and load questions.
        $bayesianobj = bayesian::create($bayesianid, $userid);

        $attemptnumber = 1;
        $attempt = null;

        if ($attempts = bayesian_get_user_attempts($bayesianid, $userid, 'all', true)) {
        ...
        return bayesian_prepare_and_start_new_attempt($bayesianobj, $attemptnumber, $attempt, false,
                $forcedrandomquestions, $forcedvariants);
    }

    /**
     * Submit responses to a bayesian attempt.
     *
    ...
    /**
     * Create a bayesian override (either user or group).
     *
     * @param array $data must specify bayesianid, and one of userid or groupid.
     */
    public function create_override(array $data): void {
        ...
        if (!isset($data['bayesian'])) {
            throw new coding_exception('Must specify bayesian (id) when creating a bayesian override.');
        }

        if (!isset($data['userid']) && !isset($data['groupid'])) {
            throw new coding_exception('Must specify one of userid or groupid when creating a bayesian override.');
        }

        if (isset($data['userid']) && isset($data['groupid'])) {
            throw new coding_exception('Cannot specify both userid and groupid when creating a bayesian override.');
        }

        // Create the override.
        $DB->insert_record('bayesian_overrides', (object) $data);

        // Update any associated calendar events, if necessary.
        bayesian_update_events($DB->get_record('bayesian', ['id' => $data['bayesian']], '*', MUST_EXIST));
    }
}
