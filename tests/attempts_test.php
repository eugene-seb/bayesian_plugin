...
use mod_bayesian_overdue_attempt_updater;
use question_engine;
use bayesian;
...
/**
 * Unit tests for bayesian attempt overdue handling
 *
...
class attempts_test extends \advanced_testcase {
    ...
    /**
     * Test the functions bayesian_update_open_attempts(), get_list_of_overdue_attempts() and
     * update_overdue_attempts().
     */
    public function test_bulk_update_functions() {
        global $DB,$CFG;

        require_once($CFG->dirroot.'/mod/bayesian/cronlib.php');
        ...
        // Basic bayesian settings

        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        $usertimes[$attemptid] = ['timeclose' => 1200, 'timelimit' => 600, 'message' => 'Test1A', 'time1000state' => 'finished'];

        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 1800]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        $usertimes[$attemptid] = ['timeclose' => 1200, 'timelimit' => 1800, 'message' => 'Test1B', 'time1000state' => 'inprogress'];

        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 0]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        $usertimes[$attemptid] = ['timeclose' => 1200, 'timelimit' => 0, 'message' => 'Test1C', 'time1000state' => 'inprogress'];

        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 0, 'timelimit' => 600]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        $usertimes[$attemptid] = ['timeclose' => 0, 'timelimit' => 600, 'message' => 'Test1D', 'time1000state' => 'finished'];

        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 0, 'timelimit' => 0]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => null]);
        $usertimes[$attemptid] = ['timeclose' => 1300, 'timelimit' => 0, 'message' => 'Test2A', 'time1000state' => 'inprogress'];

        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 0]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1100, 'timelimit' => null]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => null, 'timelimit' => 700]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 0, 'timelimit' => 600]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => null, 'timelimit' => 500]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 0, 'timelimit' => 600]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => null, 'timelimit' => 0]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 500]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        $usertimes[$attemptid] = ['timeclose' => 1300, 'timelimit' => 500, 'message' => 'Test2F', 'time1000state' => 'finished'];
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 1000, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian), 'attempt' => 1]);
        $usertimes[$attemptid] = ['timeclose' => 1300, 'timelimit' => 500, 'message' => 'Test2G', 'time1000state' => 'inprogress'];

        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group3->id, 'timeclose' => 1300, 'timelimit' => 500]); // User not in group.
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 501]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group2->id, 'timeclose' => 1301, 'timelimit' => 500]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1301, 'timelimit' => 500]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group2->id, 'timeclose' => 1300, 'timelimit' => 501]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group2->id, 'timeclose' => 1300, 'timelimit' => 501]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group3->id, 'timeclose' => 1500, 'timelimit' => 1000]); // User not in group.
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 500]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group2->id, 'timeclose' => null, 'timelimit' => 501]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 500]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group2->id, 'timeclose' => 1301, 'timelimit' => null]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 500]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group2->id, 'timeclose' => 1301, 'timelimit' => 0]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 500]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group2->id, 'timeclose' => 0, 'timelimit' => 501]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'timeclose' => 1201, 'timelimit' => 601]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'timeclose' => 0, 'timelimit' => 601]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'timeclose' => 1201, 'timelimit' => 0]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'timeclose' => null, 'timelimit' => 601]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'timeclose' => 1201, 'timelimit' => null]);
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'userid' => 0, 'timeclose' => 1201, 'timelimit' => 601]); // Not user.
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        $attemptid = $DB->insert_record('bayesian_attempts', ['bayesian' => $bayesian->id, 'userid' => $user1->id, 'state' => 'overdue',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($bayesian)]);
        ...
        foreach ($usertimes as $attemptid => $times) {
            $attempt = $DB->get_record('bayesian_attempts', ['id' => $attemptid], '*', MUST_EXIST);
        ...
        //
        // Test bayesian_update_open_attempts().
        //

        bayesian_update_open_attempts(['courseid' => $course->id]);
        foreach ($usertimes as $attemptid => $times) {
            $attempt = $DB->get_record('bayesian_attempts', ['id' => $attemptid], '*', MUST_EXIST);

            if ($attempt->state == 'overdue') {
                $graceperiod = $DB->get_field('bayesian', 'graceperiod', ['id' => $attempt->bayesian]);
            } else {
                $graceperiod = 0;
            }
        ...
        $overduehander = new mod_bayesian_overdue_attempt_updater();
        ...
        $attempts->close();
        $this->assertEquals($DB->count_records_select('bayesian_attempts', 'timecheckstate IS NOT NULL'), $count);
        ...
        [$count, $bayesiancount] = $overduehander->update_overdue_attempts(1000, 940);

        $attempts = $DB->get_records('bayesian_attempts', null, 'bayesian, userid, attempt',
                'id, bayesian, userid, attempt, state, timestart, timefinish, timecheckstate');
        foreach ($attempts as $attempt) {
            $this->assertTrue(isset($usertimes[$attempt->id]));
            $times = $usertimes[$attempt->id];
        ...
        $this->assertEquals(19, $bayesiancount);
    }

    /**
     * Make any old question usage for a bayesian.
     *
     ...
     */
    protected function usage_id(\stdClass $bayesian): int {
        $quba = question_engine::make_questions_usage_by_activity('mod_bayesian',
                \context_module::instance($bayesian->cmid));
    ...
    public function test_group_event_handlers() {
        ...
        bayesian_update_open_attempts(['bayesianid' => $bayesian->id]);
        $this->assertEquals(1300, $DB->get_field('bayesian_attempts', 'timecheckstate', ['id' => $attemptid]));

        // remove from group
        $this->assertTrue(groups_remove_member($group1, $user1));
        $this->assertEquals(1200, $DB->get_field('bayesian_attempts', 'timecheckstate', ['id' => $attemptid]));
        ...
        $this->assertEquals(1200, $DB->get_field('bayesian_attempts', 'timecheckstate', ['id' => $attemptid]));
        $this->assertEquals(0, $DB->count_records('bayesian_overrides', ['bayesian' => $bayesian->id]));

        // add a group2 override
        $DB->insert_record('bayesian_overrides',
                ['bayesian' => $bayesian->id, 'groupid' => $group2->id, 'timeclose' => 1400, 'timelimit' => null]);
        bayesian_update_open_attempts(['bayesianid' => $bayesian->id]);
        $this->assertEquals(1400, $DB->get_field('bayesian_attempts', 'timecheckstate', ['id' => $attemptid]));
    ...
    /**
     * Test the functions bayesian_create_attempt_handling_errors
     */
    public function test_bayesian_create_attempt_handling_errors() {
        ...
        $bayesian = $bayesiangenerator->create_instance(['course' => $course->id, 'questionsperpage' => 0, 'grade' => 100.0,
            'sumgrades' => 2]);
        // Create questions.
        ...
        $bayesianobj = bayesian::create($bayesian->id, $user1->id);
        ...
        $timenow = time();
        // Create an attempt.
        $attempt = bayesian_create_attempt($bayesianobj, 1, null, $timenow, false, $user1->id);
        bayesian_start_new_attempt($bayesianobj, $quba, $attempt, 1, $timenow);
        bayesian_attempt_save_started($bayesianobj, $quba, $attempt);
        $result = bayesian_create_attempt_handling_errors($attempt->id, $bayesian->cmid);
        $this->assertEquals($result->get_attemptid(), $attempt->id);
        try {
            $result = bayesian_create_attempt_handling_errors($attempt->id, 9999);
            $this->fail('Exception expected due to invalid course module id.');
        } catch (\moodle_exception $e) {
            $this->assertEquals('invalidcoursemodule', $e->errorcode);
        }
        try {
            bayesian_create_attempt_handling_errors(9999, $result->get_cmid());
            $this->fail('Exception expected due to bayesian content change.');
        } catch (\moodle_exception $e) {
            $this->assertEquals('attempterrorcontentchange', $e->errorcode);
        }
        try {
            bayesian_create_attempt_handling_errors(9999);
            $this->fail('Exception expected due to invalid bayesian attempt id.');
        } catch (\moodle_exception $e) {
        ...
        try {
            bayesian_create_attempt_handling_errors(9999, $result->get_cmid());
            $this->fail('Exception expected due to bayesian content change for user without permission.');
        } catch (\moodle_exception $e) {
            $this->assertEquals('attempterrorcontentchangeforuser', $e->errorcode);
        }
        try {
            bayesian_create_attempt_handling_errors($attempt->id, 9999);
