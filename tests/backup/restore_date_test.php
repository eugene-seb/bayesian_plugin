...
class restore_date_test extends \restore_date_testcase {
    ...
    public function test_restore_dates() {
        ...
        list($course, $bayesian) = $this->create_course_and_module('bayesian', $record);
        ...
        $attempt = bayesian_create_attempt($bayesianobj, 1, false, $timestamp, false);
        ...
        bayesian_start_new_attempt($bayesianobj, $quba, $attempt, 1, $timestamp);
        ...
        $grade->id = $DB->insert_record('bayesian_grades', $grade);
        ...
        $DB->insert_record('bayesian_overrides', $override);

        // Set time fields to a constant for easy validation.
        $DB->set_field('bayesian_attempts', 'timefinish', $timestamp);

        // Do backup and restore.
        $newcourseid = $this->backup_and_restore($course);
        $newbayesian = $DB->get_record('bayesian', ['course' => $newcourseid]);

        $this->assertFieldsNotRolledForward($bayesian, $newbayesian, ['timecreated', 'timemodified']);
        $props = ['timeclose', 'timeopen'];
        $this->assertFieldsRolledForward($bayesian, $newbayesian, $props);

        $newattempt = $DB->get_record('bayesian_attempts', ['bayesian' => $newbayesian->id]);
        $newoverride = $DB->get_record('bayesian_overrides', ['bayesian' => $newbayesian->id]);
        $newgrade = $DB->get_record('bayesian_grades', ['bayesian' => $newbayesian->id]);
