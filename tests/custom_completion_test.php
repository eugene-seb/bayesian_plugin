...
use grade_item;
use mod_bayesian\completion\custom_completion;
use question_engine;
use bayesian;
...
/**
 * Class for unit testing mod_bayesian/custom_completion.
 *
 * @package   mod_bayesian
 * @copyright 2021 Shamim Rezaie <shamim@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \mod_bayesian\completion\custom_completion
 */
class custom_completion_test extends advanced_testcase {
     ...
     * @return array [$students, $bayesian, $cm, $litecm]
     */
    private function setup_bayesian_for_testing_completion(array $completionoptions): array {
        global $CFG, $DB;
        ...
        $litecm = get_coursemodule_from_id('bayesian', $bayesian->cmid);
     ...
     *
     * @param array $attemptoptions ['bayesian'] => object, ['student'] => object, ['tosubmit'] => array, ['attemptnumber'] => int
     */
    private function do_attempt_bayesian(array $attemptoptions) {
    ...
    /**
     * Test checking the completion state of a bayesian base on core's completionpassgrade criteria.
     * The bayesian requires a passing grade to be completed.
     */
    public function test_completionpass() {
        list($students, $bayesian, $cm) = $this->setup_bayesian_for_testing_completion([
            'nbstudents' => 2,
            'qtype' => 'numerical',
            'bayesianoptions' => [
    ...
    public function test_update_moduleinfo() {
        ...
        list($students, $bayesian, $cm, $litecm) = $this->setup_bayesian_for_testing_completion([
            'nbstudents' => 1,
            'qtype' => 'numerical',
            'nbquestions' => 2,
            'sumgrades' => 100,
        ...
        // Update bayesian with passgrade = 20 and use highest grade to calculate.
        $moduleinfo = $this->prepare_module_info($cm, $bayesian, $course, 20, bayesian_GRADEHIGHEST);
        ...
        // Update bayesian with gradepass = 80 and use highest grade to calculate completion.
        $moduleinfo = $this->prepare_module_info($cm, $bayesian, $course, 80, bayesian_GRADEHIGHEST);
        update_moduleinfo($litecm, $moduleinfo, $course, null);
        ...
        // Update bayesian with gradepass = 80 and use average grade to calculate completion.
        $moduleinfo = $this->prepare_module_info($cm, $bayesian, $course, 80, bayesian_GRADEAVERAGE);
        update_moduleinfo($litecm, $moduleinfo, $course, null);
        ...
        // Update bayesian with gradepass = 50 and use average grade to calculate completion.
        $moduleinfo = $this->prepare_module_info($cm, $bayesian, $course, 50, bayesian_GRADEAVERAGE);
        update_moduleinfo($litecm, $moduleinfo, $course, null);
        ...
        // Update bayesian with gradepass = 50 and use first attempt grade to calculate completion.
        $moduleinfo = $this->prepare_module_info($cm, $bayesian, $course, 50, bayesian_ATTEMPTFIRST);
        update_moduleinfo($litecm, $moduleinfo, $course, null);
        ...
        // Update bayesian with gradepass = 50 and use last attempt grade to calculate completion.
        $moduleinfo = $this->prepare_module_info($cm, $bayesian, $course, 50, bayesian_ATTEMPTLAST);
        update_moduleinfo($litecm, $moduleinfo, $course, null);
    ...
    /**
     * Set up moduleinfo object sample data for bayesian instance.
     *
     * @param object $cm course-module instance
     * @param object $bayesian bayesian instance data.
     ...
     * @return \stdClass
     */
    private function prepare_module_info(object $cm, object $bayesian, object $course, int $gradepass, string $grademethod): \stdClass {
        ...
        $moduleinfo->modulename = 'bayesian';
        $moduleinfo->bayesianpassword = '';
        ...
        $moduleinfo->name = $bayesian->name;
        $moduleinfo->timeopen = $bayesian->timeopen;
        $moduleinfo->timeclose = $bayesian->timeclose;
        $moduleinfo->timelimit = $bayesian->timelimit;
        $moduleinfo->graceperiod = $bayesian->graceperiod;
        $moduleinfo->decimalpoints = $bayesian->decimalpoints;
        $moduleinfo->questiondecimalpoints = $bayesian->questiondecimalpoints;
