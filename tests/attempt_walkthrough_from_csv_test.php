...
class attempt_walkthrough_from_csv_test extends \advanced_testcase {
    ...
    /**
     * @var stdClass the bayesian record we create.
     */
    protected $bayesian;
     ...
     * directory.
     *
     * @param array $bayesiansettings of settings read from csv file bayesianzes.csv
    ...
    public function test_walkthrough_from_csv($bayesiansettings, $csvdata) {
        ...
        // https://github.com/jamiepratt/moodle-bayesian-tools/tree/master/responsegenerator

        $this->create_bayesian_simulate_attempts_and_check_results($bayesiansettings, $csvdata);
    }

    public function create_bayesian($bayesiansettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $bayesiansettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->bayesian = $bayesiangenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                bayesian_add_bayesian_question($slotquestion['id'], $this->bayesian, 0, $slotquestion['mark']);
            } else {
                bayesian_add_random_questions($this->bayesian, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create bayesian, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $bayesiansettings bayesian overrides for this bayesian.
     * @param array $csvdata Data loaded from csv files for this test.
     */
    protected function create_bayesian_simulate_attempts_and_check_results(array $bayesiansettings, array $csvdata) {
        $this->resetAfterTest();
    ...
    public function get_data_for_walkthrough(): array {
        $bayesianzes = $this->load_csv_data_file('bayesianzes')['bayesianzes'];
        $datasets = array();
        foreach ($bayesianzes as $bayesiansettings) {
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $bayesiansettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $bayesiansettings['testnumber'])[$file];
                }
            }
            $datasets[] = array($bayesiansettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the bayesian_attempt as stored in the db.
     */
    protected function walkthrough_attempts(array $steps): array {
        ...
        foreach ($steps as $steprow) {
            ...
            // Find existing user or make a new user to do the bayesian.
            ...
            if (!isset($attemptids[$step['bayesianattempt']])) {
                // Start the attempt.
                $bayesianobj = bayesian::create($this->bayesian->id, $user->id);
                ...
                $prevattempts = bayesian_get_user_attempts($this->bayesian->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = bayesian_create_attempt($bayesianobj, $attemptnumber, null, $timenow, false, $user->id);
                ...
                bayesian_start_new_attempt($bayesianobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                bayesian_attempt_save_started($bayesianobj, $quba, $attempt);
                $attemptid = $attemptids[$step['bayesianattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['bayesianattempt']];
            }
     ...
     * @param array $attemptids attempt no as in csv file => the id of the bayesian_attempt as stored in the db.
     */
    protected function check_attempts_results(array $results, array $attemptids) {
        foreach ($results as $resultrow) {
            ...
            $attemptobj = bayesian_attempt::create($attemptids[$result['bayesianattempt']]);
     ...
     * @param bayesian_attempt $attemptobj         the attempt object loaded from db.
     */
    protected function check_attempt_results(array $result, bayesian_attempt $attemptobj) {
        foreach ($result as $fieldname => $value) {
            if ($value === '!NULL!') {
                $value = null;
            }
            switch ($fieldname) {
                case 'bayesianattempt' :
                    break;
                case 'attemptnumber' :
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['bayesianattempt']}.");
                                    break;
                                default :
                ...
                case 'summarks' :
                    $this->assertEquals((float)$value, $attemptobj->get_sum_marks(),
                        "Sum of marks of attempt {$result['bayesianattempt']}.");
                    break;
                case 'bayesiangrade' :
                    // Check bayesian grades.
                    $grades = bayesian_get_user_grades($attemptobj->get_bayesian(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "bayesian grade for attempt {$result['bayesianattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    $gradebookgrades = grade_get_grades($attemptobj->get_courseid(),
                                                        'mod', 'bayesian',
                                                        $attemptobj->get_bayesianid(),
                                                        $attemptobj->get_userid());
                    $gradebookitem = array_shift($gradebookgrades->items);
                    $gradebookgrade = array_shift($gradebookitem->grades);
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['bayesianattempt']}.");
                    break;
                default :
