<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Helper trait for bayesian question unit tests.
 *
 * This trait helps to execute different tests for bayesian, for example if it needs to create a bayesian, add question
 * to the question, add random quetion to the bayesian, do a backup or restore.
 *
 * @package    mod_bayesian
 * @category   test
 * @copyright  2021 Catalyst IT Australia Pty Ltd
 * @author     Safat Shahin <safatshahin@catalyst-au.net>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
trait bayesian_question_helper_test_trait {

    /** @var \stdClass $course Test course to contain bayesian. */
    protected $course;

    /** @var \stdClass $bayesian A test bayesian. */
    protected $bayesian;

    /** @var \stdClass $user A test logged-in user. */
    protected $user;

    /**
     * Create a test bayesian for the specified course.
     *
     * @param \stdClass $course
     * @return  \stdClass
     */
    protected function create_test_bayesian(\stdClass $course): \stdClass {

        /** @var mod_bayesian_generator $bayesiangenerator */
        $bayesiangenerator = $this->getDataGenerator()->get_plugin_generator('mod_bayesian');

        return $bayesiangenerator->create_instance([
            'course' => $course->id,
            'questionsperpage' => 0,
            'grade' => 100.0,
            'sumgrades' => 2,
        ]);
    }

    /**
     * Helper method to add regular questions in bayesian.
     *
     * @param component_generator_base $questiongenerator
     * @param \stdClass $bayesian
     * @param array $override
     */
    protected function add_two_regular_questions($questiongenerator, \stdClass $bayesian, $override = null): void {
        // Create a couple of questions.
        $cat = $questiongenerator->create_question_category($override);

        $saq = $questiongenerator->create_question('shortanswer', null, array('category' => $cat->id));
        // Create another version.
        $questiongenerator->update_question($saq);
        bayesian_add_bayesian_question($saq->id, $bayesian);
        $numq = $questiongenerator->create_question('numerical', null, array('category' => $cat->id));
        // Create two version.
        $questiongenerator->update_question($numq);
        $questiongenerator->update_question($numq);
        bayesian_add_bayesian_question($numq->id, $bayesian);
    }

    /**
     * Helper method to add random question to bayesian.
     *
     * @param component_generator_base $questiongenerator
     * @param \stdClass $bayesian
     * @param array $override
     */
    protected function add_one_random_question($questiongenerator, \stdClass $bayesian, $override = []): void {
        // Create a random question.
        $cat = $questiongenerator->create_question_category($override);
        $questiongenerator->create_question('truefalse', null, array('category' => $cat->id));
        $questiongenerator->create_question('essay', null, array('category' => $cat->id));
        bayesian_add_random_questions($bayesian, 0, $cat->id, 1, false);
    }

    /**
     * Attempt questions for a bayesian and user.
     *
     * @param \stdClass $bayesian bayesian to attempt.
     * @param \stdClass $user A user to attempt the bayesian.
     * @param int $attemptnumber
     * @return array
     */
    protected function attempt_bayesian(\stdClass $bayesian, \stdClass $user, $attemptnumber = 1): array {
        $this->setUser($user);

        $starttime = time();
        $bayesianobj = bayesian::create($bayesian->id, $user->id);

        $quba = question_engine::make_questions_usage_by_activity('mod_bayesian', $bayesianobj->get_context());
        $quba->set_preferred_behaviour($bayesianobj->get_bayesian()->preferredbehaviour);

        // Start the attempt.
        $attempt = bayesian_create_attempt($bayesianobj, $attemptnumber, null, $starttime, false, $user->id);
        bayesian_start_new_attempt($bayesianobj, $quba, $attempt, $attemptnumber, $starttime);
        bayesian_attempt_save_started($bayesianobj, $quba, $attempt);

        // Finish the attempt.
        $attemptobj = bayesian_attempt::create($attempt->id);
        $attemptobj->process_finish($starttime, false);

        $this->setUser();
        return [$bayesianobj, $quba, $attemptobj];
    }

    /**
     * A helper method to backup test bayesian.
     *
     * @param \stdClass $bayesian bayesian to attempt.
     * @param \stdClass $user A user to attempt the bayesian.
     * @return string A backup ID ready to be restored.
     */
    protected function backup_bayesian(\stdClass $bayesian, \stdClass $user): string {
        global $CFG;

        // Get the necessary files to perform backup and restore.
        require_once($CFG->dirroot . '/backup/util/includes/backup_includes.php');
        require_once($CFG->dirroot . '/backup/util/includes/restore_includes.php');

        $backupid = 'test-question-backup-restore';

        $bc = new backup_controller(backup::TYPE_1ACTIVITY, $bayesian->cmid, backup::FORMAT_MOODLE,
            backup::INTERACTIVE_NO, backup::MODE_GENERAL, $user->id);
        $bc->execute_plan();

        $results = $bc->get_results();
        $file = $results['backup_destination'];
        $fp = get_file_packer('application/vnd.moodle.backup');
        $filepath = $CFG->dataroot . '/temp/backup/' . $backupid;
        $file->extract_to_pathname($fp, $filepath);
        $bc->destroy();

        return $backupid;
    }

    /**
     * A helper method to restore provided backup.
     *
     * @param string $backupid Backup ID to restore.
     * @param stdClass $course
     * @param stdClass $user
     */
    protected function restore_bayesian(string $backupid, stdClass $course, stdClass $user): void {
        $rc = new restore_controller($backupid, $course->id,
            backup::INTERACTIVE_NO, backup::MODE_GENERAL, $user->id, backup::TARGET_CURRENT_ADDING);
        $this->assertTrue($rc->execute_precheck());
        $rc->execute_plan();
        $rc->destroy();
    }

    /**
     * A helper method to emulate duplication of the bayesian.
     *
     * @param stdClass $course
     * @param stdClass $bayesian
     * @return \cm_info|null
     */
    protected function duplicate_bayesian($course, $bayesian): ?\cm_info {
        return duplicate_module($course, get_fast_modinfo($course)->get_cm($bayesian->cmid));
    }
}
