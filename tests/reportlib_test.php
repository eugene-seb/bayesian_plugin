...
class reportlib_test extends \advanced_testcase {
    public function test_bayesian_report_index_by_keys() {
        $datum = array();
        $object = new \stdClass();
        $object->qid = 3;
        $object->aid = 101;
        ...
        $indexed = bayesian_report_index_by_keys($datum, array('aid', 'qid'));
        ...
        $indexed = bayesian_report_index_by_keys($datum, array('aid', 'qid'), false);
    ...
    public function test_bayesian_report_scale_summarks_as_percentage() {
        $bayesian = new \stdClass();
        $bayesian->sumgrades = 10;
        $bayesian->decimalpoints = 2;

        $this->assertEquals('12.34567%',
            bayesian_report_scale_summarks_as_percentage(1.234567, $bayesian, false));
        $this->assertEquals('12.35%',
            bayesian_report_scale_summarks_as_percentage(1.234567, $bayesian, true));
        $this->assertEquals('-',
            bayesian_report_scale_summarks_as_percentage('-', $bayesian, true));
    }

    public function test_bayesian_report_qm_filter_select_only_one_attempt_allowed() {
        $bayesian = new \stdClass();
        $bayesian->attempts = 1;
        $this->assertSame('', bayesian_report_qm_filter_select($bayesian));
    }

    public function test_bayesian_report_qm_filter_select_average() {
        $bayesian = new \stdClass();
        $bayesian->attempts = 10;
        $bayesian->grademethod = bayesian_GRADEAVERAGE;
        $this->assertSame('', bayesian_report_qm_filter_select($bayesian));
    }

    public function test_bayesian_report_qm_filter_select_first_last_best() {
        global $DB;
        $this->resetAfterTest();

        $fakeattempt = new \stdClass();
        $fakeattempt->userid = 123;
        $fakeattempt->bayesian = 456;
        $fakeattempt->layout = '1,2,0,3,4,0,5';
        $fakeattempt->state = bayesian_attempt::FINISHED;

        // We intentionally insert these in a funny order, to test the SQL better.
        // The test data is:
        // id | bayesianid | user | attempt | sumgrades | state
        // ---------------------------------------------------
        ...
        $DB->insert_record('bayesian_attempts', $fakeattempt);
        ...
        $fakeattempt->state = bayesian_attempt::IN_PROGRESS;
        $DB->insert_record('bayesian_attempts', $fakeattempt);
        ...
        $bayesian->grademethod = bayesian_ATTEMPTFIRST;
        $firstattempt = $DB->get_records_sql("
                SELECT * FROM {bayesian_attempts} bayesiana WHERE userid = ? AND bayesian = ? AND "
                        . bayesian_report_qm_filter_select($bayesian), array(123, 456));
        $this->assertEquals(1, count($firstattempt));
        $firstattempt = reset($firstattempt);
        $this->assertEquals(1, $firstattempt->attempt);

        $bayesian->grademethod = bayesian_ATTEMPTLAST;
        ...
        $bestattempt = $DB->get_records_sql("
                SELECT * FROM {bayesian_attempts} qa_alias WHERE userid = ? AND bayesian = ? AND "
                . bayesian_report_qm_filter_select($bayesian, 'qa_alias'), array(123, 456));
