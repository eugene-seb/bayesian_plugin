...
Feature: Settings form fields disabled if not required
  In to create bayesianzes as simply as possible
  As a teacher
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | bayesian     | C1     | 1       | Test bayesian 1 |

  @javascript
  Scenario: Depending on the number of attempts, different form fields are disabled.
    When I am on the "Test bayesian 1" "bayesian activity editing" page logged in as teacher
  ...
  Scenario: Depending on whether there is a close date, some review options are disabled.
    ...
    And I add a "bayesian" to section "1"
    And I expand all fieldsets
    And I set the field "Name" to "Test bayesian"
  ...
  @javascript
  Scenario: If there are bayesian attempts, there is not option to repaginate.
    ...
    And bayesian "bayesian 1" contains the following questions:
      ...
      | TF1      | 1    |         |
    When I am on the "bayesian 1" "bayesian activity editing" page logged in as teacher
    And I expand all fieldsets
    Then I should see "Repaginate now"
    And user "student1" has attempted "bayesian 1" with responses:
      | slot | response |
      |   1  | True     |
    And I am on the "bayesian 1" "bayesian activity editing" page
    And I expand all fieldsets
    And I should not see "Repaginate now"
