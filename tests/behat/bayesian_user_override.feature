@mod @mod_bayesian
Feature: bayesian user override
  In order to grant a student special access to a bayesian
  As a teacher
  ...
  Scenario: Add, modify then delete a user override
    Given the following "activities" exist:
      ...
      | bayesian       | Test bayesian | C1     | bayesian1    |
    And I am on the "Test bayesian" "mod_bayesian > View" page logged in as "teacher"
  ...
  @javascript
  Scenario: Can add a user override when the bayesian is not available to the student
    Given the following "activities" exist:
      | activity   | name      | course | idnumber | visible |
      | bayesian       | Test bayesian | C1     | bayesian1    | 0       |
    When I am on the "Test bayesian" "mod_bayesian > User overrides" page logged in as "teacher"
    And I press "Add user override"
    And I set the following fields to these values:
      | Override user    | Student One (student1@example.com) |
      | Attempts allowed | 1                                  |
  ...
  Scenario: A teacher without accessallgroups permission should only be able to add user override for users that he/she shares groups with,
    ...
    And the following "activities" exist:
      ...
      | bayesian     | Test bayesian | C1     | bayesian1    | 1         |
    When I am on the "Test bayesian" "mod_bayesian > User overrides" page logged in as "teacher"
    And I press "Add user override"
  ...
  Scenario: A non-editing teacher can see the overrides, but not change them
    Given the following "activities" exist:
      | activity   | name      | course | idnumber |
      | bayesian       | Test bayesian | C1     | bayesian1    |
    And the following "mod_bayesian > user overrides" exist:
      | bayesian      | user     | attempts |
      | Test bayesian | student1 | 2        |
      | Test bayesian | student2 | 2        |
    And I am on the "Test bayesian" "mod_bayesian > View" page logged in as "helper"
    ...
    And I am on the "Test bayesian" "mod_bayesian > View" page
  ...
  Scenario: Add button disabled if no users
    ...
    And the following "activities" exist:
      | activity   | name       | course | idnumber |
      | bayesian       | Other bayesian | C2     | bayesian2    |
    When I am on the "Other bayesian" "mod_bayesian > User overrides" page logged in as "admin"
