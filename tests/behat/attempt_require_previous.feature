@mod @mod_bayesian
Feature: Attempt a bayesian where some questions require that the previous question has been answered.
  In order to complete a bayesian where questions require previous ones to be complete
  As a student
  ...
  Scenario: A question that requires the previous one is initally blocked
    ...
    And the following "activities" exist:
      ...
      | bayesian       | bayesian 1 | bayesian 1 description | C1     | bayesian1    | immediatefeedback  |
    ...
    And I log out
    And I am on the "bayesian 1 > student > Attempt 1" "mod_bayesian > Attempt review" page logged in as "teacher"
  ...
  @javascript
  Scenario: After bayesian submitted, all questions show on the review page
    Given the following "questions" exist:
      | questioncategory | qtype       | name  | questiontext    |
      | Test questions   | truefalse   | TF1   | First question  |
      | Test questions   | truefalse   | TF2   | Second question |
  ...
  @javascript
  Scenario: A questions cannot be blocked in a deferred feedback bayesian (despite what is set in the DB).
    Given the following "questions" exist:
      | questioncategory | qtype       | name  | questiontext    |
      | Test questions   | truefalse   | TF1   | First question  |
      | Test questions   | truefalse   | TF2   | Second question |
    And the following "activities" exist:
      | activity   | name   | intro              | course | idnumber | preferredbehaviour |
      | bayesian       | bayesian 1 | bayesian 1 description | C1     | bayesian1    | deferredfeedback   |
    And bayesian "bayesian 1" contains the following questions:
      | question | page | requireprevious |
      | TF1      | 1    | 0               |
      | TF2      | 1    | 1               |
  ...
  Scenario: Questions cannot be blocked in a shuffled section (despite what is set in the DB).
    ...
    And the following "activities" exist:
      ...
      | bayesian       | bayesian 1 | bayesian 1 description | C1     | bayesian1    | immediatefeedback  | 2                |
    And bayesian "bayesian 1" contains the following questions:
      | question | page | requireprevious |
      | TF1      | 1    | 1               |
      | TF2      | 2    | 1               |
  ...
  @javascript
  Scenario: A questions cannot be blocked in sequential bayesian (despite what is set in the DB).
    Given the following "questions" exist:
      | questioncategory | qtype       | name  | questiontext    |
      | Test questions   | truefalse   | TF1   | First question  |
      | Test questions   | truefalse   | TF2   | Second question |
