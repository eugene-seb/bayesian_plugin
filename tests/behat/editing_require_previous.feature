@mod @mod_bayesian
Feature: Edit bayesianzes where some questions require the previous one to have been completed
  In order to create bayesianzes where later questions can only be seen after earlier ones are answered
  As a teacher
  I need to be able to configure this on the Edit bayesian page
  ...
  @javascript
  Scenario: Question dependency cannot apply to deferred feedback bayesianzes so UI is hidden
    Given the following "activities" exist:
      | activity   | name   | intro              | course | idnumber | preferredbehaviour |
  ...
  @javascript
  Scenario: Question dependency cannot apply to bayesianzes with sequential navigation so UI is hidden
    Given the following "activities" exist:
  ...
  Scenario: When questions are reordered, the dependency icons are updated correctly
    ...
    And bayesian "bayesian 1" contains the following questions:
      ...
      | TF3      | 1    | 1               |
    And I am on the "bayesian 1" "mod_bayesian > Edit" page
    When I move "Question 1" to "After Question 3" in the bayesian by clicking the move icon
    Then "Question 2 cannot be attempted until the previous question 1 has been completed • Click to change" "link" should be visible
    And "No restriction on when question 3 can be attempted • Click to change" "link" should be visible
