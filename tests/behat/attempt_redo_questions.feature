@mod @mod_bayesian
Feature: Allow students to redo questions in a practice bayesian, without starting a whole new attempt
  In order to practice particular skills I am struggling with
  As a student
  I need to be able to redo each question in a bayesian as often as necessary without starting a whole new attempt, if my teacher allows it.
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | bayesian     | bayesian 1 | bayesian 1 description | C1     | bayesian1    | immediatefeedback  | 1                |
  ...
  Scenario: After completing a question, there is a redo question button that restarts the question
    Given I am on the "bayesian 1" "mod_bayesian > View" page logged in as "student"
  ...
  Scenario: The redo question button is visible but disabled for teachers
    ...
    And I click on "Check" "button" in the "First question" "question"
    And I log out
    And I am on the "bayesian 1" "mod_bayesian > View" page logged in as "teacher"
  ...
  Scenario: Teachers reviewing can see all the questions attempted in a slot
    ...
    And I am on the "bayesian 1" "mod_bayesian > Statistics report" page logged in as teacher
    And I follow "TF1"
    And "False" row "Frequency" column of "bayesianresponseanalysis" table should contain "100.00%"
    And "True" row "Frequency" column of "bayesianresponseanalysis" table should contain "0.00%"
    And "[No response]" row "Frequency" column of "bayesianresponseanalysis" table should contain "100.00%"
  ...
  Scenario: Redoing questions should work with random questions as well
    Given the following "activities" exist:
      | activity | name   | intro              | course | idnumber | preferredbehaviour | canredoquestions |
      | bayesian     | bayesian 2 | bayesian 2 description | C1     | bayesian2    | immediatefeedback  | 1                |
    And I am on the "bayesian 2" "mod_bayesian > Edit" page logged in as "admin"
    And I open the "last" add to bayesian menu
    And I follow "a random question"
    And I press "Add random question"
    And user "student" has started an attempt at bayesian "bayesian 2" randomised as follows:
      | slot | actualquestion |
      | 1    | TF1            |
    And I am on the "bayesian 2" "mod_bayesian > View" page logged in as "student"
  ...
  Scenario: Teachers reviewing can see author of action in review attempt
    ...
    And the following "activities" exist:
      ...
      | bayesian     | bayesian 2 | bayesian 2 description | C1     | bayesian2    |
    And bayesian "bayesian 2" contains the following questions:
      | question | page |
      | SA1      | 1    |
    And user "student" has attempted "bayesian 2" with responses:
      | slot | response    |
      | 1    | J.K.Rowling |
    And I am on the "bayesian 2" "mod_bayesian > Manual grading report" page logged in as "teacher"
