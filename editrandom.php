...
// Get the bayesian slot.
$slot = $DB->get_record('bayesian_slots', ['id' => $slotid]);
if (!$slot) {
    new moodle_exception('invalidrandomslot', 'mod_bayesian');
}

if (!$bayesian = $DB->get_record('bayesian', ['id' => $slot->bayesianid])) {
    new moodle_exception('invalidbayesianid', 'bayesian');
}

$cm = get_coursemodule_from_instance('bayesian', $slot->bayesianid, $bayesian->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/bayesian/edit.php', ['cmid' => $cm->id]);
}

$url = new moodle_url('/mod/bayesian/editrandom.php', ['slotid' => $slotid]);
$PAGE->set_url($url);
...
$mform = new mod_bayesian\form\randomquestion_form(new moodle_url('/mod/bayesian/editrandom.php'), ['contexts' => $contexts]);
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_bayesian');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_bayesian');
