...
/**
 * Back-end code for handling data about bayesianzes and the current user's attempt.
 *
 * There are classes for loading all the information about a bayesian and attempts,
...
/**
 * Class for bayesian exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_bayesian_exception extends moodle_exception {
    /**
     * Constructor.
     *
     * @param bayesian $bayesianobj the bayesian the error relates to.
     ...
     */
    public function __construct($bayesianobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $bayesianobj->view_url();
        }
        parent::__construct($errorcode, 'bayesian', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a bayesian and the questions it contains, and making the
...
class bayesian {
    ...
    protected $cm;
    /** @var stdClass the bayesian settings from the database. */
    protected $bayesian;
    /** @var context the bayesian context. */
    protected $context;
    ...
    /** @var stdClass[] of bayesian_section rows. */
    protected $sections = null;
    /** @var bayesian_access_manager the access manager for this bayesian. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/bayesian:preview. */
     ...
     *
     * @param object $bayesian the row from the bayesian table.
     * @param object $cm the course_module object for this bayesian.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($bayesian, $cm, $course, $getcontext = true) {
        $this->bayesian = $bayesian;
        $this->cm = $cm;
        $this->bayesian->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new bayesian object for a specific user.
     *
     * @param int $bayesianid the the bayesian id.
     * @param int|null $userid the the userid (optional). If passed, relevant overrides are applied.
     * @return bayesian the new bayesian object.
     */
    public static function create($bayesianid, $userid = null) {
        global $DB;

        $bayesian = bayesian_access_manager::load_bayesian_and_settings($bayesianid);
        $course = $DB->get_record('course', array('id' => $bayesian->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('bayesian', $bayesian->id, $course->id, false, MUST_EXIST);

        // Update bayesian with override information.
        if ($userid) {
            $bayesian = bayesian_update_effective_access($bayesian, $userid);
        }

        return new bayesian($bayesian, $cm, $course);
    }

    /**
     * Create a {@link bayesian_attempt} for an attempt at this bayesian.
     *
     * @param object $attemptdata row from the bayesian_attempts table.
     * @return bayesian_attempt the new bayesian_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new bayesian_attempt($attemptdata, $this->bayesian, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this bayesian.
     */
    public function preload_questions() {
        $slots = qbank_helper::get_question_structure($this->bayesian->id, $this->context);
        $this->questions = [];
    ...
    /**
     * Fully load some or all of the questions for this bayesian. You must call
    ...
    public function load_questions($deprecated = null) {
        if ($deprecated !== null) {
            debugging('The argument to bayesian::load_questions is no longer supported. ' .
    ...
    /**
     * Get an instance of the {@link \mod_bayesian\structure} class for this bayesian.
     * @return \mod_bayesian\structure describes the questions in the bayesian.
     */
    public function get_structure() {
        return \mod_bayesian\structure::create_for_bayesian($this);
    }

    // Simple getters ==========================================================
    /**
     * Get the id of the course this bayesian belongs to.
     *
    ...
    /**
     * Get the course settings object that this bayesian belongs to.
     *
    ...
    /**
     * Get this bayesian's id (in the bayesian table).
     *
     * @return int the bayesian id.
     */
    public function get_bayesianid() {
        return $this->bayesian->id;
    }

    /**
     * Get the bayesian settings object.
     *
     * @return stdClass the row of the bayesian table.
     */
    ...
    /**
     * Get the bayesian name.
     *
     * @return string the name of this bayesian.
     */
    public function get_bayesian_name() {
        return $this->bayesian->name;
    }

    /**
     * Get the navigation method in use.
     *
     * @return int bayesian_NAVMETHOD_FREE or bayesian_NAVMETHOD_SEQ.
     */
    public function get_navigation_method() {
        return $this->bayesian->navmethod;
    }

    /** @return int the number of attempts allowed at this bayesian (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->bayesian->attempts;
    }

    /**
     * Get the course-module id for this bayesian.
     *
    ...
    /**
     * Get the course-module object for this bayesian.
     *
    ...
    /**
     * Get the bayesian context.
     *
     * @return context_module the module context for this bayesian.
     */
    ...
    /**
     * @return bool whether the current user is someone who previews the bayesian,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/bayesian:preview', $this->context);
        }
    ...
    public function is_participant($userid) {
        return is_enrolled($this->get_context(), $userid, 'mod/bayesian:attempt', $this->show_only_active_users());
    }
    ...
    /**
     * @return bool whether any questions have been added to this bayesian.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'bayesian', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this bayesian.
     *
     * @return array 0, 1, 2, ... => bayesian_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('bayesian_sections',
                    array('bayesianid' => $this->get_bayesianid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return bayesian_access_manager and instance of the bayesian_access_manager class
     * for this bayesian at this time.
     *
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new bayesian_access_manager($this, $timenow,
                    has_capability('mod/bayesian:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the bayesian context.
     *
     * @param string $capability the name of the capability to check. For example mod/bayesian:view.
    ...
    /**
     * Wrapper round the require_capability function that automatically passes in the bayesian context.
     *
     * @param string $capability the name of the capability to check. For example mod/bayesian:view.
     * @param int|null $userid A user id. By default (null) checks the permissions of the current user.
     * @param bool $doanything If false, ignore effect of admin role assignment.
    ...
    /**
     * @return string the URL of this bayesian's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/bayesian/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this bayesian's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/bayesian/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/bayesian/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
     ...
     * @return moodle_url the URL of this bayesian's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/bayesian/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/bayesian/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/bayesian/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by bayesian access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by bayesian access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_bayesian_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_bayesian_display_options::DURING ||
                $when == mod_bayesian_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_bayesian_display_options::LATER_WHILE_OPEN && $this->bayesian->timeclose &&
                $this->bayesian->reviewattempt & mod_bayesian_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'bayesian',
                    userdate($this->bayesian->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'bayesian');
        }
    }
     ...
     *
     * @param string $title the name of this particular bayesian page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_bayesian_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this bayesian.
     *
     * @param  boolean $includepotential if the bayesian include random questions,
...
/**
 * This class extends the bayesian class to hold data about the state of a particular attempt,
 * in addition to the data about the bayesian.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class bayesian_attempt {
    ...
    /** @var int maximum number of slots in the bayesian for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var bayesian object containing the bayesian settings. */
    protected $bayesianobj;

    /** @var stdClass the bayesian_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this bayesian attempt. */
    protected $quba;
    ...
    /** @var array of bayesian_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_bayesian_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the bayesian_attempts table.
     * @param object $bayesian the bayesian object for this attempt and user.
     * @param object $cm the course_module object for this bayesian.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $bayesian, $cm, $course, $loadquestions = true) {
        $this->attempt = $attempt;
        $this->bayesianobj = new bayesian($bayesian, $cm, $course);
     ...
     *
     * @param array $conditions passed to $DB->get_record('bayesian_attempts', $conditions).
     * @return bayesian_attempt the desired instance of this class.
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('bayesian_attempts', $conditions, '*', MUST_EXIST);
        $bayesian = bayesian_access_manager::load_bayesian_and_settings($attempt->bayesian);
        $course = $DB->get_record('course', array('id' => $bayesian->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('bayesian', $bayesian->id, $course->id, false, MUST_EXIST);
        ...
        return new bayesian_attempt($attempt, $bayesian, $cm, $course);
    }

    /**
     * Static function to create a new bayesian_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new bayesian_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return bayesian_attempt_state_name($state);
    }
    ...
    public function load_questions() {
        ...
        if (isset($this->quba)) {
            throw new coding_exception('This bayesian attempt has already had the questions loaded.');
        }

        $this->quba = question_engine::load_questions_usage_by_activity($this->attempt->uniqueid);
        $this->slots = $DB->get_records('bayesian_slots',
                array('bayesianid' => $this->get_bayesianid()), 'slot', 'slot, id, requireprevious');
        $this->sections = array_values($DB->get_records('bayesian_sections',
                array('bayesianid' => $this->get_bayesianid()), 'firstslot'));
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'bayesian');
                }
    ...
    // Simple getters ==========================================================
    public function get_bayesian() {
        return $this->bayesianobj->get_bayesian();
    }

    public function get_bayesianobj() {
        return $this->bayesianobj;
    }

    /** @return int the course id. */
    public function get_courseid() {
        return $this->bayesianobj->get_courseid();
    }
     ...
     */
    public function get_course() {
        return $this->bayesianobj->get_course();
    }

    /** @return int the bayesian id. */
    public function get_bayesianid() {
        return $this->bayesianobj->get_bayesianid();
    }

    /** @return string the name of this bayesian. */
    public function get_bayesian_name() {
        return $this->bayesianobj->get_bayesian_name();
    }

    /** @return int the bayesian navigation method. */
    public function get_navigation_method() {
        return $this->bayesianobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->bayesianobj->get_cm();
    }
     ...
     */
    public function get_cmid() {
        return $this->bayesianobj->get_cmid();
    }
     ...
     */
    public function is_preview_user() {
        return $this->bayesianobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this bayesian (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->bayesianobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this bayesian. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->bayesianobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the bayesian_attempts table. */
    ...
    /** @return string one of the bayesian_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/bayesian:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the bayesian in a non-trivial way?
     *
     ...
     * To see how this works, try setting a bayesian to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the bayesian
     ...
     *
     * @param question_display_options $options the display options for this bayesian attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return bayesian_feedback_for_grade($grade, $this->get_bayesian(),
                $this->bayesianobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->bayesianobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        $this->bayesianobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_bayesian_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/bayesian:attempt';
        } else {
            $capability = 'mod/bayesian:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a bayesian
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the bayesian as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/bayesian:viewreports') ||
                $this->has_capability('mod/bayesian:preview')) {
    ...
    public function can_navigate_to($slot) {
        ...
        switch ($this->get_navigation_method()) {
            case bayesian_NAVMETHOD_FREE:
                return true;
                break;
            case bayesian_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return bayesian_attempt_state($this->get_bayesian(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_bayesian_display_options for this bayesian at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = bayesian_get_review_options($this->get_bayesian(),
                        $this->attempt, $this->bayesianobj->get_context());
            ...
            $options = mod_bayesian_display_options::make_from_bayesian($this->get_bayesian(),
                    mod_bayesian_display_options::DURING);
            $options->flags = bayesian_get_flag_option($this->attempt, $this->bayesianobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the bayesian.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the bayesian, or for the
     * whole bayesian.
     *
    ...
    public function get_question_usage() {
        if (!(PHPUNIT_TEST || defined('BEHAT_TEST'))) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the bayesian_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function is_blocked_by_previous_question($slot) {
        return $slot > 1 && isset($this->slots[$slot]) && $this->slots[$slot]->requireprevious &&
            ...
            $this->get_navigation_method() != bayesian_NAVMETHOD_SEQ &&
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_bayesian()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the bayesian where this question appears.
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the bayesian this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the bayesian.
     */
    public function get_question_mark($slot) {
        return bayesian_format_question_grade($this->get_bayesian(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->bayesianobj->get_bayesian()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->bayesianobj->get_bayesian()->timelimit;
        }
        if ($this->bayesianobj->get_bayesian()->timeclose) {
            $deadlines[] = $this->bayesianobj->get_bayesian()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->bayesianobj->get_bayesian()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string bayesian view url.
     */
    public function view_url() {
        return $this->bayesianobj->view_url();
    }
     ...
     * @return string the URL of this bayesian's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->bayesianobj->start_attempt_url($page);
    }
    ...
    public function attempt_page_title(int $page) : string {
        if ($this->get_num_pages() > 1) {
            $a = new stdClass();
            $a->name = $this->get_bayesian_name();
            $a->currentpage = $page + 1;
            $a->totalpages = $this->get_num_pages();
            $title = get_string('attempttitlepaged', 'bayesian', $a);
        } else {
            $title = get_string('attempttitle', 'bayesian', $this->get_bayesian_name());
        }
    ...
    public function summary_page_title() : string {
        return get_string('attemptsummarytitle', 'bayesian', $this->get_bayesian_name());
    }

    /**
     * @return moodle_url the URL of this bayesian's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/bayesian/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/bayesian/processattempt.php');
    }
    ...
    public function review_page_title(int $page, bool $showall = false) : string {
        if (!$showall && $this->get_num_pages() > 1) {
            ...
            $a->currentpage = $page + 1;
            $a->totalpages = $this->get_num_pages();
            $title = get_string('attemptreviewtitlepaged', 'bayesian', $a);
        } else {
            $title = get_string('attemptreviewtitle', 'bayesian', $this->get_bayesian_name());
        }
    ...
    public function cannot_review_message($short = false) {
        return $this->bayesianobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'bayesian'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_bayesian_renderer $renderer the bayesian renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_bayesian_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_bayesian-blocked_question_warning');
        }
    ...
    protected function render_question_helper($slot, $reviewing, $thispageurl,
            mod_bayesian_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'bayesian', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'bayesian');
        ...
        $placeholderqa->start($this->get_bayesian()->preferredbehaviour, 1);
     ...
     *
     * @param int $slot the slot number of a question in this bayesian attempt.
    ...
    public function render_question_at_step($slot, $seq, $reviewing,
            mod_bayesian_renderer $renderer, $thispageurl = null) {
     ...
     *
     * @param int $slot the slot of a question in this bayesian attempt.
     * @param bool $reviewing is the being printed on an attempt or a review page.
     ...
     *
     * @param mod_bayesian_renderer $output the bayesian renderer to use to output things.
     * @param string $panelclass The type of panel, bayesian_attempt_nav_panel or bayesian_review_nav_panel
     * @param int $page the current page number.
     * @param bool $showall whether we are showing the whole bayesian on one page. (Used by review.php.)
     * @return block_contents the requested object.
     */
    public function get_navigation_panel(mod_bayesian_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_bayesian_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->title = get_string('bayesiannavigation', 'bayesian');
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this bayesian.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_bayesian_links_to_other_attempts} object returned contains an
     ...
     * @return mod_bayesian_links_to_other_attempts|bool containing array int => null|moodle_url.
     *      False if none.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = bayesian_get_user_attempts($this->get_bayesian()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_bayesian_links_to_other_attempts();
     ...
     * @return mod_bayesian_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->bayesianobj->get_bayesian()->graceperiod;
        ...
        // Otherwise, we were in bayesian_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->bayesianobj->get_bayesian()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('bayesian_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            bayesian_save_best_grade($this->get_bayesian(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_bayesian\question\qubaids_for_users_attempts(
                $this->get_bayesianid(), $this->get_userid(), 'all', true);

        $transaction = $DB->start_delegated_transaction();

        // Add the question to the usage. It is important we do this before we choose a variant.
        $newquestionid = qbank_helper::choose_question_for_redo($this->get_bayesianid(),
                    $this->get_bayesianobj()->get_context(), $this->slots[$slot]->id, $qubaids);
        $newquestion = question_bank::load_question($newquestionid, $this->get_bayesian()->shuffleanswers);
     ...
     * Submit the attempt.
     *
     * The separate $timefinish argument should be used when the bayesian attempt
    ...
    public function process_finish($timestamp, $processsubmitted, $timefinish = null, $studentisonline = false) {
        ...
        if (!$this->requires_manual_grading() ||
                !has_capability('mod/bayesian:emailnotifyattemptgraded', $this->get_bayesianobj()->get_context(),
                        $this->get_userid())) {
        ...
        if (!$this->is_preview()) {
            bayesian_save_best_grade($this->get_bayesian(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_bayesian\event\attempt_submitted', $timestamp, $studentisonline);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('bayesian_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_bayesian\event\attempt_becameoverdue', $timestamp, $studentisonline);

        $transaction->allow_commit();

        bayesian_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_bayesian\event\attempt_abandoned', $timestamp, $studentisonline);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp, $studentisonline) {
        global $USER;
        $bayesianrecord = $this->get_bayesian();
        $params = array(
            'context' => $this->get_bayesianobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'bayesianid' => $bayesianrecord->id,
                'studentisonline' => $studentisonline
            )
        );
        $event = $eventclass::create($params);
        $event->add_record_snapshot('bayesian', $this->get_bayesian());
        $event->add_record_snapshot('bayesian_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the bayesian.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/bayesian/$script.php.
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/bayesian/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a bayesian.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        $graceperiodmin = get_config('bayesian', 'graceperiodmin');
        ...
        $toolate = false;
        if ($timeclose !== false) {
            if ($timenow > $timeclose - bayesian_MIN_TIME_TO_CONTINUE) {
                // If there is only a very small amount of time left, there is no point trying
                // to show the student another page of the bayesian. Just finish now.
                $timeup = true;
        ...
        if ($timeup) {
            if ($this->get_bayesian()->overduehandling === 'graceperiod') {
                if ($timenow > $timeclose + $this->get_bayesian()->graceperiod + $graceperiodmin) {
        ...
        // Update the bayesian attempt record.
        try {
            if ($becomingabandoned) {
                $this->process_abandon($timenow, true);
            } else {
                if (!$toolate || $this->get_bayesian()->overduehandling === 'graceperiod') {
    ...
    public function check_page_access(int $page, bool $allownext = true): bool {
        if ($this->get_navigation_method() != bayesian_NAVMETHOD_SEQ) {
            return true;
        }
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('bayesian_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
    ...
    public function fire_attempt_viewed_event() {
        $params = array(
            ...
            'other' => array(
                'bayesianid' => $this->get_bayesianid(),
                'page' => $this->get_currentpage()
            )
        );
 ...
 * @since      Moodle 2.9
 */
class bayesian_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class bayesian_nav_question_button implements renderable {
    ...
    public $url;
    /** @var int bayesian_NAVMETHOD_FREE or bayesian_NAVMETHOD_SEQ. */
    public $navmethod;
}
 ...
 * @since      Moodle 2.0
 */
abstract class bayesian_nav_panel_base {
    /** @var bayesian_attempt */
    ...
    public function __construct(bayesian_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the bayesian navigation block.
     *
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            ...
            if (!is_null($heading)) {
                $sections = $this->attemptobj->get_bayesianobj()->get_sections();
                if (!(empty($heading) && count($sections) == 1)) {
                    $buttons[] = new bayesian_nav_section_heading(format_string($heading));
                }
            }
            ...
            $button = new bayesian_nav_question_button();
            $button->id          = 'bayesiannavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'bayesian');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'bayesian');
        } else {
            return get_string('viewed', 'bayesian');
        }
    }
     ...
     *
     * @param mod_bayesian_renderer $output the bayesian renderer to use.
     * @return string HTML to output.
     */
    public function render_before_button_bits(mod_bayesian_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_bayesian_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_bayesian()->showuserpicture == bayesian_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_bayesian()->showuserpicture == bayesian_SHOWIMAGE_LARGE) {
    ...
    public function get_button_container_class() {
        // bayesian navigation is set on 'Show all questions on one page'.
        if ($this->showall) {
            return 'allquestionsononepage';
        }
        // bayesian navigation is set on 'Show one page at a time'.
        return 'multipages';
    }
}
...
/**
 * Specialisation of {@link bayesian_nav_panel_base} for the attempt bayesian page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class bayesian_attempt_nav_panel extends bayesian_nav_panel_base {
    ...
    public function render_before_button_bits(mod_bayesian_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'bayesian'),
                array('id' => 'bayesiannojswarning'));
    }

    public function render_end_bits(mod_bayesian_renderer $output) {
        if ($this->page == -1) {
            // Don't link from the summary page to itself.
            return '';
        }
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'bayesian'), array('class' => 'endtestlink aalink')) .
...
/**
 * Specialisation of {@link bayesian_nav_panel_base} for the review bayesian page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class bayesian_review_nav_panel extends bayesian_nav_panel_base {
    public function get_question_url($slot) {
