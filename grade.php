...
$cm = get_coursemodule_from_id('bayesian', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$bayesian = $DB->get_record('bayesian', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = bayesian_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see bayesian reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/bayesian/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = bayesian_get_user_attempts($bayesian->id, $userid, 'finished');
    $attempt = null;
...
redirect(new moodle_url('/mod/bayesian/report.php', array(
