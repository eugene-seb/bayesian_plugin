...
/**
 * Defines the renderer for the bayesian module.
 *
...
/**
 * The renderer for the bayesian module.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_bayesian_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param bayesian_attempt $attemptobj an instance of bayesian_attempt.
     ...
     * @param mod_bayesian_display_options $displayoptions instance of mod_bayesian_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(bayesian_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_bayesian_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(bayesian_attempt $attemptobj, $slot, $seq,
            mod_bayesian_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(bayesian_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_bayesian_name(), true,
                                  array("context" => $attemptobj->get_bayesianobj()->get_context())));
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox bayesianreviewsummary'));
     ...
     *
     * @param bayesian_attempt $attemptobj instance of bayesian_attempt
     ...
     * @param mod_bayesian_display_options $displayoptions instance of mod_bayesian_display_options
     */
    public function questions(bayesian_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_bayesian_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_bayesian.init_review_form', null, false,
                bayesian_get_js_module());
     ...
     * @param bayesian_attempt $attemptobj instance of bayesian_attempt
     */
    public function finish_review_link(bayesian_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_bayesian.secure_window.init_close_button',
                    array($url), false, bayesian_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'bayesian'),
                    'id' => 'secureclosebutton',
                    'class' => 'mod_bayesian-next-nav btn btn-primary'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'bayesian'),
                    array('class' => 'mod_bayesian-next-nav'));
        }
    }
     ...
     */
    public function review_next_navigation(bayesian_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
            $nav .= link_arrow_left(get_string('navigateprevious', 'bayesian'),
                    $attemptobj->review_url(null, $page - 1, $showall), false, 'mod_bayesian-prev-nav');
        }
        if ($lastpage) {
            $nav .= $this->finish_review_link($attemptobj);
        } else {
            $nav .= link_arrow_right(get_string('navigatenext', 'bayesian'),
                    $attemptobj->review_url(null, $page + 1, $showall), false, 'mod_bayesian-next-nav');
        }
        return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
    }

    /**
     * Return the HTML of the bayesian timer.
     * @return string HTML content.
     */
    public function countdown_timer(bayesian_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the bayesian to be submitted immediately.
        ...
        return $this->output->render_from_template('mod_bayesian/timer', (object)[]);
    }
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'bayesian'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param bayesian_nav_panel_base $panel instance of bayesian_nav_panel_base
     */
    public function navigation_panel(bayesian_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_bayesian.nav.init', null, false,
                bayesian_get_js_module());
    ...
    /**
     * Display a bayesian navigation button.
     *
     * @param bayesian_nav_question_button $button
     * @return string HTML fragment.
     */
    protected function render_bayesian_nav_question_button(bayesian_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod, 'btn');
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'bayesian');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'bayesian', $a);
        $tagattributes = array('class' => implode(' ', $classes), 'id' => $button->id,
                                  'title' => $button->statestring, 'data-bayesian-page' => $button->page);
    ...
    /**
     * Display a bayesian navigation heading.
     *
     * @param bayesian_nav_section_heading $heading the heading.
     * @return string HTML fragment.
     */
    protected function render_bayesian_nav_section_heading(bayesian_nav_section_heading $heading) {
        if (empty($heading->heading)) {
            $headingtext = get_string('sectionnoname', 'bayesian');
            $class = ' dimmed_text';
        } else {
            $headingtext = $heading->heading;
            $class = '';
        }
        return $this->heading($headingtext, 3, 'mod_bayesian-section-heading' . $class);
    }

    /**
     * outputs the link the other attempts.
     *
     * @param mod_bayesian_links_to_other_attempts $links
     */
    protected function render_mod_bayesian_links_to_other_attempts(
            mod_bayesian_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(bayesian $bayesianobj, mod_bayesian_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->during_attempt_tertiary_nav($bayesianobj->view_url());
        $output .= $this->heading(format_string($bayesianobj->get_bayesian_name(), true,
                                  array("context" => $bayesianobj->get_context())));
        $output .= $this->bayesian_intro($bayesianobj->get_bayesian(), $bayesianobj->get_cm());
        $output .= $mform->render();
        $output .= $this->footer();
        return $output;
    }
     ...
     * @param bayesian_attempt $attemptobj Instance of bayesian_attempt
     * @param int $page Current page number
     * @param bayesian_access_manager $accessmanager Instance of bayesian_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output .= $this->bayesian_notices($messages);
     ...
     *
     * @param string|moodle_url $bayesianviewurl url of the view.php page for this bayesian.
     * @return string HTML to output.
     */
    public function during_attempt_tertiary_nav($bayesianviewurl): string {
        $output = '';
        $output .= html_writer::start_div('container-fluid tertiary-navigation');
        $output .= html_writer::start_div('row');
        $output .= html_writer::start_div('navitem');
        $output .= html_writer::link($bayesianviewurl, get_string('back'),
     ...
     */
    public function bayesian_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->notification(
            html_writer::tag('p', get_string('accessnoticesheader', 'bayesian')) . $this->access_messages($messages),
            'warning',
            false
        );
    }
     ...
     *
     * @param bayesian_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        $navmethod = $attemptobj->get_bayesian()->navmethod;
     ...
     * @param bool $lastpage is this the last page in the bayesian?
     * @param string $navmethod Optional bayesian attribute, 'free' (default) or 'sequential'
     * @return string HTML fragment.
     */
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'bayesian'), 'class' => 'mod_bayesian-prev-nav btn btn-secondary',
                    'id' => 'mod_bayesian-prev-nav'));
            $this->page->requires->js_call_amd('core_form/submit', 'init', ['mod_bayesian-prev-nav']);
        }
        if ($lastpage) {
            $nextlabel = get_string('endtest', 'bayesian');
        } else {
            $nextlabel = get_string('navigatenext', 'bayesian');
        }
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                'value' => $nextlabel, 'class' => 'mod_bayesian-next-nav btn btn-primary', 'id' => 'mod_bayesian-next-nav'));
        $output .= html_writer::end_tag('div');
        $this->page->requires->js_call_amd('core_form/submit', 'init', ['mod_bayesian-next-nav']);
    ...
    public function redo_question_button($slot, $disabled) {
        $attributes = array('type' => 'submit',  'name' => 'redoslot' . $slot,
            'value' => get_string('redoquestion', 'bayesian'),
            'class' => 'mod_bayesian-redo_question_button btn btn-secondary');
        if ($disabled) {
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_bayesian.timer.init', $options, false, bayesian_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'bayesian'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'bayesian'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_bayesian.secure_window.close',
                array($url, $delay), false, bayesian_get_js_module());
     ...
     *
     * @param bayesian_attempt $attemptobj
     * @param mod_bayesian_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->during_attempt_tertiary_nav($attemptobj->view_url());
        $output .= $this->heading(format_string($attemptobj->get_bayesian_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'bayesian'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->attributes['class'] = 'generaltable bayesiansummaryofattempt boxaligncenter';
        $table->head = array(get_string('question', 'bayesian'), get_string('status', 'bayesian'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'bayesian');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($slots as $slot) {
            ...
            if ($heading !== null) {
                ...
                $rowclasses = 'bayesiansummaryheading';
                if ($heading) {
                    $heading = format_string($heading);
                } else if (count($attemptobj->get_bayesianobj()->get_sections()) > 1) {
                    // If this is the start of an unnamed section, and the bayesian has more
                    // than one section, then add a default heading.
                    $heading = get_string('sectionnoname', 'bayesian');
            ...
            if ($attemptobj->is_question_flagged($slot)) {
                // bayesian has custom JS manipulating these image tags - so we can't use the pix_icon method here.
            ...
            $table->data[] = $row;
            $table->rowclasses[] = 'bayesiansummary' . $slot . ' ' . $attemptobj->get_question_state_class(
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == bayesian_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'bayesian'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'bayesian'));
        $button->id = 'responseform';
        $button->class = 'btn-finishattempt';
        $button->formid = 'frm-finishattempt';
        if ($attemptobj->get_state() == bayesian_attempt::IN_PROGRESS) {
            $totalunanswered = 0;
            if ($attemptobj->get_bayesian()->navmethod == 'free') {
            ...
            $this->page->requires->js_call_amd('mod_bayesian/submission_confirmation', 'init', [$totalunanswered]);
        }
        $button->primary = true;

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == bayesian_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'bayesian', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'bayesian', userdate($duedate));
        }
     ...
     * @param stdClass $bayesian the bayesian settings row from the database.
     * @param stdClass $cm the course_module settings row from the database.
     * @param context_module $context the bayesian context.
     * @param mod_bayesian_view_object $viewobj
     * @return string HTML to display
     */
    public function view_page($course, $bayesian, $cm, $context, $viewobj) {
        $output = '';

        $output .= $this->view_page_tertiary_nav($viewobj);
        $output .= $this->view_information($bayesian, $cm, $context, $viewobj->infomessages);
        $output .= $this->view_table($bayesian, $context, $viewobj);
        $output .= $this->view_result_info($bayesian, $context, $cm, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'bayesianattempt');
        return $output;
    }
     ...
     *
     * @param mod_bayesian_view_object $viewobj the information required to display the view page.
     * @return string HTML to output.
     */
    public function view_page_tertiary_nav(mod_bayesian_view_object $viewobj): string {
        $content = '';
        ...
        if ($viewobj->canedit && !$viewobj->bayesianhasquestions) {
            $content .= html_writer::link($viewobj->editurl, get_string('addquestion', 'bayesian'),
                    ['class' => 'btn btn-secondary']);
        }
     ...
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_bayesian_view_object $viewobj) {
        $output = '';

        if (!$viewobj->bayesianhasquestions) {
            $output .= html_writer::div(
                    $this->notification(get_string('noquestions', 'bayesian'), 'warning', false),
                    'text-left mb-3');
        }
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'bayesian'), 'get',
     ...
     * @param mod_bayesian_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_bayesian_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_bayesian_preflight_check_form from ' .
                    'bayesian_access_manager::get_preflight_check_form, not a warning message string.');
        }

        $button = new single_button($url, $buttontext, 'post', true);
        $button->class .= ' bayesianstartbuttondiv';
        if ($popuprequired) {
            $button->class .= ' bayesiansecuremoderequired';
        }
        ...
        $this->page->requires->js_call_amd('mod_bayesian/preflightcheck', 'init',
                array('.bayesianstartbuttondiv [type=submit]', get_string('startattempt', 'bayesian'),
                       '#mod_bayesian_preflight_form', $popupjsoptions));
     ...
     *
     * @param bool $canedit can the current user edit the bayesian?
     * @param moodle_url $editurl URL of the edit bayesian page.
     * @return string HTML to output.
     *
    ...
    public function no_questions_message($canedit, $editurl) {
        ...
        $output .= $this->notification(get_string('noquestions', 'bayesian'), 'warning', false);
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('editbayesian', 'bayesian'), 'get');
        }
        $output .= html_writer::end_tag('div');
        $output .= html_writer::end_tag('div');
    ...
    /**
     * Outputs an error message for any guests accessing the bayesian
     *
     * @param stdClass $course the course settings row from the database.
     * @param stdClass $bayesian the bayesian settings row from the database.
     * @param stdClass $cm the course_module settings row from the database.
     ...
     * @param mod_bayesian_view_object $viewobj
     */
    public function view_page_guest($course, $bayesian, $cm, $context, $messages, $viewobj) {
        $output = '';
        $output .= $this->view_page_tertiary_nav($viewobj);
        $output .= $this->view_information($bayesian, $cm, $context, $messages);
        $guestno = html_writer::tag('p', get_string('guestsno', 'bayesian'));
     ...
     * @param mod_bayesian_view_object $viewobj
     */
    public function view_page_notenrolled($course, $bayesian, $cm, $context, $messages, $viewobj) {
        global $CFG;
        $output = '';
        $output .= $this->view_page_tertiary_nav($viewobj);
        $output .= $this->view_information($bayesian, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'bayesian'));
     ...
     * @param bool $bayesianhasquestions does bayesian has questions added.
     * @return string HTML to output.
     */
    public function view_information($bayesian, $cm, $context, $messages, bool $bayesianhasquestions = false) {
        $output = '';

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'bayesianinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/bayesian:viewreports', $context)) {
            if ($strattemptnum = $this->bayesian_attempt_summary_link_to_reports($bayesian, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'bayesianattemptcounts'));
            }
        }

        if (has_any_capability(['mod/bayesian:manageoverrides', 'mod/bayesian:viewoverrides'], $context)) {
            if ($overrideinfo = $this->bayesian_override_summary_links($bayesian, $cm)) {
                $output .= html_writer::tag('div', $overrideinfo, ['class' => 'bayesianattemptcounts']);
            }
        }
    ...
    /**
     * Output the bayesian intro.
     * @param object $bayesian the bayesian settings.
     * @param object $cm the course_module object.
     * @return string HTML to output.
     */
    public function bayesian_intro($bayesian, $cm) {
        if (html_is_blank($bayesian->intro)) {
            return '';
        }

        return $this->box(format_module_intro('bayesian', $bayesian, $cm->id), 'generalbox', 'intro');
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'bayesian'), 3);
    }

    /**
     * Generates the table of data
     *
     * @param array $bayesian Array contining bayesian data
     * @param int $context The page context ID
     * @param mod_bayesian_view_object $viewobj
     */
    public function view_table($bayesian, $context, $viewobj) {
        if (!$viewobj->attempts) {
            return '';
        }
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable bayesianattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'bayesian');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'bayesian');
        $table->align[] = 'left';
        $table->size[] = '';
        if ($viewobj->markcolumn) {
            $table->head[] = get_string('marks', 'bayesian') . ' / ' .
                    bayesian_format_grade($bayesian, $bayesian->sumgrades);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        ...
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'bayesian');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'bayesian');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->attemptcolumn) {
                if ($attemptobj->is_preview()) {
                    $row[] = get_string('preview', 'bayesian');
                } else {
            ...
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    $row[] = bayesian_format_grade($bayesian, $attemptobj->get_sum_marks());
                } else {
                    $row[] = '';
                }
            }
            ...
            $attemptgrade = bayesian_rescale_grade($attemptobj->get_sum_marks(), $bayesian, false);
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $attemptobj->get_state() == bayesian_attempt::FINISHED
                            && $attemptgrade == $viewobj->mygrade
                            && $bayesian->grademethod == bayesian_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = bayesian_format_grade($bayesian, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = bayesian_feedback_for_grade($attemptgrade, $bayesian, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param bayesian_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case bayesian_attempt::OVERDUE:
                return get_string('stateoverdue', 'bayesian') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'bayesian',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case bayesian_attempt::FINISHED:
                return get_string('statefinished', 'bayesian') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'bayesian',
    ...
    /**
     * Generates data pertaining to bayesian results
     *
     * @param array $bayesian Array containing bayesian data
     * @param int $context The page context ID
     * @param int $cm The Course Module Id
     * @param mod_bayesian_view_object $viewobj
     */
    public function view_result_info($bayesian, $context, $cm, $viewobj) {
        $output = '';
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = bayesian_get_grading_option_name($bayesian->grademethod);
                $a->mygrade = bayesian_format_grade($bayesian, $viewobj->mygrade);
                $a->bayesiangrade = bayesian_format_grade($bayesian, $bayesian->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'bayesian', $a), 3);
            } else {
                $a = new stdClass();
                $a->grade = bayesian_format_grade($bayesian, $viewobj->mygrade);
                $a->maxgrade = bayesian_format_grade($bayesian, $bayesian->grade);
                $a = get_string('outofshort', 'bayesian', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'bayesian', $a), 3);
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'bayesian'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'bayesianteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'bayesian'), 3);
            $resultinfo .= html_writer::div(
                    bayesian_feedback_for_grade($viewobj->mygrade, $bayesian, $context),
                    'bayesiangradefeedback') . "\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'bayesian'));
            $button->add_action(new popup_action('click', $url, 'bayesianpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'bayesian'),
                    array('title' => get_string('reviewthisattempt', 'bayesian')));
        }
    }
     ...
     *
     * @param stdClass $bayesian the bayesian object. Only $bayesian->id is used at the moment.
     ...
     */
    public function bayesian_attempt_summary_link_to_reports($bayesian, $cm, $context,
                                                          $returnzero = false, $currentgroup = 0) {
        global $CFG;
        $summary = bayesian_num_attempt_summary($bayesian, $cm, $returnzero, $currentgroup);
        if (!$summary) {
            return '';
        }

        require_once($CFG->dirroot . '/mod/bayesian/report/reportlib.php');
        $url = new moodle_url('/mod/bayesian/report.php', array(
                'id' => $cm->id, 'mode' => bayesian_report_default_report($context)));
     ...
     * @return string HTML fragment for the link.
     */
    public function bayesian_override_summary_links(stdClass $bayesian, stdClass $cm, $currentgroup = 0): string {

        $baseurl = new moodle_url('/mod/bayesian/overrides.php', ['cmid' => $cm->id]);
        $counts = bayesian_override_summary($bayesian, $cm, $currentgroup);

        $links = [];
        if ($counts['group']) {
            $links[] = html_writer::link(new moodle_url($baseurl, ['mode' => 'group']),
                    get_string('overridessummarygroup', 'bayesian', $counts['group']));
        }
        if ($counts['user']) {
            $links[] = html_writer::link(new moodle_url($baseurl, ['mode' => 'user']),
                    get_string('overridessummaryuser', 'bayesian', $counts['user']));
        }
        ...
        switch ($counts['mode']) {
            case 'onegroup':
                return get_string('overridessummarythisgroup', 'bayesian', $links);

            case 'somegroups':
                return get_string('overridessummaryyourgroups', 'bayesian', $links);

            case 'allgroups':
                return get_string('overridessummary', 'bayesian', $links);
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'bayesian'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'bayesian'), FORMAT_MARKDOWN, $options);
...
class mod_bayesian_links_to_other_attempts implements renderable {
    /**
...
class mod_bayesian_view_object {
    /** @var array $infomessages of messages with information to display about the bayesian. */
    public $infomessages;
    /** @var array $attempts contains all the user's attempts at this bayesian. */
    public $attempts;
    /** @var array $attemptobjs bayesian_attempt objects corresponding to $attempts. */
    public $attemptobjs;
    /** @var bayesian_access_manager $accessmanager contains various access rules. */
    public $accessmanager;
    /** @var bool $canreviewmine whether the current user has the capability to
     *       review their own attempts. */
    public $canreviewmine;
    /** @var bool $canedit whether the current user has the capability to edit the bayesian. */
    public $canedit;
    /** @var moodle_url $editurl the URL for editing this bayesian. */
    public $editurl;
    ...
    public $numattempts;
    /** @var float $mygrade contains the user's final grade for a bayesian. */
    public $mygrade;
    ...
    /** @var mod_bayesian_preflight_check_form|null $preflightcheckform confirmation form that must be
    ...
    public $popupoptions;
    /** @var bool $bayesianhasquestions whether the bayesian has any questions. */
    public $bayesianhasquestions;
