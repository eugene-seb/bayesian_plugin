...
namespace bayesianaccess_numattempts;

use bayesian;
use bayesianaccess_numattempts;

defined('MOODLE_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/mod/bayesian/accessrule/numattempts/rule.php');


/**
 * Unit tests for the bayesianaccess_numattempts plugin.
 *
 * @package    bayesianaccess_numattempts
 * @category   test
...
class rule_test extends \basic_testcase {
    public function test_num_attempts_access_rule() {
        ...
        $rule = new bayesianaccess_numattempts($bayesianobj, 0);
        $attempt = new \stdClass();

        $this->assertEquals($rule->description(),
            get_string('attemptsallowedn', 'bayesianaccess_numattempts', 3));

        $this->assertFalse($rule->prevent_new_attempt(0, $attempt));
        $this->assertFalse($rule->prevent_new_attempt(2, $attempt));
        $this->assertEquals($rule->prevent_new_attempt(3, $attempt),
            get_string('nomoreattempts', 'bayesian'));
