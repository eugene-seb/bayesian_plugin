...
/**
 * Implementaton of the bayesianaccess_timelimit plugin.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesianaccess_timelimit extends bayesian_access_rule_base {

    public static function make(bayesian $bayesianobj, $timenow, $canignoretimelimits) {

        if (empty($bayesianobj->get_bayesian()->timelimit) || $canignoretimelimits) {
            return null;
        }
    ...
    public function description() {
        return get_string('bayesiantimelimit', 'bayesianaccess_timelimit',
                format_time($this->bayesian->timelimit));
    }

    public function end_time($attempt) {
        $timedue = $attempt->timestart + $this->bayesian->timelimit;
        if ($this->bayesian->timeclose) {
            $timedue = min($timedue, $this->bayesian->timeclose);
        }
        return $timedue;
    }
    ...
    public function add_preflight_check_form_fields(mod_bayesian_preflight_check_form $bayesianform,
        ...
        $mform->addElement('header', 'honestycheckheader',
                get_string('confirmstartheader', 'bayesianaccess_timelimit'));
        $mform->addElement('static', 'honestycheckmessage', '',
                get_string('confirmstart', 'bayesianaccess_timelimit', format_time($this->bayesian->timelimit)));
    }
}
