...
<plist version=\"1.0\"><dict><key>showTaskBar</key><true/><key>allowWlan</key><false/><key>showReloadButton</key><true/>"
        ...
        $this->assertTrue(\bayesianaccess_seb\helper::is_valid_seb_config($validseb));
        $this->assertFalse(\bayesianaccess_seb\helper::is_valid_seb_config($invalidseb));
        $this->assertFalse(\bayesianaccess_seb\helper::is_valid_seb_config($emptyseb));
    }
    ...
    public function test_get_seb_file_headers() {
        ...
        $headers = \bayesianaccess_seb\helper::get_seb_file_headers($expiretime);
    ...
    public function test_can_not_get_config_content_with_invalid_cmid() {
        ...
        $this->expectExceptionMessage("Can't find data record in database. (SELECT cm.*, m.name, md.name AS modname \n"
            ...
            . "                   JOIN {bayesian} m ON m.id = cm.instance\n"
            . "                   \n"
            ...
            . "  'modulename' => 'bayesian',\n"
            .')])');
        \bayesianaccess_seb\helper::get_seb_config_content('999');
    }
    ...
    public function test_can_not_get_config_content_when_user_not_enrolled_in_course() {
        ...
        $this->setAdminUser();
        $course = $this->getDataGenerator()->create_course();
        $bayesian = $this->create_test_bayesian($course, \bayesianaccess_seb\settings_provider::USE_SEB_CONFIG_MANUALLY);
        ...
        \bayesianaccess_seb\helper::get_seb_config_content($bayesian->cmid);
    }

    /**
     * Test that if SEB bayesian settings can't be found, a seb config content won't be provided.
     */
    public function test_can_not_get_config_content_if_config_not_found_for_cmid() {
        ...
        $this->expectException(\moodle_exception::class);
        $this->expectExceptionMessage("No SEB config could be found for bayesian with cmid: $bayesian->cmid");
        \bayesianaccess_seb\helper::get_seb_config_content($bayesian->cmid);
    }

    /**
     * That that if config is empty for a bayesian, a seb config content won't be provided.
     */
    public function test_can_not_get_config_content_if_config_empty() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $bayesian = $this->create_test_bayesian($course, \bayesianaccess_seb\settings_provider::USE_SEB_NO);
    ...
    public function test_config_provided() {
        ...
        $config = \bayesianaccess_seb\helper::get_seb_config_content($bayesian->cmid);

        $url = new \moodle_url("/mod/bayesian/view.php", ['id' => $bayesian->cmid]);
