...
use bayesianaccess_seb\privacy\provider;
...
class provider_test extends provider_testcase {
    ...
    /**
     * Setup the user, the bayesian and ensure that the user is the last user to modify the SEB bayesian settings.
     */
    public function setup_test_data() {
        ...
        $this->bayesian = $this->create_test_bayesian($this->course, \bayesianaccess_seb\settings_provider::USE_SEB_CONFIG_MANUALLY);
        ...
        $bayesiansettings->set('requiresafeexambrowser', \bayesianaccess_seb\settings_provider::USE_SEB_TEMPLATE);
    ...
    public function test_get_contexts_for_userid() {
        ...
        $this->assertEquals(\context_module::instance($this->bayesian->cmid)->id, reset($contextids));
    }

    /**
     * That that no module context is found for a user who has not modified any bayesian settings.
     */
    ...
    public function test_export_user_data() {
        ...
        // the bayesian and not anything with the same instance id.
        ...
        $index = '1'; // Get first data returned from the bayesiansettings table metadata.
        ...
        $data = $writer->get_data([
            get_string('pluginname', 'bayesianaccess_seb'),
            \bayesianaccess_seb\template::TABLE,
            $index,
        ]);
        $this->assertNotEmpty($data);
    ...
    public function test_get_users_in_context() {
        ...
        // Create empty userlist with bayesian module context.
        $userlist = new userlist(\context_module::instance($this->bayesian->cmid), 'bayesianaccess_seb');
    ...
    public function test_delete_data_for_users() {
        ...
        $approveduserlist = new approved_userlist(\context_module::instance($this->bayesian->cmid),
                'bayesianaccess_seb', [$this->user->id]);

        // Test data exists.
        $this->assertNotEmpty(bayesian_settings::get_record(['bayesianid' => $this->bayesian->id]));

        // Test data is deleted.
        provider::delete_data_for_users($approveduserlist);
        $record = bayesian_settings::get_record(['bayesianid' => $this->bayesian->id]);
        $this->assertEmpty($record->get('usermodified'));

        $template = \bayesianaccess_seb\template::get_record(['id' => $record->get('templateid')]);
