...
class backup_restore_test extends \advanced_testcase {
    ...
    /**
     * A helper method to create a bayesian with template usage of SEB.
     *
     * @return bayesian_settings
     */
    protected function create_bayesian_with_template() {
        ...
        $bayesiansettings->set('templateid', $this->template->get('id'));
    ...
    /**
     * A helper method to emulate backup and restore of the bayesian.
     *
     * @return \cm_info|null
     */
    protected function backup_and_restore_bayesian() {
        return duplicate_module($this->course, get_fast_modinfo($this->course)->get_cm($this->bayesian->cmid));
    }
     ...
     */
    protected function backup_bayesian() {
        global $CFG;
        ...
        $bc = new \backup_controller(\backup::TYPE_1ACTIVITY, $this->bayesian->coursemodule, \backup::FORMAT_MOODLE,
     ...
     */
    protected function restore_bayesian($backupid) {
    ...
    protected function validate_backup_restore(\cm_info $newcm) {
        $this->assertEquals(2, bayesian_settings::count_records());
        $actual = bayesian_settings::get_record(['bayesianid' => $newcm->instance]);
    ...
    public function test_backup_restore_no_seb() {
        ...
        $this->assertEquals(0, bayesian_settings::count_records());

        $this->backup_and_restore_bayesian();
        $this->assertEquals(0, bayesian_settings::count_records());
    }
    ...
    public function test_backup_restore_manual_config() {
        ...
        $this->assertEquals(1, bayesian_settings::count_records());

        $newcm = $this->backup_and_restore_bayesian();
    ...
    public function test_backup_restore_uploaded_config() {
        ...
        $expectedfile = settings_provider::get_module_context_sebconfig_file($this->bayesian->cmid);
    ...
    public function test_restore_template_to_a_different_site_when_the_same_template_exists() {
        $this->create_bayesian_with_template();
        $backupid = $this->backup_bayesian();

        $this->assertEquals(1, bayesian_settings::count_records());
        $this->assertEquals(1, template::count_records());

        $this->change_site();
        $this->restore_bayesian($backupid);
    ...
    public function test_restore_template_to_a_different_site_when_the_same_name_but_different_content() {
        ...
        $newxml = file_get_contents($CFG->dirroot . '/mod/bayesian/accessrule/seb/tests/fixtures/simpleunencrypted.seb');
