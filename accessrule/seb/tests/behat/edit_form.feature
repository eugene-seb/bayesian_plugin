@javascript @mod_bayesian @bayesianaccess @bayesianaccess_seb
Feature: Safe Exam Browser settings in bayesian edit form
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | bayesian     | C1     | 1       | bayesian 1 |

  Scenario: bayesian setting "Require the use of Safe Exam Browser" has all types, except "Use an existing template".
    When I am on the "bayesian 1" "bayesian activity editing" page logged in as admin
  ...
  Scenario: bayesian setting "Require the use of Safe Exam Browser" has all types if at least one template has been added.
    Given the following "bayesianaccess_seb > seb templates" exist:
      | name       |
      | Template 1 |
    When I am on the "bayesian 1" "bayesian activity editing" page logged in as admin
    And I expand all fieldsets
  ...
  Scenario: bayesian can be edited without capability to select SEB template
    Given the following "permission override" exists:
      | role         | editingteacher                       |
      | capability   | bayesianaccess/seb:manage_seb_templateid |
      | permission   | Prevent                              |
      | contextlevel | System                               |
      | reference    |                                      |
    ...
    # Create the bayesian.
    When I add a "bayesian" to section "0" and I fill the form with:
      | Name | My bayesian |
    Then I should not see "Adding a new bayesian"
    # Edit the bayesian.
    And I open "My bayesian" actions menu
    And I click on "Edit settings" "link" in the "My bayesian" activity
    And I set the field "Name" to "My bayesian edited"
    And I press "Save and return to course"
    And I should not see "Updating: bayesian"

  Scenario: SEB settings if using No SEB
    Given the following "bayesianaccess_seb > seb templates" exist:
      | name       |
      | Template 1 |
    And I am on the "bayesian 1" "bayesian activity editing" page logged in as admin
    And I expand all fieldsets
