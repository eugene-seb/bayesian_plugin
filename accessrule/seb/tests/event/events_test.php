...
namespace bayesianaccess_seb\event;
...
class events_test extends \advanced_testcase {
     ...
     *
     * @covers \bayesianaccess_seb\event\access_prevented
     */
    public function test_event_access_prevented() {
        $this->resetAfterTest();

        $this->setAdminUser();
        $bayesian = $this->create_test_bayesian($this->course, \bayesianaccess_seb\settings_provider::USE_SEB_CONFIG_MANUALLY);
        $accessmanager = new \bayesianaccess_seb\access_manager(new bayesian($bayesian,
            get_coursemodule_from_id('bayesian', $bayesian->cmid), $this->course));
        ...
        $event = \bayesianaccess_seb\event\access_prevented::create_strict($accessmanager, 'Because I said so.');
        ...
        $this->assertEquals('bayesian access was prevented', $event->get_name());
        $this->assertEquals(
            "The user with id '$user->id' has been prevented from accessing bayesian with id '$bayesian->id' by the "
        ...
        $this->assertEquals($bayesian->id, $event->objectid);
     ...
     *
     * @covers \bayesianaccess_seb\event\template_created
     */
    public function test_event_create_template() {
        ...
        $event = \bayesianaccess_seb\event\template_created::create_strict(
            $template,
        ...
        // Test that the event data is as expected.
        $this->assertInstanceOf('\bayesianaccess_seb\event\template_created', $event);
