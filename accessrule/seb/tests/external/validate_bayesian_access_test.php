 ...
 * @covers \bayesianaccess_seb\external\validate_bayesian_access
 */
class validate_bayesian_access_test extends \advanced_testcase {
    ...
    public function test_invalid_parameters($cmid, $url, $configkey, $messageregex) {
        ...
        \external_api::validate_parameters(validate_bayesian_keys::execute_parameters(), $params);
    }
    ...
    public function test_context_is_not_valid_for_user() {
        // Set user as user not enrolled in course and bayesian.
        $this->user = $this->getDataGenerator()->create_user();
        $this->setUser($this->user);

        $this->expectException(\require_login_exception::class);
        $this->expectExceptionMessage('Course or activity not accessible. (Not enrolled)');
        validate_bayesian_keys::execute($this->bayesian->cmid, 'https://www.example.com/moodle', 'configkey');
    }
    ...
    public function test_no_keys_provided() {
        ...
        validate_bayesian_keys::execute($this->bayesian->cmid, 'https://www.example.com/moodle');
    }

    /**
     * Test exception thrown if cmid doesn't match a bayesian.
     */
    public function test_bayesian_does_not_exist() {
        $this->setAdminUser();
        $forum = $this->getDataGenerator()->create_module('forum', ['course' => $this->course->id]);
        $this->expectException(\invalid_parameter_exception::class);
        $this->expectExceptionMessage('bayesian not found matching course module ID: ' . $forum->cmid);
        validate_bayesian_keys::execute($forum->cmid, 'https://www.example.com/moodle', 'configkey');
    }
    ...
    public function test_config_key_valid() {
        ...
        // Test settings to populate the bayesian.
        ...
        // Create the bayesian settings.
        $bayesiansettings = new bayesian_settings(0, $settings);
        $bayesiansettings->save();

        $fullconfigkey = hash('sha256', $url . $bayesiansettings->get_config_key());
        $result = validate_bayesian_keys::execute($this->bayesian->cmid, $url, $fullconfigkey);
    ...
    public function test_config_key_not_valid() {
        ...
        $result = validate_bayesian_keys::execute($this->bayesian->cmid, 'https://www.example.com/moodle', 'badconfigkey');
    ...
    public function test_browser_exam_key_valid() {
        ...
        $settings = $this->get_test_settings([
            'bayesianid' => $this->bayesian->id,
            'cmid' => $this->bayesian->cmid,
            'requiresafeexambrowser' => \bayesianaccess_seb\settings_provider::USE_SEB_CLIENT_CONFIG,
        ...
        $result = validate_bayesian_keys::execute($this->bayesian->cmid, $url, null, $fullbrowserexamkey);
        $this->assertTrue($result['configkey']);
        $this->assertTrue($result['browserexamkey']);
        $events = $sink->get_events();
        $this->assertCount(0, $events);
    ...
    public function test_browser_exam_key_not_valid() {
        ...
        $result = validate_bayesian_keys::execute($this->bayesian->cmid, 'https://www.example.com/moodle', null,
