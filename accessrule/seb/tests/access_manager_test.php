 ...
 * @covers \bayesianaccess_seb\access_manager
 */
class access_manager_test extends \advanced_testcase {
    ...
    /**
     * Test access_manager private property bayesiansettings is null.
     */
    public function test_access_manager_bayesiansettings_null() {
        ...
        $reflection = new \ReflectionClass('\bayesianaccess_seb\access_manager');
        $property = $reflection->getProperty('bayesiansettings');
    ...
    /**
     * Test that the bayesian Config Key matches the incoming request header.
     */
    public function test_access_keys_validate_with_config_key() {
        ...
        $configkey = bayesian_settings::get_record(['bayesianid' => $this->bayesian->id])->get_config_key();
    ...
    /**
     * Test that the bayesian Config Key matches a provided config key with no incoming request header.
     */
    ...
    /**
     * Test that the bayesian Config Key does not match the incoming request header.
     */
    ...
    /**
     * Test that if there are no browser exam keys for bayesian, check is skipped.
     */
    ...
    /**
     * Test can correctly get type of SEB usage for the bayesian.
     */
    ...
    public function test_access_manager_uses_cached_config_key() {
        ...
        $this->assertNotEquals($bayesiansettings->get_config_key(), $configkey);
        $this->assertTrue($accessmanager->validate_config_key());
    ...
    /**
     * Check that valid SEB config key is null if bayesian doesn't have SEB settings.
     */
    public function test_valid_config_key_is_null_if_no_settings() {
        ...
        $this->assertEmpty(bayesian_settings::get_record(['bayesianid' => $this->bayesian->id]));
    ...
    public function test_set_session_access() {
        ...
        $this->assertTrue($SESSION->bayesianaccess_seb_access[$this->bayesian->cmid]);
    }
    ...
    public function test_session_access_set_for_specific_course_module() {
        ...
        $this->bayesian = $this->create_test_bayesian($this->course, settings_provider::USE_SEB_CLIENT_CONFIG);
        $bayesian2 = $this->create_test_bayesian($this->course, settings_provider::USE_SEB_CLIENT_CONFIG);
        $accessmanager = $this->get_access_manager();

        $accessmanager->set_session_access(true);

        $this->assertCount(1, $SESSION->bayesianaccess_seb_access);
        $this->assertTrue($SESSION->bayesianaccess_seb_access[$this->bayesian->cmid]);
        $this->assertTrue(empty($SESSION->bayesianaccess_seb_access[$bayesian2->cmid]));
    }
    ...
    public function test_should_redirect_to_seb_config_link() {
        ...
        set_config('autoreconfigureseb', '1', 'bayesianaccess_seb');
        $_SERVER['HTTP_USER_AGENT'] = 'SEB';
