...
class settings_provider_test extends \advanced_testcase {
    ...
    /**
     * Mocked bayesian form instance.
     * @var \mod_bayesian_mod_form
     */
    protected $mockedbayesianform;
    ...
    protected function set_up_form_mocks() {
        ...
        $this->mockedbayesianform = $this->createMock('mod_bayesian_mod_form');
        $this->mockedbayesianform->method('get_context')->willReturn($this->context);
        $this->mockedbayesianform->method('get_instance')->willReturn($this->bayesian->id);
    ...
    /**
     * Test that settings types to be added to bayesian settings, are part of bayesian_settings persistent class.
     */
    public function test_setting_elements_are_part_of_bayesian_settings_table() {
        $dbsettings = (array) (new bayesian_settings())->to_record();
        ...
        // Get all elements to be added to form, that are not in the persistent bayesian_settings class.
    ...
    public function test_hideifs() {
        $settinghideifs = settings_provider::get_bayesian_hideifs();
    ...
    public function test_build_setting_capability_name_incorrect_setting() {
        ...
        $this->expectExceptionMessage('Incorrect SEB bayesian setting broken');
    ...
    public function test_build_setting_capability_name_correct_setting() {
        foreach (settings_provider::get_seb_config_elements() as $name => $type) {
            $expected = 'bayesianaccess/seb:manage_' . $name;
    ...
    public function test_get_requiresafeexambrowser_options($settingcapability) {
        ...
        assign_capability('bayesianaccess/seb:manage_seb_templateid', CAP_ALLOW, $this->roleid, $this->context->id);
        $options = settings_provider::get_requiresafeexambrowser_options($this->context);
        $this->assertCount(4, $options);
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_CONFIG_MANUALLY, $options));
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_TEMPLATE, $options));
        ...
        assign_capability('bayesianaccess/seb:manage_filemanager_sebconfigfile', CAP_ALLOW, $this->roleid, $this->context->id);
        $options = settings_provider::get_requiresafeexambrowser_options($this->context);
        $this->assertCount(5, $options);
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_CONFIG_MANUALLY, $options));
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_TEMPLATE, $options));
    ...
    public function test_get_requiresafeexambrowser_options_with_conflicting_permissions() {
        ...
        $settings = bayesian_settings::get_record(['bayesianid' => $this->bayesian->id]);
    ...
    public function test_form_elements_are_frozen_if_conflicting_permissions() {
        ...
        assign_capability('bayesianaccess/seb:manage_seb_requiresafeexambrowser', CAP_ALLOW, $this->roleid, $this->context->id);
        assign_capability('bayesianaccess/seb:manage_seb_showsebdownloadlink', CAP_ALLOW, $this->roleid, $this->context->id);
        assign_capability('bayesianaccess/seb:manage_seb_allowedbrowserexamkeys', CAP_ALLOW, $this->roleid, $this->context->id);

        $this->set_up_form_mocks();

        settings_provider::add_seb_settings_fields($this->mockedbayesianform, $this->mockedform);
    ...
    /**
     * Test that All settings are frozen if bayesian was attempted and use seb with manual settings.
     */
    public function test_form_elements_are_locked_when_bayesian_attempted_manual() {
        $this->resetAfterTest();
        $this->course = $this->getDataGenerator()->create_course();
    ...
    /**
     * Test that All settings are frozen if a bayesian was attempted and use template.
     */
    public function test_form_elements_are_locked_when_bayesian_attempted_template() {
        $this->resetAfterTest();
        $this->setAdminUser();
        $this->course = $this->getDataGenerator()->create_course();
    ...
    public function test_validate_draftarea_configfile_failure() {
        ...
        $itemid = $this->create_test_draftarea_file($xml);
        $errors = settings_provider::validate_draftarea_configfile($itemid);
        $this->assertEquals($errors, new \lang_string('fileparsefailed', 'bayesianaccess_seb'));
    }
    ...
    /**
     * Test saving files from the user draft area into the bayesian context area storage.
     */
    public function test_save_filemanager_sebconfigfile_draftarea() {
        ...
        settings_provider::save_filemanager_sebconfigfile_draftarea($draftitemid, $this->bayesian->cmid);

        $fs = get_file_storage();
        $files = $fs->get_area_files($this->context->id, 'bayesianaccess_seb', 'filemanager_sebconfigfile');

        $this->assertCount(2, $files);
    }

    /**
     * Test deleting the $this->bayesian->cmid itemid from the file area.
     */
    public function test_delete_uploaded_config_file() {
        ...
        settings_provider::delete_uploaded_config_file($this->bayesian->cmid);
    ...
    public function test_get_module_context_sebconfig_file() {
        ...
        $file = settings_provider::get_module_context_sebconfig_file($this->bayesian->cmid);
    ...
    public function test_is_seb_settings_locked() {
        ...
        $this->assertFalse(settings_provider::is_seb_settings_locked($this->bayesian->id));

        $this->attempt_bayesian($this->bayesian, $user);
        $this->assertTrue(settings_provider::is_seb_settings_locked($this->bayesian->id));
    }
