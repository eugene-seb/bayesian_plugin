...
/**
 * Class for providing bayesian settings, to make setting up bayesian form manageable.
 *
...
/**
 * Helper class for providing bayesian settings, to make setting up bayesian form manageable.
 *
...
class settings_provider {
     ...
     */
    protected static function insert_element(\mod_bayesian_mod_form $bayesianform,
     ...
     */
    protected static function remove_element(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform, string  $elementname) {
     ...
     * @param string $elementname Element name.
     */
    protected static function add_help_button(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform, string $elementname) {
        if ($mform->elementExists($elementname)) {
            $mform->addHelpButton($elementname, $elementname, 'bayesianaccess_seb');
        }
    }
     ...
     */
    protected static function set_default(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform, string  $elementname, $value) {
     ...
     */
    protected static function set_type(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform, string $elementname, string $type) {
     ...
     * @param string $elementname Element name.
     */
    protected static function freeze_element(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform, string $elementname) {
        if ($mform->elementExists($elementname)) {
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_header_element(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) {
        global  $OUTPUT;

        $element = $mform->createElement('header', 'seb', get_string('seb', 'bayesianaccess_seb'));
        self::insert_element($bayesianform, $mform, $element);

        // Display notification about locked settings.
        if (self::is_seb_settings_locked($bayesianform->get_instance())) {
            $notify = new \core\output\notification(
                get_string('settingsfrozen', 'bayesianaccess_seb'),
                \core\output\notification::NOTIFY_WARNING
            );

            $notifyelement = $mform->createElement('html', $OUTPUT->render($notify));
            self::insert_element($bayesianform, $mform, $notifyelement);
        }

        if (self::is_conflicting_permissions($bayesianform->get_context())) {
            $notify = new \core\output\notification(
                get_string('conflictingsettings', 'bayesianaccess_seb'),
                \core\output\notification::NOTIFY_WARNING
            );
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_usage_options(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) {
        $element = $mform->createElement(
            'select',
            'seb_requiresafeexambrowser',
            get_string('seb_requiresafeexambrowser', 'bayesianaccess_seb'),
            self::get_requiresafeexambrowser_options($bayesianform->get_context())
        );

        self::insert_element($bayesianform, $mform, $element);
        self::set_type($bayesianform, $mform, 'seb_requiresafeexambrowser', PARAM_INT);
        self::set_default($bayesianform, $mform, 'seb_requiresafeexambrowser', self::USE_SEB_NO);
        self::add_help_button($bayesianform, $mform, 'seb_requiresafeexambrowser');

        if (self::is_conflicting_permissions($bayesianform->get_context())) {
            self::freeze_element($bayesianform, $mform, 'seb_requiresafeexambrowser');
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_templates(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) {
        if (self::can_use_seb_template($bayesianform->get_context()) || self::is_conflicting_permissions($bayesianform->get_context())) {
            $element = $mform->createElement(
                'select',
                'seb_templateid',
                get_string('seb_templateid', 'bayesianaccess_seb'),
        ...
        self::insert_element($bayesianform, $mform, $element);
        self::set_type($bayesianform, $mform, 'seb_templateid', PARAM_INT);
        self::set_default($bayesianform, $mform, 'seb_templateid', 0);
        self::add_help_button($bayesianform, $mform, 'seb_templateid');

        // In case if the user can't use templates, but the bayesian is configured to use them,
        // we'd like to display template, but freeze it.
        if (self::is_conflicting_permissions($bayesianform->get_context())) {
            self::freeze_element($bayesianform, $mform, 'seb_templateid');
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_config_file(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) {
        $itemid = 0;

        $draftitemid = 0;
        file_prepare_draft_area(
            $draftitemid,
            $bayesianform->get_context()->id,
            'bayesianaccess_seb',
            'filemanager_sebconfigfile',
            $itemid
        );

        if (self::can_upload_seb_file($bayesianform->get_context())) {
            $element = $mform->createElement(
                'filemanager',
                'filemanager_sebconfigfile',
                get_string('filemanager_sebconfigfile', 'bayesianaccess_seb'),
                null,
        ...
        self::insert_element($bayesianform, $mform, $element);
        self::set_type($bayesianform, $mform, 'filemanager_sebconfigfile', PARAM_RAW);
        self::set_default($bayesianform, $mform, 'filemanager_sebconfigfile', $draftitemid);
        self::add_help_button($bayesianform, $mform, 'filemanager_sebconfigfile');
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_show_download_link(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) {
        if (self::can_change_seb_showsebdownloadlink($bayesianform->get_context())) {
            $element = $mform->createElement('selectyesno',
                'seb_showsebdownloadlink',
                get_string('seb_showsebdownloadlink', 'bayesianaccess_seb')
            );
            self::insert_element($bayesianform, $mform, $element);
            self::set_type($bayesianform, $mform, 'seb_showsebdownloadlink', PARAM_BOOL);
            self::set_default($bayesianform, $mform, 'seb_showsebdownloadlink', 1);
            self::add_help_button($bayesianform, $mform, 'seb_showsebdownloadlink');
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_allowedbrowserexamkeys(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) {
        if (self::can_change_seb_allowedbrowserexamkeys($bayesianform->get_context())) {
            $element = $mform->createElement('textarea',
                'seb_allowedbrowserexamkeys',
                get_string('seb_allowedbrowserexamkeys', 'bayesianaccess_seb')
            );
            self::insert_element($bayesianform, $mform, $element);
            self::set_type($bayesianform, $mform, 'seb_allowedbrowserexamkeys', PARAM_RAW);
            self::set_default($bayesianform, $mform, 'seb_allowedbrowserexamkeys', '');
            self::add_help_button($bayesianform, $mform, 'seb_allowedbrowserexamkeys');
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_config_elements(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) {
        ...
        foreach (self::get_seb_config_elements() as $name => $type) {
            if (!self::can_manage_seb_config_setting($name, $bayesianform->get_context())) {
                $type = 'hidden';
            }

            $element = $mform->createElement($type, $name, get_string($name, 'bayesianaccess_seb'));
            self::insert_element($bayesianform, $mform, $element);
            unset($element); // We need to make sure each &element only references the current element in loop.

            self::add_help_button($bayesianform, $mform, $name);

            if (isset($defaults[$name])) {
                self::set_default($bayesianform, $mform, $name, $defaults[$name]);
            }

            if (isset($types[$name])) {
                self::set_type($bayesianform, $mform, $name, $types[$name]);
            }
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_seb_settings_fields(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) {
        if (self::can_configure_seb($bayesianform->get_context())) {
            self::add_seb_header_element($bayesianform, $mform);
            self::add_seb_usage_options($bayesianform, $mform);
            self::add_seb_templates($bayesianform, $mform);
            self::add_seb_config_file($bayesianform, $mform);
            self::add_seb_show_download_link($bayesianform, $mform);
            self::add_seb_config_elements($bayesianform, $mform);
            self::add_seb_allowedbrowserexamkeys($bayesianform, $mform);
            self::hide_seb_elements($bayesianform, $mform);
            self::lock_seb_elements($bayesianform, $mform);
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function hide_seb_elements(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) {
        foreach (self::get_bayesian_hideifs() as $elname => $rules) {
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function lock_seb_elements(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) {
        if (self::is_seb_settings_locked($bayesianform->get_instance()) || self::is_conflicting_permissions($bayesianform->get_context())) {
            // Freeze common bayesian settings.
            self::freeze_element($bayesianform, $mform, 'seb_requiresafeexambrowser');
            self::freeze_element($bayesianform, $mform, 'seb_templateid');
            self::freeze_element($bayesianform, $mform, 'seb_showsebdownloadlink');
            self::freeze_element($bayesianform, $mform, 'seb_allowedbrowserexamkeys');

            $bayesiansettings = bayesian_settings::get_by_bayesian_id((int) $bayesianform->get_instance());

            // If the file has been uploaded, then replace it with the link to download the file.
            if (!empty($bayesiansettings) && $bayesiansettings->get('requiresafeexambrowser') == self::USE_SEB_UPLOAD_CONFIG) {
                self::remove_element($bayesianform, $mform, 'filemanager_sebconfigfile');
                if ($link = self::get_uploaded_seb_file_download_link($bayesianform, $mform)) {
                    $element = $mform->createElement(
                        'static',
                        'filemanager_sebconfigfile',
                        get_string('filemanager_sebconfigfile', 'bayesianaccess_seb'),
                    ...
                    self::insert_element($bayesianform, $mform, $element, 'seb_showsebdownloadlink');
                }
            }

            // Remove template ID if not using template for this bayesian.
            if (empty($bayesiansettings) || $bayesiansettings->get('requiresafeexambrowser') != self::USE_SEB_TEMPLATE) {
                $mform->removeElement('seb_templateid');
            }

            // Freeze all SEB specific settings.
            foreach (self::get_seb_config_elements() as $element => $type) {
                self::freeze_element($bayesianform, $mform, $element);
            }
        }
    }
     ...
     * @return string
     */
    protected static function get_uploaded_seb_file_download_link(\mod_bayesian_mod_form $bayesianform, \MoodleQuickForm $mform) : string {
        $link = '';
        $file = self::get_module_context_sebconfig_file($bayesianform->get_coursemodule()->id);
        ...
        if ($file) {
            ...
            $link = \html_writer::link($url, get_string('downloadsebconfig', 'bayesianaccess_seb'));
        }
    ...
    /**
     * Get the type of element for each of the form elements in bayesian settings.
     *
     * Contains all setting elements. Array key is name of 'form element'/'database column (excluding prefix)'.
     *
     * @return array All bayesian form elements to be added and their types.
     */
    ...
    /**
     * Get the types of the bayesian settings elements.
     ...
     * use template (but still can update other settings) edit the same bayesian. This is
    ...
    public static function is_conflicting_permissions(\context $context) {
        ...
        $settings = bayesian_settings::get_record(['cmid' => (int) $context->instanceid]);
    ...
    public static function get_requiresafeexambrowser_options(\context $context) : array {
        ...
        if (self::can_configure_manually($context) || self::is_conflicting_permissions($context)) {
            $options[self::USE_SEB_CONFIG_MANUALLY] = get_string('seb_use_manually', 'bayesianaccess_seb');
        }

        if (self::can_use_seb_template($context) || self::is_conflicting_permissions($context)) {
            if (!empty(self::get_template_options())) {
                $options[self::USE_SEB_TEMPLATE] = get_string('seb_use_template', 'bayesianaccess_seb');
            }
        }

        if (self::can_upload_seb_file($context) || self::is_conflicting_permissions($context)) {
            $options[self::USE_SEB_UPLOAD_CONFIG] = get_string('seb_use_upload', 'bayesianaccess_seb');
        }

        $options[self::USE_SEB_CLIENT_CONFIG] = get_string('seb_use_client', 'bayesianaccess_seb');
    ...
    /**
     * Get the default values of the bayesian settings.
     *
    ...
    public static function validate_draftarea_configfile($itemid) {
        ...
        if ($file) {
            if (!helper::is_valid_seb_config($file->get_content())) {
                return new lang_string('fileparsefailed', 'bayesianaccess_seb');
            }
        }

        // If we require an SEB config uploaded, and the file does not exist, error.
        if (!$file) {
            return new lang_string('filenotpresent', 'bayesianaccess_seb');
        }
    }
    ...
    public static function get_module_context_sebconfig_file(string $cmid) : ?stored_file {
        ...
        if (!$files = $fs->get_area_files($context->id, 'bayesianaccess_seb', 'filemanager_sebconfigfile', 0,
            'id DESC', false)) {
            return null;
        }
     ...
     * @param string $cmid The cmid of for the bayesian.
     * @return bool Always true
     */
    public static function save_filemanager_sebconfigfile_draftarea(string $draftitemid, string $cmid) : bool {
        if ($draftitemid) {
            $context = context_module::instance($cmid);
            file_save_draft_area_files($draftitemid, $context->id, 'bayesianaccess_seb', 'filemanager_sebconfigfile',
                0, []);
        }
    ...
    public static function can_configure_seb(\context $context) : bool {
        return has_capability('bayesianaccess/seb:manage_seb_requiresafeexambrowser', $context);
    }
    ...
    public static function can_use_seb_template(\context $context) : bool {
        return has_capability('bayesianaccess/seb:manage_seb_templateid', $context);
    }
    ...
    public static function can_upload_seb_file(\context $context) : bool {
        return has_capability('bayesianaccess/seb:manage_filemanager_sebconfigfile', $context);
    }
    ...
    public static function can_change_seb_showsebdownloadlink(\context $context) : bool {
        return has_capability('bayesianaccess/seb:manage_seb_showsebdownloadlink', $context);
    }
    ...
    public static function can_change_seb_allowedbrowserexamkeys(\context $context) : bool {
        return has_capability('bayesianaccess/seb:manage_seb_allowedbrowserexamkeys', $context);
    }
     ...
     */
    public static function get_bayesian_hideifs() : array {
        $hideifs = [];
    ...
    public static function build_setting_capability_name(string $settingname) : string {
        if (!key_exists($settingname, self::get_seb_config_elements())) {
            throw new \coding_exception('Incorrect SEB bayesian setting ' . $settingname);
        }

        return 'bayesianaccess/seb:manage_' . $settingname;
    }

    /**
     * Check if settings is locked.
     *
     * @param int $bayesianid bayesian ID.
     * @return bool
     */
    public static function is_seb_settings_locked($bayesianid) : bool {
        if (empty($bayesianid)) {
            return false;
        }

        return bayesian_has_attempts($bayesianid);
    }

    /**
     * Filter a standard class by prefix.
     *
     * @param stdClass $settings bayesian settings object.
    ...
    /**
     * Filter bayesian settings for this plugin only.
     *
     * @param stdClass $settings bayesian settings.
