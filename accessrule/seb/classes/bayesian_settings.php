...
/**
 * Entity model representing bayesian settings for the seb plugin.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_settings extends persistent {

    /** Table name for the persistent. */
    const TABLE = 'bayesianaccess_seb_settings';
    ...
    /**
     * Return an instance by bayesian id.
     *
     * This method gets data from cache before doing any DB calls.
     *
     * @param int $bayesianid bayesian id.
     * @return false|\bayesianaccess_seb\bayesian_settings
     */
    public static function get_by_bayesian_id(int $bayesianid) {
        if ($data = self::get_bayesian_settings_cache()->get($bayesianid)) {
            return new static(0, $data);
        }

        return self::get_record(['bayesianid' => $bayesianid]);
    }

    /**
     * Return cached SEB config represented as a string by bayesian ID.
     *
     * @param int $bayesianid bayesian id.
     * @return string|null
     */
    public static function get_config_by_bayesian_id(int $bayesianid) : ?string {
        $config = self::get_config_cache()->get($bayesianid);
        ...
        $config = null;
        if ($settings = self::get_by_bayesian_id($bayesianid)) {
            $config = $settings->get_config();
            self::get_config_cache()->set($bayesianid, $config);
        }
    ...
    /**
     * Return cached SEB config key by bayesian ID.
     *
     * @param int $bayesianid bayesian id.
     * @return string|null
     */
    public static function get_config_key_by_bayesian_id(int $bayesianid) : ?string {
        $configkey = self::get_config_key_cache()->get($bayesianid);
        ...
        if ($settings = self::get_by_bayesian_id($bayesianid)) {
            ...
            self::get_config_key_cache()->set($bayesianid, $configkey);
        }
    ...
    private static function get_config_key_cache() : \cache_application {
        return \cache::make('bayesianaccess_seb', 'configkey');
    }
    ...
    private static function get_config_cache() : \cache_application {
        return \cache::make('bayesianaccess_seb', 'config');
    }

    /**
     * Return bayesian settings cache object,
     *
     * @return \cache_application
     */
    private static function get_bayesian_settings_cache() : \cache_application {
        return \cache::make('bayesianaccess_seb', 'bayesiansettings');
    }
    ...
    private function after_save() {
        self::get_bayesian_settings_cache()->set($this->get('bayesianid'), $this->to_record());
        self::get_config_cache()->set($this->get('bayesianid'), $this->config);
        self::get_config_key_cache()->set($this->get('bayesianid'), $this->configkey);
    }
    ...
    protected function before_delete() {
        $key = $this->get('bayesianid');
        self::get_bayesian_settings_cache()->delete($key);
    ...
    protected function validate_allowedbrowserexamkeys($keys) {
        ...
        foreach ($keys as $i => $key) {
            if (!preg_match('~^[a-f0-9]{64}$~', $key)) {
                return new lang_string('allowedbrowserkeyssyntax', 'bayesianaccess_seb');
            }
        }
        if (count($keys) != count(array_unique($keys))) {
            return new lang_string('allowedbrowserkeysdistinct', 'bayesianaccess_seb');
        }
        return true;
    }
    ...
    /**
     * Create or update the config string based on the current bayesian settings.
     */
    ...
    /**
     * Case for USE_SEB_UPLOAD_CONFIG. This creates a plist from an uploaded configuration file, then applies the bayesian
    ...
    private function process_seb_upload_config() {
        ...
        if (empty($file)) {
            throw new moodle_exception('noconfigfilefound', 'bayesianaccess_seb', '', $this->get('cmid'));
        } else {
    ...
    private function process_required_enforced_settings() {
        ...
        $bayesianurl = new moodle_url($CFG->wwwroot . "/mod/bayesian/view.php", ['id' => $this->get('cmid')]);
        $this->plist->set_or_update_value('startURL', new CFString($bayesianurl->out(true)));
    ...
    /**
     * Sets the quitURL if found in the bayesian_settings.
     */
    ...
    /**
     * Sets the bayesian_setting's linkquitseb if a quitURL value was found in a template or uploaded config.
     */
