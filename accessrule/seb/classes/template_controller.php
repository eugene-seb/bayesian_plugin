...
use core\notification;
use bayesianaccess_seb\local\table\template_list;
...
class template_controller {
    ...
    protected function set_external_page() {
        admin_externalpage_setup('bayesianaccess_seb/template');
    }
     ...
     *
     * @return \bayesianaccess_seb\template
     */
    ...
    protected function get_create_button_text() : string {
        return get_string('addtemplate', 'bayesianaccess_seb');
    }

    /**
     * Returns form for the record.
     *
     * @param \bayesianaccess_seb\template|null $instance
     *
     * @return \bayesianaccess_seb\local\form\template
     */
    protected function get_form($instance) : \bayesianaccess_seb\local\form\template {
        global $PAGE;

        return new \bayesianaccess_seb\local\form\template($PAGE->url->out(false), ['persistent' => $instance]);
    }
    ...
    protected function get_view_heading() : string {
        return get_string('managetemplates', 'bayesianaccess_seb');
    }
    ...
    protected function get_new_heading() : string {
        return get_string('newtemplate', 'bayesianaccess_seb');
    }
    ...
    protected function get_edit_heading() : string {
        return get_string('edittemplate', 'bayesianaccess_seb');
    }
    ...
    public static function get_base_url() : string {
        return '/mod/bayesian/accessrule/seb/template.php';
    }
        ...
        if ($form->is_cancelled()) {
            ...
            try {
                if (empty($data->id)) {
                    ...
                    \bayesianaccess_seb\event\template_created::create_strict(
                        $persistent,
                    ...
                    \bayesianaccess_seb\event\template_updated::create_strict(
                        $instance,
                        \context_system::instance()
                    )->trigger();
            ...
            if (empty($instance)) {
                ...
                if (!$instance->can_delete()) {
                    notification::warning(get_string('cantedit', 'bayesianaccess_seb'));
                }
    ...
    protected function delete($id) {
        ...
        if ($instance->can_delete()) {
            ...
            \bayesianaccess_seb\event\template_deleted::create_strict(
                $id,
                \context_system::instance()
            )->trigger();
            ...
            notification::warning(get_string('cantdelete', 'bayesianaccess_seb'));
            redirect(new \moodle_url(static::get_base_url()));
        }
    }
    ...
    protected function view() {
        ...
        $PAGE->requires->js_call_amd('bayesianaccess_seb/managetemplates', 'setup');
    ...
    private function trigger_enabled_event(template $template) {
        ...
        $func = '\bayesianaccess_seb\event\template_' . $eventstring;
        $func::create_strict(
            $template,
            \context_system::instance()
        )->trigger();
