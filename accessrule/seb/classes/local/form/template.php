...
namespace bayesianaccess_seb\local\form;
...
class template extends \core\form\persistent {
    ...
    protected static $persistentclass = 'bayesianaccess_seb\\template';
    ...
    protected function definition() {
        ...
        $mform->addElement('text', 'name', get_string('name', 'bayesianaccess_seb'));
        $mform->addRule('name', get_string('required'), 'required', null, 'client');
        $mform->setType('name', PARAM_TEXT);

        $mform->addElement('textarea', 'description', get_string('description', 'bayesianaccess_seb'));
        $mform->setType('description', PARAM_TEXT);

        if ($this->get_persistent()->get('id')) {
            $mform->addElement('textarea', 'content', get_string('content', 'bayesianaccess_seb'), ['rows' => 20, 'cols' => 60]);
            $mform->addRule('content', get_string('required'), 'required');
        } else {
            $mform->addElement('filepicker', 'content', get_string('content', 'bayesianaccess_seb'));
            $mform->addRule('content', get_string('required'), 'required');
        }

        $mform->addElement('selectyesno', 'enabled', get_string('enabled', 'bayesianaccess_seb'));
    ...
    protected function extra_validation($data, $files, array &$errors) {
        ...
        // Check name.
        if (empty($data->name)) {
            $newerrors['name'] = get_string('namerequired', 'bayesianaccess_seb');
        }
