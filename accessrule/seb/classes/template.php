...
class template extends persistent {
    ...
    const TABLE = 'bayesianaccess_seb_template';
    ...
    protected function validate_content(string $content) {
        if (helper::is_valid_seb_config($content)) {
            ...
            return new \lang_string('invalidtemplate', 'bayesianaccess_seb');
        }
    }
    ...
    public function can_delete() : bool {
        ...
        if ($this->get('id')) {
            $settings = bayesian_settings::get_records(['templateid' => $this->get('id')]);
