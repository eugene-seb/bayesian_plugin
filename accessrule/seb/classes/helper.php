...
class helper {
    ...
    /**
     * Get seb config content for a particular bayesian. This method checks caps.
     *
     * @param string $cmid The course module ID for a bayesian with config.
    ...
    public static function get_seb_config_content(string $cmid) : string {
        ...
        $cm = get_coursemodule_from_id('bayesian', $cmid, 0, false, MUST_EXIST);

        // Make sure the user is logged in and has access to the module.
        require_login($cm->course, false, $cm);

        // Retrieve the config for bayesian.
        $config = bayesian_settings::get_config_by_bayesian_id($cm->instance);
        if (empty($config)) {
            throw new \moodle_exception('noconfigfound', 'bayesianaccess_seb', '', $cm->id);
        }
        return $config;
    }
