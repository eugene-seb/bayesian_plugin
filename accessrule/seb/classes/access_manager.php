...
/**
 * Manage the access to the bayesian.
 *
...
class access_manager {
    ...
    /** @var bayesian $bayesian A bayesian object containing all information pertaining to current bayesian. */
    private $bayesian;

    /** @var bayesian_settings $bayesiansettings A bayesian settings persistent object containing plugin settings */
    private $bayesiansettings;

    /** @var context_module $context Context of this bayesian activity. */
    private $context;
     ...
     *
     * @param bayesian $bayesian The details of the bayesian.
     */
    public function __construct(bayesian $bayesian) {
        $this->bayesian = $bayesian;
        $this->context = context_module::instance($bayesian->get_cmid());
        $this->bayesiansettings = bayesian_settings::get_by_bayesian_id($bayesian->get_bayesianid());
        $this->validconfigkey = bayesian_settings::get_config_key_by_bayesian_id($bayesian->get_bayesianid());
    }
    ...
    public function validate_browser_exam_key(?string $browserexamkey = null, ?string $url = null): bool {
        ...
        $validbrowserexamkeys = $this->bayesiansettings->get('allowedbrowserexamkeys');
    ...
    /**
     * Check if Safe Exam Browser is required to access bayesian.
     * If bayesiansettings do not exist, then there is no requirement for using SEB.
     *
     * @return bool If required.
     */
    public function seb_required() : bool {
        if (!$this->bayesiansettings) {
            return false;
        } else {
    ...
    /**
     * This is the basic check for the Safe Exam Browser previously used in the bayesianaccess_safebrowser plugin that
    ...
    public function can_bypass_seb(): bool {
        return has_capability('bayesianaccess/seb:bypassseb', $this->context);
    }
    ...
    /**
     * Getter for the bayesian object.
     *
     * @return bayesian
     */
    public function get_bayesian() : bayesian {
        return $this->bayesian;
    }

    /**
     * Check that at least one browser exam key exists in the bayesian settings.
     *
     * @return bool True if one or more keys are set in bayesian settings.
     */
    private function is_allowed_browser_examkeys_configured(): bool {
        return !empty($this->bayesiansettings->get('allowedbrowserexamkeys'));
    }
    ...
    /**
     * Get type of SEB usage for the bayesian.
     *
     * @return int
     */
    public function get_seb_use_type(): int {
        if (empty($this->bayesiansettings)) {
            return settings_provider::USE_SEB_NO;
        } else {
            return $this->bayesiansettings->get('requiresafeexambrowser');
        }
    }
    ...
    /**
     * Set session access for bayesian.
     *
     * @param bool $accessallowed
     */
    public function set_session_access(bool $accessallowed): void {
        global $SESSION;
        if (!isset($SESSION->bayesianaccess_seb_access)) {
            $SESSION->bayesianaccess_seb_access = [];
        }
        $SESSION->bayesianaccess_seb_access[$this->bayesian->get_cmid()] = $accessallowed;
    }

    /**
     * Check session access for bayesian if already set.
     *
     * @return bool
     */
    public function validate_session_access(): bool {
        global $SESSION;
        return !empty($SESSION->bayesianaccess_seb_access[$this->bayesian->get_cmid()]);
    }

    /**
     * Unset the global session access variable for this bayesian.
     */
    public function clear_session_access(): void {
        global $SESSION;
        unset($SESSION->bayesianaccess_seb_access[$this->bayesian->get_cmid()]);
    }
    ...
    public function redirect_to_seb_config_link(): void {
        ...
        $seblink = \bayesianaccess_seb\link_generator::get_link($this->bayesian->get_cmid(), true, is_https());
    ...
    public function should_redirect_to_seb_config_link(): bool {
        ...
        return $this->is_using_seb()
                && get_config('bayesianaccess_seb', 'autoreconfigureseb')
                && $haskeyinheader;
    }
}
