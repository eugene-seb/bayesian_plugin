...
namespace bayesianaccess_ipaddress;

use bayesian;
use bayesianaccess_ipaddress;

defined('MOODLE_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/mod/bayesian/accessrule/ipaddress/rule.php');


/**
 * Unit tests for the bayesianaccess_ipaddress plugin.
 *
 * @package    bayesianaccess_ipaddress
 * @category   test
...
class rule_test extends \basic_testcase {
    public function test_ipaddress_access_rule() {
        ...
        $bayesian->subnet = getremoteaddr(null);
        if (!empty($bayesian->subnet)) {
            $bayesianobj = new bayesian($bayesian, $cm, null);
            $rule = new bayesianaccess_ipaddress($bayesianobj, 0);
