...
namespace bayesianaccess_delaybetweenattempts;

use bayesian;
use bayesianaccess_delaybetweenattempts;

defined('MOODLE_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/mod/bayesian/accessrule/delaybetweenattempts/rule.php');


/**
 * Unit tests for the bayesianaccess_delaybetweenattempts plugin.
 *
 * @package    bayesianaccess_delaybetweenattempts
 * @category   test
...
class rule_test extends \basic_testcase {
    public function test_just_first_delay() {
        ...
        $bayesian->attempts = 3;
        $bayesian->timelimit = 0;
        $bayesian->delay1 = 1000;
        $bayesian->delay2 = 0;
        ...
        $rule = new bayesianaccess_delaybetweenattempts($bayesianobj, 10000);
        ...
        $this->assertEquals($rule->prevent_new_attempt(1, $attempt),
            get_string('youmustwait', 'bayesianaccess_delaybetweenattempts', userdate(11000)));
        ...
        $this->assertEquals($rule->prevent_new_attempt(1, $attempt),
            get_string('youmustwait', 'bayesianaccess_delaybetweenattempts', userdate(10001)));
        $this->assertFalse($rule->prevent_new_attempt(2, $attempt));
    }
    ...
    public function test_just_both_delays() {
        ...
        $this->assertFalse($rule->prevent_new_attempt(5, $attempt));
        $this->assertEquals($rule->prevent_new_attempt(1, $attempt),
            get_string('youmustwait', 'bayesianaccess_delaybetweenattempts', userdate(12000)));
        $this->assertEquals($rule->prevent_new_attempt(2, $attempt),
            get_string('youmustwait', 'bayesianaccess_delaybetweenattempts', userdate(11000)));
        $this->assertEquals($rule->prevent_new_attempt(3, $attempt),
            get_string('youmustwait', 'bayesianaccess_delaybetweenattempts', userdate(11000)));
        ...
        $attempt->timefinish = 9001;
        $this->assertEquals($rule->prevent_new_attempt(1, $attempt),
            get_string('youmustwait', 'bayesianaccess_delaybetweenattempts', userdate(11001)));
        $this->assertEquals($rule->prevent_new_attempt(2, $attempt),
            get_string('youmustwait', 'bayesianaccess_delaybetweenattempts', userdate(10001)));
        $this->assertEquals($rule->prevent_new_attempt(4, $attempt),
            get_string('youmustwait', 'bayesianaccess_delaybetweenattempts', userdate(10001)));
    ...
    public function test_with_close_date() {
        ...
        $attempt->timefinish = 13000;
        $this->assertEquals($rule->prevent_new_attempt(1, $attempt),
            get_string('youmustwait', 'bayesianaccess_delaybetweenattempts', userdate(15000)));
        $attempt->timefinish = 13001;
        $this->assertEquals($rule->prevent_new_attempt(1, $attempt),
            get_string('youcannotwait', 'bayesianaccess_delaybetweenattempts'));
        ...
        $rule = new bayesianaccess_delaybetweenattempts($bayesianobj, 15000);
        $attempt->timefinish = 13000;
        $this->assertFalse($rule->prevent_new_attempt(1, $attempt));
        $attempt->timefinish = 13001;
        $this->assertEquals($rule->prevent_new_attempt(1, $attempt),
        ...
        $rule = new bayesianaccess_delaybetweenattempts($bayesianobj, 15001);
        $attempt->timefinish = 13000;
        $this->assertFalse($rule->prevent_new_attempt(1, $attempt));
        $attempt->timefinish = 13001;
        $this->assertFalse($rule->prevent_new_attempt(1, $attempt));
