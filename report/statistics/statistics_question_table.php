...
/**
 * bayesian statistics report, table for showing response analysis for a particular question (or sub question).
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_statistics_question_table extends flexible_table {
    ...
    public function __construct($qid) {
        parent::__construct('mod-bayesian-report-statistics-question-table' . $qid);
    }
    ...
    public function question_setup($reporturl, $questiondata, $s, $responseanalysis) {
        ...
        $this->set_attribute('class', 'generaltable generalbox boxaligncenter bayesianresponseanalysis');
        ...
        if ($responseanalysis->has_subparts()) {
            $columns[] = 'part';
            $headers[] = get_string('partofquestion', 'bayesian_statistics');
        }

        if ($responseanalysis->has_multiple_response_classes()) {
            $columns[] = 'responseclass';
            $headers[] = get_string('modelresponse', 'bayesian_statistics');

            if ($responseanalysis->has_actual_responses()) {
                $columns[] = 'response';
                $headers[] = get_string('actualresponse', 'bayesian_statistics');
            }

        } else {
            $columns[] = 'response';
            $headers[] = get_string('response', 'bayesian_statistics');
        }

        $columns[] = 'fraction';
        $headers[] = get_string('optiongrade', 'bayesian_statistics');

        if (!$responseanalysis->has_multiple_tries_data()) {
            $columns[] = 'totalcount';
            $headers[] = get_string('count', 'bayesian_statistics');
        } else {
            $countcolumns = range(1, $responseanalysis->get_maximum_tries());
            foreach ($countcolumns as $countcolumn) {
                $columns[] = 'trycount'.$countcolumn;
                $headers[] = get_string('counttryno', 'bayesian_statistics', $countcolumn);
            }
        }

        $columns[] = 'frequency';
        $headers[] = get_string('frequency', 'bayesian_statistics');
