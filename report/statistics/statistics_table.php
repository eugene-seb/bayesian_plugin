...
/**
 * bayesian statistics report, table for showing statistics of each question in the bayesian.
 *
...
/**
 * This table has one row for each question in the bayesian, with sub-rows when
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_statistics_table extends flexible_table {
    /** @var object the bayesian settings. */
    protected $bayesian;

    /** @var integer the bayesian course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-bayesian-report-statistics-report');
    }
     ...
     * @param int $cmid the bayesian course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($bayesian, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'bayesian_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'bayesian_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'bayesian');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'bayesian_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'bayesian_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'bayesian_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'bayesian_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'bayesian_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'bayesian_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'bayesian_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'bayesian_statistics');
    ...
    protected function col_actions($questionstat) {
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            return bayesian_question_action_icons($this->bayesian, $this->cmid,
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'bayesian_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'bayesian_statistics')));
                ...
                if ($this->is_calculated_question_summary($questionstat)) {
                    ...
                    // in the UI.
                    $name = html_writer::div(get_string('rangeofvalues', 'bayesian_statistics'));
                    ...
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('detailedanalysis', 'bayesian_statistics')));
                }
            }
        }
        ...
        if ($this->is_calculated_question_summary($questionstat)) {
            $name .= html_writer::link($url, get_string('viewanalysis', 'bayesian_statistics'));
        } else if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'bayesian_statistics') . '<br />' . $name;
        }
     ...
     * of maximum mark for the bayesian. That is, the indended influence this question
    ...
    protected function col_intended_weight($questionstat) {
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            if (is_null($min) && is_null($max)) {
                ...
                $min = bayesian_report_scale_summarks_as_percentage($min, $this->bayesian);
                $max = bayesian_report_scale_summarks_as_percentage($max, $this->bayesian);
                return $this->format_range($min, $max);
            }
        } else {
            return bayesian_report_scale_summarks_as_percentage($questionstat->maxmark, $this->bayesian);
        }
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            if (is_null($min) && is_null($max)) {
                ...
                if ($negcovar) {
                    $min = get_string('negcovar', 'bayesian_statistics');
                }
            ...
            $negcovar = get_string('negcovar', 'bayesian_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'bayesian_statistics'),
     ...
     * other questions in this bayesian.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
    ...
    protected function format_range(string $min = null, string $max = null) {
        if (is_null($min) && is_null($max)) {
            ...
            return get_string('rangebetween', 'bayesian_statistics', $a);
        }
    }
