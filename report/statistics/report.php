...
/**
 * bayesian statistics report class.
 *
...
require_once($CFG->dirroot . '/mod/bayesian/report/statistics/statistics_form.php');
require_once($CFG->dirroot . '/mod/bayesian/report/statistics/statistics_table.php');
require_once($CFG->dirroot . '/mod/bayesian/report/statistics/statistics_question_table.php');
require_once($CFG->dirroot . '/mod/bayesian/report/statistics/statisticslib.php');

/**
 * The bayesian statistics report provides summary information about each question in
 * a bayesian, compared to the whole bayesian. It also provides a drill-down to more
 * detailed information about each question.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_statistics_report extends bayesian_default_report {

    /** @var context_module context of this bayesian.*/
    protected $context;

    /** @var bayesian_statistics_table instance of table class used for main questions stats table. */
    protected $table;
     ...
     * Display the report.
     */
    public function display($bayesian, $cm, $course) {
        global $OUTPUT, $DB;
        ...
        if (!bayesian_has_questions($bayesian->id)) {
            $this->print_header_and_tabs($cm, $course, $bayesian, 'statistics');
        ...
        $whichattempts = optional_param('whichattempts', $bayesian->grademethod, PARAM_INT);
        ...
        $reporturl = new moodle_url('/mod/bayesian/report.php', $pageoptions);

        $mform = new bayesian_statistics_settings_form($reporturl, compact('bayesian'));

        $mform->set_data(array('whichattempts' => $whichattempts, 'whichtries' => $whichtries));

        if ($whichattempts != $bayesian->grademethod) {
        ...
        if (empty($currentgroup)) {
            ...
            // All users who can attempt bayesianzes and who are in the currently selected group.
            $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                    array('mod/bayesian:reviewmyattempts', 'mod/bayesian:attempt'), $currentgroup);
        ...
        $qubaids = bayesian_statistics_qubaids_condition($bayesian->id, $groupstudentsjoins, $whichattempts);
        ...
        $this->table = new bayesian_statistics_table();
        if ($everything) {
            $report = get_string('completestatsfilename', 'bayesian_statistics');
        } else {
            $report = get_string('questionstatsfilename', 'bayesian_statistics');
        }
        $courseshortname = format_string($course->shortname, true,
                array('context' => context_course::instance($course->id)));
        $filename = bayesian_report_download_filename($report, $courseshortname, $bayesian->name);
        $this->table->is_downloading($download, $filename,
                get_string('bayesianstructureanalysis', 'bayesian_statistics'));
        $questions = $this->load_and_initialise_questions_for_calculations($bayesian);
        ...
        if (!$nostudentsingroup) {
            ...
            list($bayesianstats, $questionstats) =
                $this->get_all_stats_and_analysis($bayesian, $whichattempts, $whichtries, $groupstudentsjoins, $questions, $progress);
        } else {
            // Or create empty stats containers.
            $bayesianstats = new \bayesian_statistics\calculated($whichattempts);
            $questionstats = new \core_question\statistics\questions\all_calculated_for_qubaid_condition();
        }

        // Set up the table.
        $this->table->statistics_setup($bayesian, $cm->id, $reporturl, $bayesianstats->s());
        ...
        if (!$this->table->is_downloading()) {
            ...
            if (groups_get_activity_groupmode($cm)) {
                ...
                if ($currentgroup && $nostudentsingroup) {
                    $OUTPUT->notification(get_string('nostudentsingroup', 'bayesian_statistics'));
                }
            }

            if (!$this->table->is_downloading() && $bayesianstats->s() == 0) {
                echo $OUTPUT->notification(get_string('nogradedattempts', 'bayesian_statistics'));
            }
        ...
        if ($everything) { // Implies is downloading.
            ...
            $bayesianinfo = $bayesianstats->get_formatted_bayesian_info_data($course, $cm, $bayesian);
            $this->download_bayesian_info_table($bayesianinfo);

            if ($bayesianstats->s()) {
                $this->output_bayesian_structure_analysis_table($questionstats);

                if ($this->table->is_downloading() == 'html' && $bayesianstats->s() != 0) {
                    $this->output_statistics_graph($bayesian->id, $qubaids);
                }
            ...
            $this->output_individual_question_data($bayesian, $questionstats->for_subq($qid, $variantno));
            ...
            echo $OUTPUT->box('<a href="' . $reporturl->out() . '">' .
                              get_string('backtobayesianreport', 'bayesian_statistics') . '</a>',
            ...
            if ($variantno === null &&
                ...
                if (!$this->table->is_downloading()) {
                    ...
                    echo $OUTPUT->heading(get_string('slotstructureanalysis', 'bayesian_statistics', $number), 3);
                }
                ...
                $this->output_individual_question_data($bayesian, $questionstats->for_slot($slot, $variantno));
            ...
            echo $OUTPUT->heading(get_string('bayesianinformation', 'bayesian_statistics'), 3);
            echo $this->output_caching_info($bayesianstats->timemodified, $bayesian->id, $groupstudentsjoins, $whichattempts, $reporturl);
            echo $this->everything_download_options($reporturl);
            $bayesianinfo = $bayesianstats->get_formatted_bayesian_info_data($course, $cm, $bayesian);
            echo $this->output_bayesian_info_table($bayesianinfo);
            if ($bayesianstats->s()) {
                echo $OUTPUT->heading(get_string('bayesianstructureanalysis', 'bayesian_statistics'), 3);
                $this->output_bayesian_structure_analysis_table($questionstats);
                $this->output_statistics_graph($bayesian, $qubaids);
            }
        }
     ...
     */
    protected function output_individual_question_data($bayesian, $questionstat) {
        global $OUTPUT;

        // On-screen display. Show a summary of the question's place in the bayesian,
        ...
        $questioninfotable->data[] = array(get_string('modulename', 'bayesian'), $bayesian->name);
        $questioninfotable->data[] = array(get_string('questionname', 'bayesian_statistics'),
                $questionstat->question->name.'&nbsp;'.$datumfromtable['actions']);

        if ($questionstat->variant !== null) {
            $questioninfotable->data[] = array(get_string('variant', 'bayesian_statistics'), $questionstat->variant);

        }
        $questioninfotable->data[] = array(get_string('questiontype', 'bayesian_statistics'),
                $datumfromtable['icon'] . '&nbsp;' .
                question_bank::get_qtype($questionstat->question->qtype, false)->menu_name() . '&nbsp;' .
                $datumfromtable['icon']);
        $questioninfotable->data[] = array(get_string('positions', 'bayesian_statistics'),
        ...
        $labels = array(
            's' => get_string('attempts', 'bayesian_statistics'),
            'facility' => get_string('facility', 'bayesian_statistics'),
            'sd' => get_string('standarddeviationq', 'bayesian_statistics'),
            'random_guess_score' => get_string('random_guess_score', 'bayesian_statistics'),
            'intended_weight' => get_string('intended_weight', 'bayesian_statistics'),
            'effective_weight' => get_string('effective_weight', 'bayesian_statistics'),
            'discrimination_index' => get_string('discrimination_index', 'bayesian_statistics'),
            'discriminative_efficiency' =>
                                get_string('discriminative_efficiency', 'bayesian_statistics')
        );
        ...
        echo $OUTPUT->heading(get_string('questioninformation', 'bayesian_statistics'), 3);
        echo html_writer::table($questioninfotable);
        echo $this->render_question_text($questionstat->question);
        echo $OUTPUT->heading(get_string('questionstatistics', 'bayesian_statistics'), 3);
    ...
    protected function render_question_text($question) {
        ...
        $text = question_rewrite_question_preview_urls($question->questiontext, $question->id,
                ...
                $this->context->id, 'bayesian_statistics');
    ...
    protected function output_individual_question_response_analysis($question, $variantno, $s, $reporturl, $qubaids,
        ...
        $qtable = new bayesian_statistics_question_table($question->id);
        $exportclass = $this->table->export_class_instance();
        $qtable->export_class_instance($exportclass);
        if (!$this->table->is_downloading()) {
            // Output an appropriate title.
            echo $OUTPUT->heading(get_string('analysisofresponses', 'bayesian_statistics'), 3);
            ...
            if (!empty($question->number) && !is_null($variantno)) {
                $questiontabletitle = get_string('analysisnovariant', 'bayesian_statistics', $a);
            } else if (!empty($question->number)) {
                $questiontabletitle = get_string('analysisno', 'bayesian_statistics', $a);
            } else if (!is_null($variantno)) {
                $questiontabletitle = get_string('analysisvariant', 'bayesian_statistics', $a);
            } else {
                $questiontabletitle = get_string('analysisnameonly', 'bayesian_statistics', $a);
            }

            if ($this->table->is_downloading() == 'html') {
                $questiontabletitle = get_string('analysisofresponsesfor', 'bayesian_statistics', $questiontabletitle);
            }
    ...
    /**
     * Output the table that lists all the questions in the bayesian with their statistics.
     *
     ...
     *                                                                                               the bayesian including subqs and
     *                                                                                               variants.
     */
    protected function output_bayesian_structure_analysis_table($questionstats) {
            ...
            if (is_null($structureanalysis)) {
                ...
                foreach ($structureanalysis as $row) {
                    ...
                    if ($row instanceof \core_question\statistics\questions\calculated_question_summary) {
                        ...
                        $bgcssclass = 'bayesian_statistics-summaryrow';
    ...
    /**
     * Return HTML for table of overall bayesian statistics.
     *
     * @param array $bayesianinfo as returned by {@link get_formatted_bayesian_info_data()}.
     * @return string the HTML.
     */
    protected function output_bayesian_info_table($bayesianinfo) {

        $bayesianinfotable = new html_table();
        $bayesianinfotable->align = array('center', 'center');
        $bayesianinfotable->width = '60%';
        $bayesianinfotable->attributes['class'] = 'generaltable titlesleft';
        $bayesianinfotable->data = array();

        foreach ($bayesianinfo as $heading => $value) {
             $bayesianinfotable->data[] = array($heading, $value);
        }

        return html_writer::table($bayesianinfotable);
    }

    /**
     * Download the table of overall bayesian statistics.
     *
     * @param array $bayesianinfo as returned by {@link get_formatted_bayesian_info_data()}.
     */
    protected function download_bayesian_info_table($bayesianinfo) {
        global $OUTPUT;
        ...
        // Do the output.
        $exportclass = $this->table->export_class_instance();
        $exportclass->start_table(get_string('bayesianinformation', 'bayesian_statistics'));
     ...
     *
     * @param int|object $bayesianorid The bayesian, or its ID.
     ...
     */
    protected function output_statistics_graph($bayesianorid, $qubaids) {
        global $DB, $PAGE;

        $bayesian = $bayesianorid;
        if (!is_object($bayesian)) {
            $bayesian = $DB->get_record('bayesian', array('id' => $bayesianorid), '*', MUST_EXIST);
        }

        // Load the rest of the required data.
        $questions = bayesian_report_get_significant_questions($bayesian);
        ...
        $fieldstoplot = [
            'facility' => get_string('facility', 'bayesian_statistics'),
            'discriminativeefficiency' => get_string('discriminative_efficiency', 'bayesian_statistics')
        ];
        ...
        $chart->get_xaxis(0, true)->set_label(get_string('position', 'bayesian_statistics'));
        ...
        $graphname = get_string('statisticsreportgraph', 'bayesian_statistics');
        echo $output->chart($chart, $graphname);
    }

    /**
     * Get the bayesian and question statistics, either by loading the cached results,
     ...
     * @return array with 2 elements:    - $bayesianstats The statistics for overall attempt scores.
    ...
    public function get_all_stats_and_analysis(
            $bayesian, $whichattempts, $whichtries, \core\dml\sql_join $groupstudentsjoins, $questions, $progress = null) {
        ...
        $bayesiancalc = new \bayesian_statistics\calculator($progress);

        $progress->start_progress('', 3);
        if ($bayesiancalc->get_last_calculated_time($qubaids) === false) {

            // Recalculate now.
            $questionstats = $qcalc->calculate($qubaids);
            $progress->progress(1);

            $bayesianstats = $bayesiancalc->calculate($bayesian->id, $whichattempts, $groupstudentsjoins, count($questions),
                                              $qcalc->get_sum_of_mark_variance());
            $progress->progress(2);
        } else {
            $bayesianstats = $bayesiancalc->get_cached($qubaids);
            $progress->progress(1);
        ...
        return array($bayesianstats, $questionstats);
    }
    ...
    protected function get_progress_trace_instance() {
        if ($this->progress === null) {
            if (!$this->table->is_downloading()) {
                $this->progress = new \core\progress\display_if_slow(get_string('calculatingallstats', 'bayesian_statistics'));
    ...
    /**
     * Analyse responses for all questions and sub questions in this bayesian.
     *
    ...
    /**
     * Return a little form for the user to request to download the full report, including bayesian stats and response analysis for
    ...
    protected function everything_download_options(moodle_url $reporturl) {
        global $OUTPUT;
        return $OUTPUT->download_dataformat_selector(get_string('downloadeverything', 'bayesian_statistics'),
     ...
     * @return string HTML.
     */
    protected function output_caching_info($lastcachetime, $bayesianid, $groupstudentsjoins, $whichattempts, $reporturl) {
        global $DB, $OUTPUT;
        ...
        list($fromqa, $whereqa, $qaparams) = bayesian_statistics_attempts_sql($bayesianid, $groupstudentsjoins, $whichattempts, true);
        $count = $DB->count_records_sql("
                SELECT COUNT(1)
                FROM $fromqa
                WHERE $whereqa
                AND bayesiana.timefinish > {$lastcachetime}", $qaparams);
        ...
        $output .= get_string('lastcalculated', 'bayesian_statistics', $a);
        $output .= $OUTPUT->single_button($recalcualteurl,
                get_string('recalculatenow', 'bayesian_statistics'));
    ...
    public function clear_cached_data($qubaids) {
        global $DB;
        $DB->delete_records('bayesian_statistics', array('hashcode' => $qubaids->get_hash_code()));
    ...
    /**
     * Load the questions in this bayesian and add some properties to the objects needed in the reports.
     *
     * @param object $bayesian the bayesian.
     * @return array of questions for this bayesian.
     */
    public function load_and_initialise_questions_for_calculations($bayesian) {
        // Load the questions.
        $questions = bayesian_report_get_significant_questions($bayesian);
        $questionids = [];
        $randomquestions = [];
        foreach ($questions as $qs => $question) {
            if ($question->qtype === 'random') {
                $question->id = 0;
                $question->name = get_string('random', 'bayesian');
                $question->questiontext = get_string('random', 'bayesian');
    ...
    /**
     * Load question stats for a bayesian
     *
     * @param int $bayesianid question usage
     * @return all_calculated_for_qubaid_condition question stats
     */
    public function calculate_questions_stats_for_question_bank(int $bayesianid): all_calculated_for_qubaid_condition {
        global $DB;
        $bayesian = $DB->get_record('bayesian', ['id' => $bayesianid], '*', MUST_EXIST);
        $questions = $this->load_and_initialise_questions_for_calculations($bayesian);

        [, $questionstats] = $this->get_all_stats_and_analysis($bayesian,
            $bayesian->grademethod, question_attempt::ALL_TRIES, new \core\dml\sql_join(), $questions);
