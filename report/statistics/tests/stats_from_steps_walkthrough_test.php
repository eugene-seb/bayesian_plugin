...
use question_bank;
use question_finder;
use bayesian_statistics_report;
 ...
 *
 * The bayesian stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-bayesian-tools/tree/master/statsspreadsheet
 *
 ...
 */
class stats_from_steps_walkthrough_test extends \mod_bayesian\attempt_walkthrough_from_csv_test {

    /**
     * @var bayesian_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    public function test_walkthrough_from_csv($bayesiansettings, $csvdata) {
        ...
        $whichattempts = bayesian_GRADEAVERAGE; // All attempts.
        $whichtries = question_attempt::ALL_TRIES;
        $groupstudentsjoins = new \core\dml\sql_join();
        list($questions, $bayesianstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($bayesiansettings['testnumber'] === '00') {
            $this->check_variants_count_for_bayesian_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_bayesian_stats_for_bayesian_00($bayesianstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_bayesian_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $bayesianstats
     */
    protected function check_bayesian_stats_for_bayesian_00($bayesianstats) {
        $bayesianstatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($bayesianstatsexpected as $statname => $statvalue) {
            $this->assertEqualsWithDelta($statvalue, $bayesianstats->$statname, abs($statvalue) * 1.5e-5, $bayesianstats->$statname);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $bayesianstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new bayesian_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->bayesian);
        list($bayesianstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->bayesian,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = bayesian_statistics_qubaids_condition($this->bayesian->id, $groupstudentsjoins, $whichattempts);

        // We will create some bayesian and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $bayesiancalc = new calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($bayesiancalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $bayesianstats, $questionstats, $qubaids);
        }
        return array($questions, $bayesianstats, $questionstats, $qubaids);
    }
