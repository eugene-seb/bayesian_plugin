<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

namespace bayesian_statistics\task;

use bayesian_attempt;
use bayesian;
use bayesian_statistics_report;

defined('MOODLE_INTERNAL') || die();

require_once($CFG->dirroot . '/mod/bayesian/locallib.php');
require_once($CFG->dirroot . '/mod/bayesian/report/statistics/statisticslib.php');
require_once($CFG->dirroot . '/mod/bayesian/report/reportlib.php');
require_once($CFG->dirroot . '/mod/bayesian/report/statistics/report.php');

/**
 * Re-calculate question statistics.
 *
 * @package    bayesian_statistics
 * @copyright  2022 Catalyst IT Australia Pty Ltd
 * @author     Nathan Nguyen <nathannguyen@catalyst-au.net>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class recalculate extends \core\task\scheduled_task {
    /** @var int the maximum length of time one instance of this task will run. */
    const TIME_LIMIT = 3600;

    public function get_name(): string {
        return get_string('recalculatetask', 'bayesian_statistics');
    }

    public function execute(): void {
        global $DB;
        $stoptime = time() + self::TIME_LIMIT;
        $dateformat = get_string('strftimedatetimeshortaccurate', 'core_langconfig');

        // TODO: MDL-75197, add bayesianid in bayesian_statistics so that it is simpler to find bayesianzes for stats calculation.
        // Only calculate stats for bayesianzes which have recently finished attempt.
        $sql = "
            SELECT q.id AS bayesianid,
                   q.name AS bayesianname,
                   c.id AS courseid,
                   c.shortname AS courseshortname,
                   MAX(qa.timefinish) AS mostrecentattempttime,
                   COUNT(1) AS numberofattempts
              FROM {bayesian_attempts} qa
              JOIN {bayesian} q ON q.id = qa.bayesian
              JOIN {course} c ON c.id = q.course
             WHERE qa.preview = 0
               AND qa.state = :bayesianstatefinished
          GROUP BY q.id, q.name, c.id, c.shortname
        ";

        $params = [
            "bayesianstatefinished" => bayesian_attempt::FINISHED,
        ];

        $latestattempts = $DB->get_records_sql($sql, $params);

        $anyexception = null;
        foreach ($latestattempts as $attempt) {
            if (time() >= $stoptime) {
                mtrace("This task has been running for more than " .
                        format_time(self::TIME_LIMIT) . " so stopping this execution.");
                break;
            }

            mtrace("  Examining bayesian '$attempt->bayesianname' ($attempt->bayesianid) in course " .
                    "$attempt->courseshortname ($attempt->courseid) with most recent attempt at " .
                    userdate($attempt->mostrecentattempttime, $dateformat) . ".");
            $bayesianobj = bayesian::create($attempt->bayesianid);
            $bayesian = $bayesianobj->get_bayesian();
            // Hash code for question stats option in question bank.
            $qubaids = bayesian_statistics_qubaids_condition($bayesian->id, new \core\dml\sql_join(), $bayesian->grademethod);

            // Check if there is any existing question stats, and it has been calculated after latest bayesian attempt.
            $lateststatstime = $DB->get_field('bayesian_statistics', 'COALESCE(MAX(timemodified), 0)',
                    ['hashcode' => $qubaids->get_hash_code()]);

            if ($lateststatstime >= $attempt->mostrecentattempttime) {
                mtrace("    Statistics already calculated at " . userdate($lateststatstime, $dateformat) .
                        " so nothing to do for this bayesian.");
                continue;
            }

            mtrace("    Calculating statistics for $attempt->numberofattempts attempts, starting at " .
                    userdate(time(), $dateformat) . " ...");
            try {
                $report = new bayesian_statistics_report();
                $report->clear_cached_data($qubaids);
                $report->calculate_questions_stats_for_question_bank($bayesian->id);
                mtrace("    Calculations completed at " . userdate(time(), $dateformat) . ".");

            } catch (\Throwable $e) {
                // We don't want an exception from one bayesian to stop processing of other bayesianzes.
                mtrace_exception($e);
                $anyexception = $e;
            }
        }

        if ($anyexception) {
            // If there was any error, ensure the task fails.
            throw $anyexception;
        }
    }
}
