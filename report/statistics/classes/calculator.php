...
/**
 * Class to calculate and also manage caching of bayesian statistics.
 *
...
class calculator {
     ...
     * @return calculated $bayesianstats The statistics for overall attempt scores.
     */
    public function calculate($bayesianid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $bayesianstats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($bayesianid, $groupstudentsjoins);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $bayesianstats->{$propertyname} = $value;
        }
        ...
        if ($s != 0) {
            ...
            list($fromqa, $whereqa, $qaparams) =
                bayesian_statistics_attempts_sql($bayesianid, $groupstudentsjoins, $whichattempts);

            $bayesianstats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($bayesianstats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $bayesianstats->standarddeviation = sqrt($powers->power2 / ($s - 1));

                // Skewness.
                if ($s > 2) {
                    // See http://docs.moodle.org/dev/bayesian_item_analysis_calculations_in_practise#Skewness_and_Kurtosis.
                    $m2 = $powers->power2 / $s;
                    $m3 = $powers->power3 / $s;
                    $m4 = $powers->power4 / $s;
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $bayesianstats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $bayesianstats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $bayesianstats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $bayesianstats->errorratio = 100 * sqrt(1 - ($bayesianstats->cic / 100));
                            $bayesianstats->standarderror = $bayesianstats->errorratio *
                                $bayesianstats->standarddeviation / 100;
                        }
                    }
            ...
            $bayesianstats->cache(bayesian_statistics_qubaids_condition($bayesianid, $groupstudentsjoins, $whichattempts));
        }
    ...
    public function get_cached($qubaids) {
        ...
        $fromdb = $DB->get_record_select('bayesian_statistics', 'hashcode = ? AND timemodified > ?',
    ...
    public function get_last_calculated_time($qubaids) {
        ...
        $timemodified = time() - self::TIME_TO_CACHE;
        return $DB->get_field_select('bayesian_statistics', 'timemodified', 'hashcode = ? AND timemodified > ?',
                                         array($qubaids->get_hash_code(), $timemodified));
    }

    /**
     * Given a particular bayesian grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'bayesian_statistics');
    }

    /**
     * Given a particular bayesian grading method return a string id for use as a field name prefix in mdl_bayesian_statistics or to
     ...
     *
     * See : http://docs.moodle.org/dev/bayesian_item_analysis_calculations_in_practise
     ...
     */
    protected function attempt_counts_and_averages($bayesianid, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(bayesian_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = bayesian_statistics_attempts_sql($bayesianid, $groupstudentsjoins, $which);
     ...
     * Median mark.
     *
     * http://docs.moodle.org/dev/bayesian_statistics_calculations#Median_Score
     *
    ...
    protected function median($s, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT bayesiana.id, bayesiana.sumgrades
                  FROM $fromqa
                 WHERE $whereqa
              ORDER BY sumgrades";
     ...
     *
     * Explanation here : http://docs.moodle.org/dev/bayesian_item_analysis_calculations_in_practise
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((bayesiana.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((bayesiana.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((bayesiana.sumgrades - $mean), 4)) AS power4
                  FROM $fromqa
                 WHERE $whereqa";
