...
/**
 * Base class for the options that control what is visible in an {@link bayesian_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_bayesian_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the bayesian being reported on. */
    public $bayesian;

    /** @var object the course module objects for the bayesian being reported on. */
    public $cm;

    /** @var object the course settings for the course the bayesian is in. */
    public $course;

    /**
     * @var array form field name => corresponding bayesian_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => bayesian_attempt::IN_PROGRESS,
        'stateoverdue'    => bayesian_attempt::OVERDUE,
        'statefinished'   => bayesian_attempt::FINISHED,
        'stateabandoned'  => bayesian_attempt::ABANDONED,
    );

    /**
     * @var string bayesian_attempts_report::ALL_WITH or bayesian_attempts_report::ENROLLED_WITH
     *      bayesian_attempts_report::ENROLLED_WITHOUT or bayesian_attempts_report::ENROLLED_ALL
     */
    public $attempts = bayesian_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of bayesian_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(bayesian_attempt::IN_PROGRESS, bayesian_attempt::OVERDUE,
            bayesian_attempt::FINISHED, bayesian_attempt::ABANDONED);
    ...
    public $pagesize = bayesian_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $bayesian the settings for the bayesian being reported on.
     * @param object $cm the course module objects for the bayesian being reported on.
     * @param object $coures the course settings for the coures this bayesian is in.
     */
    public function __construct($mode, $bayesian, $cm, $course) {
        $this->mode   = $mode;
        $this->bayesian   = $bayesian;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = bayesian_report_should_show_grades($bayesian, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/bayesian/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('bayesian_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('bayesian_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == bayesian_attempts_report::ALL_WITH) {
                $this->attempts = bayesian_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = bayesian_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(bayesian_attempts_report::ALL_WITH, bayesian_attempts_report::ENROLLED_WITH,
                bayesian_attempts_report::ENROLLED_WITHOUT, bayesian_attempts_report::ENROLLED_ALL))) {
            $this->attempts = bayesian_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!bayesian_report_can_filter_only_graded($this->bayesian)) {
        ...
        if ($this->attempts == bayesian_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = bayesian_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(bayesian_attempt::FINISHED, $this->states);
    }
}
