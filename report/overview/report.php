...
/**
 * This file defines the bayesian overview report class.
 *
...
require_once($CFG->dirroot . '/mod/bayesian/report/overview/overview_options.php');
require_once($CFG->dirroot . '/mod/bayesian/report/overview/overview_form.php');
require_once($CFG->dirroot . '/mod/bayesian/report/overview/overview_table.php');


/**
 * bayesian report subclass for the overview (grades) report.
 *
 * @copyright 1999 onwards Martin Dougiamas and others {@link http://moodle.com}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_overview_report extends bayesian_attempts_report {
    ...
    public function display($bayesian, $cm, $course) {
        ...
        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
                'overview', 'bayesian_overview_settings_form', $bayesian, $cm, $course);

        $options = new bayesian_overview_options('overview', $bayesian, $cm, $course);
        ...
        $table = new bayesian_overview_table($bayesian, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
        $filename = bayesian_report_download_filename(get_string('overviewfilename', 'bayesian_overview'),
                $courseshortname, $bayesian->name);
        $table->is_downloading($options->download, $filename,
                $courseshortname . ' ' . format_string($bayesian->name, true));
        ...
        if ($options->attempts == self::ALL_WITH) {
            ...
            // groups mode, so setting allowed to empty (which means all bayesian attempts
            // are accessible, is not a security porblem.
            $allowedjoins = new \core\dml\sql_join();
        }

        $this->course = $course; // Hack to make this available in process_actions.
        $this->process_actions($bayesian, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $options->get_url());

        $hasquestions = bayesian_has_questions($bayesian->id);

        // Start output.
        if (!$table->is_downloading()) {
            // Only print headers if not asked to download data.
            $this->print_standard_header_and_messages($cm, $course, $bayesian,
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            if (!$table->is_downloading()) {
                ...
                if (has_capability('mod/bayesian:regrade', $this->context)) {
                    $regradesneeded = $this->count_question_attempts_needing_regrade(
                            $bayesian, $groupstudentsjoins);
                    if ($currentgroup) {
                        $a= new stdClass();
                        ...
                        $regradealldrydolabel =
                                get_string('regradealldrydogroup', 'bayesian_overview', $a);
                        $regradealldrylabel =
                                get_string('regradealldrygroup', 'bayesian_overview', $a);
                        $regradealllabel =
                                get_string('regradeallgroup', 'bayesian_overview', $a);
                    } else {
                        $regradealldrydolabel =
                                get_string('regradealldrydo', 'bayesian_overview', $regradesneeded);
                        $regradealldrylabel =
                                get_string('regradealldry', 'bayesian_overview');
                        $regradealllabel =
                                get_string('regradeall', 'bayesian_overview');
                    }
                ...
                if ($strattempthighlight = bayesian_report_highlighting_grading_method(
                        $bayesian, $this->qmsubselect, $options->onlygraded)) {
                    echo '<div class="bayesianattemptcounts">' . $strattempthighlight . '</div>';
                }
            }
            ...
            $this->add_grade_columns($bayesian, $options->usercanseegrades, $columns, $headers, false);

            if (!$table->is_downloading() && has_capability('mod/bayesian:regrade', $this->context) &&
                    $this->has_regraded_questions($table->sql->from, $table->sql->where, $table->sql->params)) {
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'bayesian_overview');
            }

            if ($options->slotmarks) {
                foreach ($questions as $slot => $question) {
                    $columns[] = 'qsgrade' . $slot;
                    $header = get_string('qbrief', 'bayesian', $question->number);
                    if (!$table->is_downloading()) {
                        $header .= '<br />';
                    } else {
                        $header .= ' ';
                    }
                    $header .= '/' . bayesian_rescale_grade($question->maxmark, $bayesian, 'question');
                    $headers[] = $header;
                }
            }
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            list($bands, $bandwidth) = self::get_bands_count_and_width($bayesian);
            $labels = self::get_bands_labels($bands, $bandwidth, $bayesian);
            ...
            if ($currentgroup && $this->hasgroupstudents) {
                $sql = "SELECT qg.id
                        ...
                        {$groupstudentsjoins->joins}
                          WHERE qg.bayesian = $bayesian->id AND {$groupstudentsjoins->wheres}";
                if ($DB->record_exists_sql($sql, $groupstudentsjoins->params)) {
                    $data = bayesian_report_grade_bands($bandwidth, $bands, $bayesian->id, $groupstudentsjoins);
                    $chart = self::get_chart($labels, $data);
                    $graphname = get_string('overviewreportgraphgroup', 'bayesian_overview', groups_get_group_name($currentgroup));
                    // Numerical range data should display in LTR even for RTL languages.
                    echo $output->chart($chart, $graphname, ['dir' => 'ltr']);
                }
            }

            if ($DB->record_exists('bayesian_grades', array('bayesian'=> $bayesian->id))) {
                $data = bayesian_report_grade_bands($bandwidth, $bands, $bayesian->id, new \core\dml\sql_join());
                $chart = self::get_chart($labels, $data);
                $graphname = get_string('overviewreportgraph', 'bayesian_overview');
                // Numerical range data should display in LTR even for RTL languages.
                echo $output->chart($chart, $graphname, ['dir' => 'ltr']);
            }
        }
    ...
    protected function process_actions($bayesian, $cm, $currentgroup, \core\dml\sql_join $groupstudentsjoins,
        ...
        parent::process_actions($bayesian, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $redirecturl);

        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('regrade', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    $this->start_regrade($bayesian, $cm);
                    $this->regrade_attempts($bayesian, false, $groupstudentsjoins, $attemptids);
        ...
        if (optional_param('regradeall', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($bayesian, $cm);
            $this->regrade_attempts($bayesian, false, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldry', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($bayesian, $cm);
            $this->regrade_attempts($bayesian, true, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldrydo', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($bayesian, $cm);
            $this->regrade_attempts_needing_it($bayesian, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);
        }
    }
     ...
     */
    protected function start_regrade($bayesian, $cm) {
        require_capability('mod/bayesian:regrade', $this->context);
        $this->print_header_and_tabs($cm, $this->course, $bayesian, $this->mode);
    }
    ...
    protected function finish_regrade($nexturl) {
        global $OUTPUT;
        \core\notification::success(get_string('regradecomplete', 'bayesian_overview'));
    ...
    /**
     * Regrade a particular bayesian attempt. Either for real ($dryrun = false), or
     * as a pretend regrade to see which fractions would change. The outcome is
     * stored in the bayesian_overview_regrades table.
     *
     ...
     *
     * @param object $attempt the bayesian attempt to regrade.
    ...
    public function regrade_attempt($attempt, $dryrun = false, $slots = null): array {
        global $DB;
        // Need more time for a bayesian with many questions.
        ...
        $messages = [];
        $finished = $attempt->state == bayesian_attempt::FINISHED;
        foreach ($slots as $slot) {
            ...
            if (abs($qqr->oldfraction - $qqr->newfraction) > 1e-7) {
                ...
                $DB->insert_record('bayesian_overview_regrades', $qqr, false);
            }
        }
    ...
    /**
     * Regrade attempts for this bayesian, exactly which attempts are regraded is
     ...
     */
    protected function regrade_attempts($bayesian, $dryrun = false,
            \core\dml\sql_join$groupstudentsjoins = null, $attemptids = array()) {
        global $DB;
        $this->unlock_session();

        $userfieldsapi = \core_user\fields::for_name();
        $sql = "SELECT bayesiana.*, " . $userfieldsapi->get_sql('u', false, '', '', false)->selects . "
                  FROM {bayesian_attempts} bayesiana
                  JOIN {user} u ON u.id = bayesiana.userid";
        $where = "bayesian = :qid AND preview = 0";
        $params = array('qid' => $bayesian->id);
        ...
        if ($attemptids) {
            ...
            $where .= " AND bayesiana.id $attemptidcondition";
        ...
        $this->regrade_batch_of_attempts($bayesian, $attempts, $dryrun, $groupstudentsjoins);
    }

    /**
     * Regrade those questions in those attempts that are marked as needing regrading
     * in the bayesian_overview_regrades table.
     * @param object $bayesian the bayesian settings.
     * @param \core\dml\sql_join $groupstudentsjoins empty for all attempts, otherwise regrade attempts
     * for these users.
     */
    protected function regrade_attempts_needing_it($bayesian, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        $this->unlock_session();

        $join = '{bayesian_overview_regrades} qqr ON qqr.questionusageid = bayesiana.uniqueid';
        $where = "bayesiana.bayesian = :qid AND bayesiana.preview = 0 AND qqr.regraded = 0";
        $params = array('qid' => $bayesian->id);
        ...
        $toregrade = $DB->get_recordset_sql("
                SELECT bayesiana.uniqueid, qqr.slot
                  FROM {bayesian_attempts} bayesiana
                  JOIN $join
                 WHERE $where", $params);
        ...
        $attempts = $DB->get_records_sql("
                SELECT bayesiana.*, " . $userfieldsapi->get_sql('u', false, '', '', false)->selects . "
                  FROM {bayesian_attempts} bayesiana
                  JOIN {user} u ON u.id = bayesiana.userid
                 WHERE bayesiana.uniqueid $uniqueidcondition
                ", $params);
        ...
        $this->regrade_batch_of_attempts($bayesian, $attempts, false, $groupstudentsjoins);
    }
     ...
     * Each object in the attempts array must be a row from the bayesian_attempts
     ...
     *
     * @param object $bayesian the bayesian settings.
     * @param array $attempts of data from the bayesian_attempts table, with extra data as above.
     * @param bool $dryrun if true, do a pretend regrade, otherwise do it for real.
     * @param \core\dml\sql_join $groupstudentsjoins empty for all attempts, otherwise regrade attempts
     */
    protected function regrade_batch_of_attempts($bayesian, array $attempts,
            bool $dryrun, \core\dml\sql_join $groupstudentsjoins) {
        global $OUTPUT;
        $this->clear_regrade_table($bayesian, $groupstudentsjoins);

        $progressbar = new progress_bar('bayesian_overview_regrade', 500, true);
        $a = array(
        ...
        foreach ($attempts as $attempt) {
            ...
            $progressbar->update($a['done'], $a['count'],
                    get_string('regradingattemptxofywithdetails', 'bayesian_overview', $a));
            $messages = $this->regrade_attempt($attempt, $dryrun, $attempt->regradeonlyslots);
            if ($messages) {
                $items = [];
                foreach ($messages as $slot => $message) {
                    $items[] = get_string('regradingattemptissue', 'bayesian_overview',
                            ['slot' => $slot, 'reason' => $message]);
                }
                echo $OUTPUT->notification(
                        html_writer::tag('p', get_string('regradingattemptxofyproblem', 'bayesian_overview', $a)) .
                        html_writer::alist($items), \core\output\notification::NOTIFY_WARNING);
            }
        }
        $progressbar->update($a['done'], $a['count'],
                get_string('regradedsuccessfullyxofy', 'bayesian_overview', $a));

        if (!$dryrun) {
            $this->update_overall_grades($bayesian);
        }
    }
     ...
     */
    protected function count_question_attempts_needing_regrade($bayesian, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        if ($this->hasgroupstudents) {
            $userjoin = "JOIN {user} u ON u.id = bayesiana.userid
                    {$groupstudentsjoins->joins}";
            $usertest = "{$groupstudentsjoins->wheres} AND u.id = bayesiana.userid AND ";
            $params = $groupstudentsjoins->params;
        }

        $params['cbayesian'] = $bayesian->id;
        $sql = "SELECT COUNT(DISTINCT bayesiana.id)
                  FROM {bayesian_attempts} bayesiana
                  JOIN {bayesian_overview_regrades} qqr ON bayesiana.uniqueid = qqr.questionusageid
                $userjoin
                 WHERE
                      $usertest
                      bayesiana.bayesian = :cbayesian AND
    ...
    protected function has_regraded_questions($from, $where, $params) {
        ...
        return $DB->record_exists_sql("
                SELECT 1
                  FROM {$from}
                  JOIN {bayesian_overview_regrades} qor ON qor.questionusageid = bayesiana.uniqueid
                 WHERE {$where}", $params);
    }
     ...
     * to these users is cleared.
     */
    protected function clear_regrade_table($bayesian, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        $select = "questionusageid IN (
                    SELECT uniqueid
                      FROM {bayesian_attempts} bayesiana";
        $where = "WHERE bayesiana.bayesian = :qid";
        $params = array('qid' => $bayesian->id);
        if ($this->hasgroupstudents && !empty($groupstudentsjoins->joins)) {
            $select .= "\nJOIN {user} u ON u.id = bayesiana.userid
                    {$groupstudentsjoins->joins}";
        ...
        $DB->delete_records_select('bayesian_overview_regrades', $select, $params);
    }
     ...
     */
    protected function update_overall_grades($bayesian) {
    ...
    /**
     * Get the bands configuration for the bayesian.
     *
     * This returns the configuration for having between 11 and 20 bars in
     * a chart based on the maximum grade to be given on a bayesian. The width of
     ...
     */
    public static function get_bands_count_and_width($bayesian) {
        $bands = $bayesian->grade;
        ...
        return [$bands, $bayesian->grade / $bands];
    }
     ...
     */
    public static function get_bands_labels($bands, $bandwidth, $bayesian) {
        $bandlabels = [];
        for ($i = 1; $i <= $bands; $i++) {
            $bandlabels[] = bayesian_format_grade($bayesian, ($i - 1) * $bandwidth) . ' - ' . bayesian_format_grade($bayesian, $i * $bandwidth);
        }
        return $bandlabels;
    }
