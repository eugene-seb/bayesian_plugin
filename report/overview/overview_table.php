...
/**
 * This file defines the bayesian grades table.
 *
...
require_once($CFG->dirroot . '/mod/bayesian/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the bayesian grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_overview_table extends bayesian_attempts_report_table {
     ...
     * @param bayesian_overview_options $options
     * @param \core\dml\sql_join $groupstudentsjoins
     * @param \core\dml\sql_join $studentsjoins
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($bayesian, $context, $qmsubselect,
            bayesian_overview_options $options, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $studentsjoins, $questions, $reporturl) {
        parent::__construct('mod-bayesian-report-overview-report', $bayesian , $context,
    ...
    /**
     * Calculate the average overall and question scores for a set of attempts at the bayesian.
     *
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(bayesianaouter.sumgrades) AS grade, COUNT(bayesianaouter.sumgrades) AS numaveraged
                  FROM {bayesian_attempts} bayesianaouter
                  JOIN (
                       SELECT DISTINCT bayesiana.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON bayesianaouter.id = relevant_attempt_ids.id
                ", $params);
        $record->grade = bayesian_rescale_grade($record->grade, $this->bayesian, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext' => strip_tags(bayesian_report_feedback_for_grade(
                                         $record->grade, $this->bayesian->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join("{bayesian_attempts} bayesianaouter
                  JOIN (
                       SELECT DISTINCT bayesiana.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON bayesianaouter.id = relevant_attempt_ids.id",
                    'bayesianaouter.uniqueid', '1 = 1', $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = bayesian_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->bayesian, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = bayesian_format_question_grade($this->bayesian, $record->grade);
        } else {
            $average = bayesian_format_grade($this->bayesian, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/bayesian:regrade', $this->context)) {
            $regradebuttonparams = [
                ...
                'name'  => 'regrade',
                'value' => get_string('regradeselected', 'bayesian_overview'),
                'data-action' => 'toggle',
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = bayesian_rescale_grade($attempt->sumgrades, $this->bayesian);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = bayesian_rescale_grade($newsumgrade, $this->bayesian);
            $oldsumgrade = bayesian_rescale_grade($oldsumgrade, $this->bayesian);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/bayesian/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'bayesian')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/bayesian/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = bayesian_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->bayesian, 'question');
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = bayesian_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->bayesian, 'question');
            $oldgrade = bayesian_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'bayesian_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'bayesian_overview');
        }
    }
    ...
    protected function update_sql_after_count($fields, $from, $where, $params) {
        $fields .= ", COALESCE((
                                SELECT MAX(qqr.regraded)
                                  FROM {bayesian_overview_regrades} qqr
                                 WHERE qqr.questionusageid = bayesiana.uniqueid
                          ), -1) AS regraded";
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/bayesian:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('bayesian_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return bayesian_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
