...
namespace bayesian_overview;

use core_question\local\bank\question_version_status;
use mod_bayesian\external\submit_question_version;
use question_engine;
use bayesian;
use bayesian_attempt;
use bayesian_attempts_report;
use bayesian_overview_options;
use bayesian_overview_report;
use bayesian_overview_table;
...
require_once($CFG->dirroot . '/mod/bayesian/report/overview/report.php');
require_once($CFG->dirroot . '/mod/bayesian/report/overview/overview_form.php');
require_once($CFG->dirroot . '/mod/bayesian/report/overview/tests/helpers.php');
require_once($CFG->dirroot . '/mod/bayesian/tests/bayesian_question_helper_test_trait.php');


/**
 * Tests for the bayesian overview report.
 *
...
class report_test extends \advanced_testcase {
    ...
    public function test_report_sql(?string $isdownloading): void {
        ...
        // Create a course and a bayesian.
        $generator = $this->getDataGenerator();
        $course = $generator->create_course();
        $bayesiangenerator = $generator->get_plugin_generator('mod_bayesian');
        $bayesian = $bayesiangenerator->create_instance(array('course' => $course->id,
                'grademethod' => bayesian_GRADEHIGHEST, 'grade' => 100.0, 'sumgrades' => 10.0,
                'attempts' => 10));
        ...
        bayesian_add_bayesian_question($q->id, $bayesian, 0 , 10);
        ...
        // because they have a role with neither 'mod/bayesian:attempt'
        // nor 'mod/bayesian:reviewmyattempts'.
        ...
        $attempts = array(
            array($bayesian, $student1, 1, 0.0,  bayesian_attempt::FINISHED),
            array($bayesian, $student1, 2, 5.0,  bayesian_attempt::FINISHED),
            array($bayesian, $student1, 3, 8.0,  bayesian_attempt::FINISHED),
            array($bayesian, $student1, 4, null, bayesian_attempt::ABANDONED),
            array($bayesian, $student1, 5, null, bayesian_attempt::IN_PROGRESS),
            array($bayesian, $student2, 1, null, bayesian_attempt::ABANDONED),
            array($bayesian, $student2, 2, null, bayesian_attempt::ABANDONED),
            array($bayesian, $student2, 3, 7.0,  bayesian_attempt::FINISHED),
            array($bayesian, $student2, 4, null, bayesian_attempt::ABANDONED),
            array($bayesian, $student2, 5, null, bayesian_attempt::ABANDONED),
        );

        // Load it in to bayesian attempts table.
        foreach ($attempts as $attemptdata) {
            list($bayesian, $student, $attemptnumber, $sumgrades, $state) = $attemptdata;
            ...
            $attempt = bayesian_create_attempt($bayesianobj, $attemptnumber, null, $timestart, false, $student->id);

            $attempt = bayesian_start_new_attempt($bayesianobj, $quba, $attempt, $attemptnumber, $timestamp);
            ...
            switch ($state) {
                ...
                case bayesian_attempt::FINISHED:
                    ...
                    $DB->update_record('bayesian_attempts', $update);
                    bayesian_save_best_grade($attemptobj->get_bayesian(), $student->id);
                    break;
            }
        }
        ...
        $qmsubselect = bayesian_report_qm_filter_select($bayesian);
        ...
        // Set the options.
        $reportoptions = new bayesian_overview_options('overview', $bayesian, $cm, null);
        $reportoptions->attempts = bayesian_attempts_report::ENROLLED_ALL;
        $reportoptions->onlygraded = true;
        $reportoptions->states = array(bayesian_attempt::IN_PROGRESS, bayesian_attempt::OVERDUE, bayesian_attempt::FINISHED);

        // Now do a minimal set-up of the table class.
        $q->slot = 1;
        $q->maxmark = 10;
        $table = new bayesian_overview_table($bayesian, $context, $qmsubselect, $reportoptions,
        ...
        $table->define_baseurl(new \moodle_url('/mod/bayesian/report.php'));
        $table->setup();
    ...
    public function test_get_bands_count_and_width(int $grade, array $expected): void {
        ...
        $bayesian = $bayesiangenerator->create_instance(['course' => SITEID, 'grade' => $grade]);
        $this->assertEquals($expected, bayesian_overview_report::get_bands_count_and_width($bayesian));
    }
    ...
    public function test_delete_selected_attempts(): void {
        ...
        $bayesian = $bayesiangenerator->create_instance([
                'course' => $course->id,
                'grademethod' => bayesian_GRADEHIGHEST,
                'grade' => 100.0,
                'sumgrades' => 10.0,
                'attempts' => 10
        ]);
        ...
        $context = \context_module::instance($bayesian->cmid);
        $cm = get_coursemodule_from_id('bayesian', $bayesian->cmid);
        $allowedjoins = get_enrolled_with_capabilities_join($context, '', ['mod/bayesian:attempt', 'mod/bayesian:reviewmyattempts']);
        $bayesianattemptsreport = new \testable_bayesian_attempts_report();

        // Create the new attempt and initialize the question sessions.
        $bayesianobj = bayesian::create($bayesian->id, $student->id);
        $quba = question_engine::make_questions_usage_by_activity('mod_bayesian', $bayesianobj->get_context());
        $quba->set_preferred_behaviour($bayesianobj->get_bayesian()->preferredbehaviour);
        $attempt = bayesian_create_attempt($bayesianobj, 1, null, $timestart, false, $student->id);
        $attempt = bayesian_start_new_attempt($bayesianobj, $quba, $attempt, 1, $timestamp);
        $attempt = bayesian_attempt_save_started($bayesianobj, $quba, $attempt);

        // Delete the student's attempt.
        $bayesianattemptsreport->delete_selected_attempts($bayesian, $cm, [$attempt->id], $allowedjoins);
    }
    ...
    public function test_regrade_question() {
        ...
        $cm = get_fast_modinfo($course->id)->get_cm($bayesian->cmid);
        $context = \context_module::instance($bayesian->cmid);
        ...
        // Add the question to the bayesian.
        bayesian_add_bayesian_question($q2->id, $bayesian, 0, 10);

        // Attempt the bayesian, submitting response 'toad'.
        ...
        // Now change the bayesian to use fixed version 1.
        $slot = $bayesianobj->get_question($q2->id);
        submit_question_version::execute($slot->slotid, 1);

        // Regrade.
        $report = new bayesian_overview_report();
        $report->init('overview', 'bayesian_overview_settings_form', $bayesian, $cm, $course);
        ...
        // Now change the bayesian back to always latest and regrade again.
