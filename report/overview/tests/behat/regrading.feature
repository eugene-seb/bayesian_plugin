@mod @mod_bayesian @bayesian @bayesian_overview @javascript
Feature: Regrading bayesian attempts using the Grades report
    In order to be able to correct mistakes I made setting up my bayesian
    As a teacher
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | bayesian     | bayesian for testing regrading | C1     | bayesian1    |
    And the following "questions" exist:
      | questioncategory | qtype       | name |
      | Test questions   | truefalse   | TF   |
      | Test questions   | shortanswer | SA   |
    And bayesian "bayesian for testing regrading" contains the following questions:
      | question | page | maxmark |
      | TF       | 1    | 5.0     |
      | SA       | 1    | 5.0     |
    And user "student1" has attempted "bayesian for testing regrading" with responses:
      | slot | response |
      | 1    | True     |
      | 2    | frog     |
    And user "student2" has attempted "bayesian for testing regrading" with responses:
      | slot | response |
      | 1    | True     |
      | 2    | toad     |

  Scenario: Regrade all attempts
    Given I am on the "bayesian for testing regrading" "bayesian activity" page logged in as teacher
    And I navigate to "Results" in current page administration
    When I press "Regrade all"

    # Note, the order is not defined, so we can only check part of the message.
    # Also, nothing has changed in the bayesian, so the regrade won't alter any scores,
    # but this is still a useful test that the regrade process completes without errors.
    Then I should see "bayesian for testing regrading"
    ...
    And I should see "bayesian for testing regrading"
  ...
  Scenario: Dry-run a full regrade, then regrade the attempts that will need it.
    Given I am on the "bayesian for testing regrading" "mod_bayesian > edit" page logged in as teacher
  ...
  Scenario: Regrade all attempts works against bayesian selected question version
    Given I am on the "bayesian for testing regrading" "bayesian activity" page logged in as teacher
    And I navigate to "Results" in current page administration
    ...
    And I should see "Overall number of students achieving grade ranges"
    And "Student One" row "Regrade" column of "attempts" table should not contain "Needed"
    And I am on the "bayesian for testing regrading" "mod_bayesian > question bank" page
    And I choose "Edit question" action for "TF" in the question bank
    And I set the field "Correct answer" to "False"
    And I press "id_submitbutton"
    And I am on the "bayesian for testing regrading" "mod_bayesian > edit" page
  ...
  Scenario: Regrade all attempts works against bayesian selected latest question version
    Given I am on the "bayesian for testing regrading" "bayesian activity" page logged in as teacher
    And I navigate to "Results" in current page administration
    And I click on "mod-bayesian-report-overview-report-selectall-attempts" "checkbox"
    ...
    And I set the field "version" in the "TF" "list_item" to "v1"
    And user "student3" has attempted "bayesian for testing regrading" with responses:
      | slot | response |
      | 1    | True     |
      | 2    | toad     |
    And I am on the "bayesian for testing regrading" "mod_bayesian > edit" page
  ...
  Scenario: Regrade attempts should always regrade against latest random question version
    ...
    And I am on the "bayesian for testing regrading" "bayesian activity" page logged in as student3
    And I click on "Attempt bayesian" "button"
    ...
    And I am on the "bayesian for testing regrading" "mod_bayesian > question bank" page logged in as teacher
    And I choose "Edit question" action for "TF" in the question bank
    And I set the field "Correct answer" to "False"
    And I press "id_submitbutton"
    And I navigate to "Results" in current page administration
