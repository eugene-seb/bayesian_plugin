...
/**
 * This file defines the bayesian responses table for showing first or all tries at a question.
 *
...
/**
 * This is a table subclass for displaying the bayesian responses report, showing first or all tries.
 *
 * @package   bayesian_responses
 * @copyright 2014 The Open University
 * @author    Jamie Pratt <me@jamiep.org>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_first_or_all_responses_table extends bayesian_last_responses_table {
        ...
        foreach ($this->rawdata as $attempt) {
            ...
            for ($try = 1; $try <= $maxtriesinanyslot; $try++) {
                ...
                if ($try !== $maxtriesinanyslot) {
                    $newtablerow->state = bayesian_attempt::IN_PROGRESS;
                }
                $newtablerow->try = $try;
    ...
    public function col_sumgrades($tablerow) {
        if ($tablerow->try == 0) {
            // We are showing a user without a bayesian attempt.
            return '-';
        } else if (!$tablerow->lasttryforallparts) {
            // There are more rows to come for this bayesian attempt, so we will show this later.
            return '';
        } else {
