...
Feature: Basic use of the Responses report
  ...
  Background: Using the Responses report
    ...
    And the following "activities" exist:
      ...
      | bayesian       | bayesian 1 | bayesian 1 description | C1     | bayesian1    | interactive        |
  ...
  Scenario: Report works when there are no attempts
    When I am on the "bayesian 1" "mod_bayesian > Responses report" page logged in as teacher
    Then I should see "Attempts: 0"
    And I should see "Nothing to display"
    And I set the field "Attempts from" to "enrolled users who have not attempted the bayesian"

  @javascript
  Scenario: Report works when there are attempts
    Given user "student1" has started an attempt at bayesian "bayesian 1"
    And user "student1" has checked answers in their attempt at bayesian "bayesian 1":
      | slot | response |
      |   1  | 1.0      |
    And user "student1" has checked answers in their attempt at bayesian "bayesian 1":
      | slot | response |
    ...
    And user "student1" has checked answers in their attempt at bayesian "bayesian 1":
      | slot | response |
      |   1  | 3.14     |
    And user "student1" has finished an attempt at bayesian "bayesian 1"

    When I am on the "bayesian 1" "mod_bayesian > Responses report" page logged in as teacher
    Then I should see "Attempts: 1"
    And I should see "Student One"
    And I should not see "Student Two"
    And I set the field "Attempts from" to "enrolled users who have, or have not, attempted the bayesian"
  ...
  Scenario: Report does not allow strange combinations of options
    Given I am on the "bayesian 1" "mod_bayesian > Responses report" page logged in as teacher
    And the "Which tries" "select" should be enabled
    When I set the field "Attempts from" to "enrolled users who have not attempted the bayesian"
