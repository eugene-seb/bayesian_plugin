...
/**
 * Class to store the options for a {@link bayesian_responses_report}.
 *
...
require_once($CFG->dirroot . '/mod/bayesian/report/attemptsreport_options.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_responses_options extends mod_bayesian_attempts_report_options {
    ...
    protected function get_url_params() {
        ...
        if (bayesian_allows_multiple_tries($this->bayesian)) {
    ...
    public function setup_from_user_preferences() {
        ...
        $this->showqtext     = get_user_preferences('bayesian_report_responses_qtext', $this->showqtext);
        $this->showresponses = get_user_preferences('bayesian_report_responses_resp',  $this->showresponses);
        $this->showright     = get_user_preferences('bayesian_report_responses_right', $this->showright);
        if (bayesian_allows_multiple_tries($this->bayesian)) {
            $this->whichtries    = get_user_preferences('bayesian_report_responses_which_tries', $this->whichtries);
        }
    }
    ...
    public function update_user_preferences() {
        ...
        set_user_preference('bayesian_report_responses_qtext', $this->showqtext);
        set_user_preference('bayesian_report_responses_resp',  $this->showresponses);
        set_user_preference('bayesian_report_responses_right', $this->showright);
        if (bayesian_allows_multiple_tries($this->bayesian)) {
            set_user_preference('bayesian_report_responses_which_tries', $this->whichtries);
        }
    }
    ...
    public function resolve_dependencies() {
        ...
        $this->checkboxcolumn = has_capability('mod/bayesian:deleteattempts', context_module::instance($this->cm->id))
                && ($this->attempts != bayesian_attempts_report::ENROLLED_WITHOUT);
    }
}
