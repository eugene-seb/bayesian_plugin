...
/**
 * This file defines the setting form for the bayesian grading report.
 *
...
/**
 * bayesian grading report settings form.
 *
 * @copyright 2010 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_grading_settings_form extends moodleform {
    ...
    /** @var stdClass $context the bayesian context. */
    protected $context;

    /**
     * bayesian_grading_settings_form constructor.
     *
    ...
    public function __construct(array $hidden, stdClass $counts, bool $shownames, bool $showcustomfields, stdClass $context) {
        ...
        parent::__construct($CFG->wwwroot . '/mod/bayesian/report.php');
    }

    protected function definition() {
        $mform = $this->_form;

        $mform->addElement('header', 'options', get_string('options', 'bayesian_grading'));
        ...
        foreach (array('needsgrading', 'manuallygraded', 'autograded', 'all') as $type) {
            ...
            $gradeoptions[$type] = get_string('gradeattempts' . $type, 'bayesian_grading',
                    $this->counts->$type);
        }
        $mform->addElement('select', 'grade', get_string('attemptstograde', 'bayesian_grading'),
                $gradeoptions);

        $mform->addElement('text', 'pagesize', get_string('questionsperpage', 'bayesian_grading'),
                array('size' => 3));
        $mform->addRule('pagesize', null, 'positiveint', null, 'client');
        $mform->setType('pagesize', PARAM_INT);

        $orderoptions = [
            'random' => get_string('random', 'bayesian_grading'),
        ...
        $mform->addElement('select', 'order', get_string('orderattemptsby', 'bayesian_grading'),
                $orderoptions);
        ...
        $mform->addElement('submit', 'submitbutton', get_string('changeoptions', 'bayesian_grading'));
    }
}
