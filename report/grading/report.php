...
/**
 * This file defines the bayesian manual grading report class.
 *
 * @package   bayesian_grading
...
require_once($CFG->dirroot . '/mod/bayesian/report/grading/gradingsettings_form.php');


/**
 * bayesian report to help teachers manually grade questions that need it.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_grading_report extends bayesian_default_report {
    ...
    /** @var array from bayesian_report_get_significant_questions. */
    protected $questions;
    ...
    /** @var stdClass the bayesian settings. */
    protected $bayesian;

    /** @var context the bayesian context. */
    protected $context;

    /** @var bayesian_grading_renderer Renderer of bayesian Grading. */
    protected $renderer;
    ...
    public function display($bayesian, $cm, $course) {
        ...
        $pagesize = optional_param('pagesize',
                get_user_preferences('bayesian_grading_pagesize', self::DEFAULT_PAGE_SIZE),
                PARAM_INT);
        $page = optional_param('page', 0, PARAM_INT);
        $order = optional_param('order',
                get_user_preferences('bayesian_grading_order', self::DEFAULT_ORDER),
                PARAM_ALPHAEXT);
        ...
        require_capability('mod/bayesian:grade', $this->context);
        $shownames = has_capability('bayesian/grading:viewstudentnames', $this->context);
        // Whether the current user can see custom user fields.
        $showcustomfields = has_capability('bayesian/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this bayesian.
        $this->questions = bayesian_report_get_significant_questions($bayesian);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'bayesian_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    array('mod/bayesian:reviewmyattempts', 'mod/bayesian:attempt'), $this->currentgroup);
        }

        $hasquestions = bayesian_has_questions($this->bayesian->id);
        if (!$hasquestions) {
            $this->print_header_and_tabs($cm, $course, $bayesian, 'grading');
            echo $this->renderer->render_bayesian_no_question_notification($bayesian, $cm, $this->context);
            return true;
        }
        ...
        if (!$counts || $counts->$grade == 0) {
            redirect($this->list_questions_url(), get_string('alldoneredirecting', 'bayesian_grading'));
        }
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "bayesiana.bayesian = :mangrbayesianid AND
                bayesiana.preview = 0 AND
                bayesiana.state = :statefinished";
        $params = array('mangrbayesianid' => $this->cm->instance, 'statefinished' => bayesian_attempt::FINISHED);
        ...
        if ($currentgroup) {
            ...
            if ($enrolleduserscount < 1) {
                $where .= ' AND bayesiana.userid = 0';
            } else {
                $usersjoin = "JOIN ({$userssql[0]}) AS enr ON bayesiana.userid = enr.id";
                $params += $userssql[1];
            }
        }

        return new qubaid_join("{bayesian_attempts} bayesiana $usersjoin ", 'bayesiana.uniqueid', $where, $params);
    }

    /**
     * Load the bayesian_attempts rows corresponding to a list of question_usage ids.
     *
     * @param int[] $qubaids the question_usage ids of the bayesian_attempts to load.
     * @return array bayesian attempts, with added user name fields.
     */
    protected function load_attempts_by_usage_ids($qubaids) {
        global $DB;

        list($asql, $params) = $DB->get_in_or_equal($qubaids);
        $params[] = bayesian_attempt::FINISHED;
        $params[] = $this->bayesian->id;
        ...
        $attemptsbyid = $DB->get_records_sql("
                ...
                {$userfieldssql->joins}
                WHERE bayesiana.uniqueid $asql AND bayesiana.state = ? AND bayesiana.bayesian = ?",
                $params);
    ...
    protected function display_index($includeauto) {
        ...
        $this->print_header_and_tabs($this->cm, $this->course, $this->bayesian, 'grading');
        ...
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'bayesian_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'bayesian_grading');
        }
        echo $this->renderer->render_display_index_heading($linktext, $this->list_questions_url(!$includeauto));
        $data = [];
        $header = [];

        $header[] = get_string('qno', 'bayesian_grading');
        $header[] = get_string('qtypeveryshort', 'question');
        $header[] = get_string('questionname', 'bayesian_grading');
        $header[] = get_string('tograde', 'bayesian_grading');
        $header[] = get_string('alreadygraded', 'bayesian_grading');
        if ($includeauto) {
            $header[] = get_string('automaticallygraded', 'bayesian_grading');
        }
        $header[] = get_string('total', 'bayesian_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new bayesian_grading_settings_form($hidden, $counts, $shownames, $showcustomfields, $this->context);
        ...
        if ($mform->is_submitted()) {
            if ($mform->is_validated()) {
                ...
                set_user_preference('bayesian_grading_pagesize', $pagesize);
                set_user_preference('bayesian_grading_order', $order);
                ...
                $pagesize = get_user_preferences('bayesian_grading_pagesize', self::DEFAULT_PAGE_SIZE);
            }
        }
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = bayesian_get_review_options($this->bayesian, $attempt, $this->context);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new bayesian_attempt($attempt, $this->bayesian, $this->cm, $this->course);
            ...
            $events[] = \mod_bayesian\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby === 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON bayesiana.userid = u.id {$userfieldssql->joins}";
     ...
     */
    public function print_header_and_tabs($cm, $course, $bayesian, $reportmode = 'overview') {
        global $PAGE;
        $this->renderer = $PAGE->get_renderer('bayesian_grading');
        parent::print_header_and_tabs($cm, $course, $bayesian, $reportmode);
    }

    /**
     * Get question heading.
     *
     * @param stdClass $attempt An instance of bayesian_attempt.
    ...
    protected function get_question_heading(stdClass $attempt, bool $shownames, bool $showcustomfields): string {
        ...
        if ($shownames && $showcustomfields) {
            return get_string('gradingattemptwithcustomfields', 'bayesian_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'bayesian_grading', $a);
