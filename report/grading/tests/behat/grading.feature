@mod @mod_bayesian @bayesian @bayesian_grading
Feature: Basic use of the Manual grading report
  ...
  Scenario: Manual grading report with attempts
    Given user "student1" has attempted "bayesian 1" with responses:
      | slot | response |
      | 1    | Paris    |
  ...
  Scenario: Manual grading settings are remembered as user preferences
    ...
    And I press "Change options"
    And I log out
    And I am on the "bayesian 1" "mod_bayesian > Manual grading report" page logged in as "teacher1"
    And I follow "Also show questions that have been graded automatically"
    And I click on "update grades" "link" in the "Short answer 001" "table_row"
  ...
  Scenario: A marker cannot access the report in separate group
    ...
    And user "student2" has attempted "bayesian 1" with responses:
      | slot | response |
      | 1    | Duck     |
    When I am on the "bayesian 1" "mod_bayesian > Manual grading report" page logged in as "marker"
    Then I should see "bayesian 1"
