...
/**
 * Defines the renderer for the bayesian_grading module.
 *
...
/**
 * The renderer for the bayesian_grading module.
 *
 * @copyright  2018 Huong Nguyen <huongnv13@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_grading_renderer extends plugin_renderer_base {
     ...
     */
    public function render_bayesian_no_question_notification($bayesian, $cm, $context) {
        return bayesian_no_questions_message($bayesian, $cm, $context);
    }
     ...
     */
    public function render_bayesian_no_grade_question_notification() {
        return $this->notification(get_string('nothingfound', 'bayesian_grading'));
    }
    ...
    public function render_display_index_heading($linktext, $listquestionurl) {
        ...
        $output .= $this->heading(get_string('questionsthatneedgrading', 'bayesian_grading'), 3);
    ...
    public function render_questions_table($includeauto, $data, $header) {
        if (empty($data)) {
            return $this->render_bayesian_no_grade_question_notification();
        }
        $output = '';
    ...
    public function render_grade_link($counts, $type, $gradestring, $gradequestionurl) {
        ...
        if ($counts->$type > 0) {
            $output .= ' ' . html_writer::link(
                            $gradequestionurl,
                            get_string($gradestring, 'bayesian_grading'),
     ...
     * @param bayesian_grading_settings_form $filterform Question filter form.
    ...
    public function render_grading_interface($questioninfo, $listquestionsurl, $filterform, $paginginfo, $pagingbar, $formaction,
        ...
        $output .= $this->heading(get_string('gradingquestionx', 'bayesian_grading', $questioninfo), 3);

        $output .= html_writer::tag('p', html_writer::link($listquestionsurl,
                get_string('backtothelistofquestions', 'bayesian_grading')),
                ['class' => 'mdl-align']);

        $output .= $filterform->render();

        $output .= $this->heading(get_string('gradingattemptsxtoyofz', 'bayesian_grading', $paginginfo), 3);
        ...
        $output .= html_writer::tag('div', html_writer::empty_tag('input', [
                ...
                'value' => get_string('saveandnext', 'bayesian_grading')
        ]), ['class' => 'mdl-align']);
