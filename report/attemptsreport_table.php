...
/**
 * Base class for the table used by a {@link bayesian_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class bayesian_attempts_report_table extends table_sql {
    ...
    /** @var object the bayesian settings for the bayesian we are reporting on. */
    protected $bayesian;
    ...
    /** @var object mod_bayesian_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this bayesian.. */
    protected $questions;
    ...
    protected $togglegroup = 'bayesian-attempts';
     ...
     * @param mod_bayesian_attempts_report_options $options
     ...
     */
    public function __construct($uniqueid, $bayesian, $context, $qmsubselect,
            mod_bayesian_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_checkbox($attempt) {
        ...
        if ($attempt->attempt) {
            $checkbox = new \core\output\checkbox_toggleall($this->togglegroup, false, [
                ...
                'label' => get_string('selectattempt', 'bayesian'),
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/bayesian/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'bayesian'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return bayesian_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = bayesian_report_feedback_for_grade(
                bayesian_rescale_grade($attempt->sumgrades, $this->bayesian, false),
                $this->bayesian->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/bayesian/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'bayesian')));
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as bayesiana.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(bayesiana.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                bayesiana.uniqueid AS usageid,
                bayesiana.id AS attempt,
                u.id AS userid,
                u.idnumber,
                u.picture,
                u.imagealt,
                ...
                bayesiana.state,
                bayesiana.sumgrades,
                bayesiana.timefinish,
                bayesiana.timestart,
                CASE WHEN bayesiana.timefinish = 0 THEN null
                     WHEN bayesiana.timefinish > bayesiana.timestart THEN bayesiana.timefinish - bayesiana.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and bayesian_attempts tables.
        $from = " {user} u";
        $from .= "\n{$userfields->joins}";
        $from .= "\nLEFT JOIN {bayesian_attempts} bayesiana ON
                                    bayesiana.userid = u.id AND bayesiana.bayesian = :bayesianid";
        $params = array_merge($userfields->params, ['bayesianid' => $this->bayesian->id]);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (bayesiana.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = bayesian_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case bayesian_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'bayesiana.id IS NOT NULL AND bayesiana.preview = 0';
                break;
            case bayesian_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "bayesiana.preview = 0 AND bayesiana.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case bayesian_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "bayesiana.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case bayesian_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(bayesiana.preview = 0 OR bayesiana.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (bayesiana.state $statesql OR bayesiana.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{bayesian_attempts} {$alias}bayesiana", "{$alias}bayesiana.uniqueid",
                "{$alias}bayesiana.bayesian = :{$alias}bayesianid", array("{$alias}bayesianid" => $this->sql->params['bayesianid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = bayesiana.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT bayesiana.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) bayesianasubquery", 'bayesianasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['bayesiana.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/bayesian:deleteattempts', $this->context)) {
            $deletebuttonparams = [
                'type'  => 'submit',
                'class' => 'btn btn-secondary mr-1',
                'id'    => 'deleteattemptsbutton',
                'name'  => 'delete',
                'value' => get_string('deleteselected', 'bayesian_overview'),
                'data-action' => 'toggle',
            ...
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog',
                    array('message' => get_string('deleteattemptcheck', 'bayesian')));
        }
    }
     ...
     *
     * It returns the HTML for a master \core\output\checkbox_toggleall component that selects/deselects all bayesian attempts.
     *
    ...
    public function checkbox_col_header(string $columnname) {
        ...
        $selectalltext = get_string('selectall', 'bayesian');
        $deselectalltext = get_string('selectnone', 'bayesian');
