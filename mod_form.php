...
/**
 * Defines the bayesian module ettings form.
 *
...
/**
 * Settings form for the bayesian module.
 *
 * @copyright  2006 Jamie Pratt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_bayesian_mod_form extends moodleform_mod {
    /** @var array options to be used with date_time_selector fields in the bayesian. */
    public static $datefieldoptions = array('optional' => true);

    protected $_feedbacks;
    protected static $reviewfields = array(); // Initialised in the constructor.

    /** @var int the max number of attempts allowed in any user or group override on this bayesian. */
    protected $maxattemptsanyoverride = null;

    public function __construct($current, $section, $cm, $course) {
        self::$reviewfields = array(
            'attempt'          => array('theattempt', 'bayesian'),
            'correctness'      => array('whethercorrect', 'question'),
            'marks'            => array('marks', 'bayesian'),
            ...
            'overallfeedback'  => array('reviewoverallfeedback', 'bayesian'),
        );
    ...
    protected function definition() {
        ...
        $bayesianconfig = get_config('bayesian');
        $mform = $this->_form;
        ...
        // Introduction.
        $this->standard_intro_elements(get_string('introduction', 'bayesian'));

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'timing', get_string('timing', 'bayesian'));

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen', get_string('bayesianopen', 'bayesian'),
                self::$datefieldoptions);
        $mform->addHelpButton('timeopen', 'bayesianopenclose', 'bayesian');

        $mform->addElement('date_time_selector', 'timeclose', get_string('bayesianclose', 'bayesian'),
                self::$datefieldoptions);

        // Time limit.
        $mform->addElement('duration', 'timelimit', get_string('timelimit', 'bayesian'),
                array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'bayesian');

        // What to do with overdue attempts.
        $mform->addElement('select', 'overduehandling', get_string('overduehandling', 'bayesian'),
                bayesian_get_overdue_handling_options());
        $mform->addHelpButton('overduehandling', 'overduehandling', 'bayesian');
        ...
        // Grace period time.
        $mform->addElement('duration', 'graceperiod', get_string('graceperiod', 'bayesian'),
                array('optional' => true));
        $mform->addHelpButton('graceperiod', 'graceperiod', 'bayesian');
        ...
        if (property_exists($this->current, 'grade')) {
            ...
            $currentgrade = $bayesianconfig->maximumgrade;
        }
        ...
        $mform->addElement('select', 'attempts', get_string('attemptsallowed', 'bayesian'),
                $attemptoptions);

        // Grading method.
        $mform->addElement('select', 'grademethod', get_string('grademethod', 'bayesian'),
                bayesian_get_grading_options());
        $mform->addHelpButton('grademethod', 'grademethod', 'bayesian');
        if ($this->get_max_attempts_for_any_override() < 2) {
            $mform->hideIf('grademethod', 'attempts', 'eq', 1);
        }

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'layouthdr', get_string('layout', 'bayesian'));

        $pagegroup = array();
        $pagegroup[] = $mform->createElement('select', 'questionsperpage',
                get_string('newpage', 'bayesian'), bayesian_questions_per_page_options(), array('id' => 'id_questionsperpage'));
        $mform->setDefault('questionsperpage', $bayesianconfig->questionsperpage);

        if (!empty($this->_cm) && !bayesian_has_attempts($this->_cm->instance)) {
            $pagegroup[] = $mform->createElement('checkbox', 'repaginatenow', '',
                    get_string('repaginatenow', 'bayesian'), array('id' => 'id_repaginatenow'));
        }

        $mform->addGroup($pagegroup, 'questionsperpagegrp',
                get_string('newpage', 'bayesian'), null, false);
        $mform->addHelpButton('questionsperpagegrp', 'newpage', 'bayesian');
        $mform->setAdvanced('questionsperpagegrp', $bayesianconfig->questionsperpage_adv);

        // Navigation method.
        $mform->addElement('select', 'navmethod', get_string('navmethod', 'bayesian'),
                bayesian_get_navigation_options());
        $mform->addHelpButton('navmethod', 'navmethod', 'bayesian');

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'interactionhdr', get_string('questionbehaviour', 'bayesian'));

        // Shuffle within questions.
        $mform->addElement('selectyesno', 'shuffleanswers', get_string('shufflewithin', 'bayesian'));
        $mform->addHelpButton('shuffleanswers', 'shufflewithin', 'bayesian');
        ...
        $redochoices = array(0 => get_string('no'), 1 => get_string('canredoquestionsyes', 'bayesian'));
        $mform->addElement('select', 'canredoquestions', get_string('canredoquestions', 'bayesian'), $redochoices);
        $mform->addHelpButton('canredoquestions', 'canredoquestions', 'bayesian');
        ...
        $mform->addHelpButton('attemptonlast', 'eachattemptbuildsonthelast', 'bayesian');
        if ($this->get_max_attempts_for_any_override() < 2) {
        ...
        $mform->addHelpButton('reviewoptionshdr', 'reviewoptionsheading', 'bayesian');

        // Review options.
        $this->add_review_options_group($mform, $bayesianconfig, 'during',
                mod_bayesian_display_options::DURING, true);
        $this->add_review_options_group($mform, $bayesianconfig, 'immediately',
                mod_bayesian_display_options::IMMEDIATELY_AFTER);
        $this->add_review_options_group($mform, $bayesianconfig, 'open',
                mod_bayesian_display_options::LATER_WHILE_OPEN);
        $this->add_review_options_group($mform, $bayesianconfig, 'closed',
        ...
        // Show user picture.
        $mform->addElement('select', 'showuserpicture', get_string('showuserpicture', 'bayesian'),
                bayesian_get_user_image_options());
        $mform->addHelpButton('showuserpicture', 'showuserpicture', 'bayesian');
        ...
        $mform->addElement('select', 'decimalpoints', get_string('decimalplaces', 'bayesian'),
                $options);
        $mform->addHelpButton('decimalpoints', 'decimalplaces', 'bayesian');
        ...
        $mform->addElement('select', 'questiondecimalpoints',
                get_string('decimalplacesquestion', 'bayesian'), $options);
        $mform->addHelpButton('questiondecimalpoints', 'decimalplacesquestion', 'bayesian');

        // Show blocks during bayesian attempt.
        $mform->addElement('selectyesno', 'showblocks', get_string('showblocks', 'bayesian'));
        $mform->addHelpButton('showblocks', 'showblocks', 'bayesian');

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'security', get_string('extraattemptrestrictions', 'bayesian'));

        // Require password to begin bayesian attempt.
        $mform->addElement('passwordunmask', 'bayesianpassword', get_string('requirepassword', 'bayesian'));
        $mform->setType('bayesianpassword', PARAM_TEXT);
        $mform->addHelpButton('bayesianpassword', 'requirepassword', 'bayesian');

        // IP address.
        $mform->addElement('text', 'subnet', get_string('requiresubnet', 'bayesian'));
        $mform->setType('subnet', PARAM_TEXT);
        $mform->addHelpButton('subnet', 'requiresubnet', 'bayesian');

        // Enforced time delay between bayesian attempts.
        $mform->addElement('duration', 'delay1', get_string('delay1st2nd', 'bayesian'),
                array('optional' => true));
        $mform->addHelpButton('delay1', 'delay1st2nd', 'bayesian');
        if ($this->get_max_attempts_for_any_override() < 2) {
            $mform->hideIf('delay1', 'attempts', 'eq', 1);
        }

        $mform->addElement('duration', 'delay2', get_string('delaylater', 'bayesian'),
                array('optional' => true));
        $mform->addHelpButton('delay2', 'delaylater', 'bayesian');
        ...
        $mform->addElement('select', 'browsersecurity', get_string('browsersecurity', 'bayesian'),
                bayesian_access_manager::get_browser_security_choices());
        $mform->addHelpButton('browsersecurity', 'browsersecurity', 'bayesian');

        // Any other rule plugins.
        bayesian_access_manager::add_settings_form_fields($this, $mform);

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'overallfeedbackhdr', get_string('overallfeedback', 'bayesian'));
        $mform->addHelpButton('overallfeedbackhdr', 'overallfeedback', 'bayesian');

        if (isset($this->current->grade)) {
            $needwarning = $this->current->grade === 0;
        } else {
            $needwarning = $bayesianconfig->maximumgrade == 0;
        }
        if ($needwarning) {
            $mform->addElement('static', 'nogradewarning', '',
                    get_string('nogradewarning', 'bayesian'));
        }

        $mform->addElement('static', 'gradeboundarystatic1',
                get_string('gradeboundary', 'bayesian'), '100%');

        $repeatarray = array();
        $repeatedoptions = array();
        $repeatarray[] = $mform->createElement('editor', 'feedbacktext',
                get_string('feedback', 'bayesian'), array('rows' => 3), array('maxfiles' => EDITOR_UNLIMITED_FILES,
                        'noclean' => true, 'context' => $this->context));
        $repeatarray[] = $mform->createElement('text', 'feedbackboundaries',
                get_string('gradeboundary', 'bayesian'), array('size' => 10));
        $repeatedoptions['feedbacktext']['type'] = PARAM_RAW;
        $repeatedoptions['feedbackboundaries']['type'] = PARAM_RAW;

        if (!empty($this->_instance)) {
            $this->_feedbacks = $DB->get_records('bayesian_feedback',
                    array('bayesianid' => $this->_instance), 'mingrade DESC');
            $numfeedbacks = count($this->_feedbacks);
        } else {
            $this->_feedbacks = array();
            $numfeedbacks = $bayesianconfig->initialnumfeedbacks;
        }
        $numfeedbacks = max($numfeedbacks, 1);

        $nextel = $this->repeat_elements($repeatarray, $numfeedbacks - 1,
                $repeatedoptions, 'boundary_repeats', 'boundary_add_fields', 3,
                get_string('addmoreoverallfeedbacks', 'bayesian'), true);

        // Put some extra elements in before the button.
        $mform->insertElementBefore($mform->createElement('editor',
                "feedbacktext[$nextel]", get_string('feedback', 'bayesian'), array('rows' => 3),
                array('maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true,
                      'context' => $this->context)),
                'boundary_add_fields');
        $mform->insertElementBefore($mform->createElement('static',
                'gradeboundarystatic2', get_string('gradeboundary', 'bayesian'), '0%'),
                'boundary_add_fields');
        ...
        $mform->setAdvanced('modoutcomes', !empty($bayesianconfig->outcomes_adv));

        // The standard_coursemodule_elements method sets this to 100, but the
        // bayesian has its own setting, so use that.
        $mform->setDefault('grade', $bayesianconfig->maximumgrade);

        // -------------------------------------------------------------------------------
        $this->apply_admin_defaults();
        $this->add_action_buttons();

        $PAGE->requires->yui_module('moodle-mod_bayesian-modform', 'M.mod_bayesian.modform.init');
    }

    protected function add_review_options_group($mform, $bayesianconfig, $whenname,
            $when, $withhelp = false) {
        global $OUTPUT;
        ...
        $mform->addGroup($group, $whenname . 'optionsgrp',
                get_string('review' . $whenname, 'bayesian'), null, false);

        foreach (self::$reviewfields as $field => $notused) {
            $cfgfield = 'review' . $field;
            if ($bayesianconfig->$cfgfield & $when) {
    ...
    public function data_preprocessing(&$toform) {
        ...
        if (count($this->_feedbacks)) {
            ...
            foreach ($this->_feedbacks as $feedback) {
                ...
                if ($toform['grade'] == 0) {
                    // When a bayesian is un-graded, there can only be one lot of
                    // feedback. If the bayesian previously had a maximum grade and
        ...
        if (isset($toform['password'])) {
            $toform['bayesianpassword'] = $toform['password'];
            unset($toform['password']);
        }

        // Load any settings belonging to the access rules.
        if (!empty($toform['instance'])) {
            $accesssettings = bayesian_access_manager::load_settings($toform['instance']);
    ...
    public function validation($data, $files) {
        ...
        if ($data['timeopen'] != 0 && $data['timeclose'] != 0 &&
            ...
            $errors['timeclose'] = get_string('closebeforeopen', 'bayesian');
        }
        ...
        if ($data['overduehandling'] == 'graceperiod') {
            ...
            if ($data['graceperiod'] <= $graceperiodmin) {
                $errors['graceperiod'] = get_string('graceperiodtoosmall', 'bayesian', format_time($graceperiodmin));
            }
        }

        if (!empty($data['completionminattempts'])) {
            if ($data['attempts'] > 0 && $data['completionminattempts'] > $data['attempts']) {
                $errors['completionminattemptsgroup'] = get_string('completionminattemptserror', 'bayesian');
            }
        }
        ...
        while (!empty($data['feedbackboundaries'][$i] )) {
                ...
                if ($boundary[strlen($boundary) - 1] == '%') {
                    ...
                    if (is_numeric($boundary)) {
                        ...
                        $errors["feedbackboundaries[$i]"] =
                                get_string('feedbackerrorboundaryformat', 'bayesian', $i + 1);
                    }
            ...
            if (is_numeric($boundary) && $boundary <= 0 || $boundary >= $data['grade'] ) {
                $errors["feedbackboundaries[$i]"] =
                        get_string('feedbackerrorboundaryoutofrange', 'bayesian', $i + 1);
            }
            if (is_numeric($boundary) && $i > 0 &&
                    $boundary >= $data['feedbackboundaries'][$i - 1]) {
                $errors["feedbackboundaries[$i]"] =
                        get_string('feedbackerrororder', 'bayesian', $i + 1);
            }
        ...
        if (!empty($data['feedbackboundaries'])) {
            for ($i = $numboundaries; $i < count($data['feedbackboundaries']); $i += 1) {
                if (!empty($data['feedbackboundaries'][$i] ) &&
                    ...
                    $errors["feedbackboundaries[$i]"] =
                            get_string('feedbackerrorjunkinboundary', 'bayesian', $i + 1);
                }
            }
        }
        for ($i = $numboundaries + 1; $i < count($data['feedbacktext']); $i += 1) {
            if (!empty($data['feedbacktext'][$i]['text']) &&
                ...
                $errors["feedbacktext[$i]"] =
                        get_string('feedbackerrorjunkinfeedback', 'bayesian', $i + 1);
            }
        }
        ...
        // Any other rule plugins.
        $errors = bayesian_access_manager::validate_settings_form_fields($errors, $data, $files, $this);
    ...
    public function add_completion_rules() {
        ...
        $mform->addElement('advcheckbox', 'completionattemptsexhausted', null,
            get_string('completionattemptsexhausted', 'bayesian'),
        ...
        $group[] = $mform->createElement('checkbox', 'completionminattemptsenabled', '',
            get_string('completionminattempts', 'bayesian'));
        $group[] = $mform->createElement('text', 'completionminattempts', '', array('size' => 3));
        $mform->setType('completionminattempts', PARAM_INT);
        $mform->addGroup($group, 'completionminattemptsgroup', get_string('completionminattemptsgroup', 'bayesian'), array(' '), false);
    ...
    public function get_max_attempts_for_any_override() {
        ...
        if (empty($this->_instance)) {
            // bayesian not created yet, so no overrides.
            return 1;
        }
