...
/**
 * This page handles editing and creation of bayesian overrides
 *
...
$url = new moodle_url('/mod/bayesian/overrideedit.php');
if ($action) {
...
// Merge bayesian defaults with data.
$keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $bayesian->{$key};
    }
}
...
$overridelisturl = new moodle_url('/mod/bayesian/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new bayesian_override_form($url, $cm, $bayesian, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->bayesian = $bayesian->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $bayesian->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('bayesian_overrides', $conditions)) {
            ...
            $bayesian->cmid = $cm->id;
            bayesian_delete_override($bayesian, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('bayesian_overrides', $fromform);
        $cachekey = $groupmode ? "{$fromform->bayesian}_g_{$fromform->groupid}" : "{$fromform->bayesian}_u_{$fromform->userid}";
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('bayesian_overrides', $fromform);
        $cachekey = $groupmode ? "{$fromform->bayesian}_g_{$fromform->groupid}" : "{$fromform->bayesian}_u_{$fromform->userid}";
        cache::make('mod_bayesian', 'overrides')->delete($cachekey);
    ...
    if ($groupmode) {
        ...
        bayesian_update_events($bayesian, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'bayesian');
...
$PAGE->activityheader->set_attrs([
    "title" => format_string($bayesian->name, true, array('context' => $context)),
    "description" => "",
    "hidecompletion" => true
]);
echo $OUTPUT->header();
