...
/**
 * Settings form for overrides in the bayesian module.
 *
...
require_once($CFG->dirroot . '/mod/bayesian/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class bayesian_override_form extends moodleform {

    /** @var cm_info course module object. */
    protected $cm;

    /** @var stdClass the bayesian settings object. */
    protected $bayesian;
     ...
     * @param object the bayesian settings object.
     * @param context the bayesian context.
     ...
     */
    public function __construct($submiturl, $cm, $bayesian, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'bayesian'));

        $bayesiangroupmode = groups_get_activity_groupmode($cm);
        $accessallgroups = ($bayesiangroupmode == NOGROUPS) || has_capability('moodle/site:accessallgroups', $this->context);
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'bayesian'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/bayesian/overrides.php', array('cmid'=>$cm->id));
                    throw new \moodle_exception('groupsnone', 'bayesian', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'bayesian'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $capabilityjoin = get_with_capability_join($this->context, 'mod/bayesian:attempt', 'u.id');
                ...
                $users = $DB->get_records_sql("
                        SELECT $userfieldsql->selects
                          FROM {user} u
                          LEFT JOIN {bayesian_overrides} existingoverride ON
                                      existingoverride.userid = u.id AND existingoverride.bayesian = :bayesianid
                         ...
                         WHERE existingoverride.id IS NULL
                           $capabilitywhere
                      ORDER BY $sort
                        ", array_merge(['bayesianid' => $this->bayesian->id], $capabilityjoin->params,
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/bayesian/overrides.php', array('cmid'=>$cm->id));
                    throw new \moodle_exception('usersnone', 'bayesian', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'bayesian'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'bayesian');
        $mform->setDefault('password', $this->bayesian->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('bayesianopen', 'bayesian'), mod_bayesian_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->bayesian->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('bayesianclose', 'bayesian'), mod_bayesian_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->bayesian->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'bayesian'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'bayesian');
        $mform->setDefault('timelimit', $this->bayesian->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'bayesian'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'bayesian');
        $mform->setDefault('attempts', $this->bayesian->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'bayesian'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'bayesian'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'bayesian'));
    ...
    public function validation($data, $files): array {
        ...
        // Ensure that at least one bayesian setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $bayesian->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'bayesian');
        }
